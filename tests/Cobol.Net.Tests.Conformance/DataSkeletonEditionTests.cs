...
public sealed class DataSkeletonEditionTests
{
    ...
    /// silently misbound to DISPLAY while the full form was recognized; both must now emit the SAME
    /// binary-usage diagnostic.</summary>
    [Fact]
    public void BareAndFullBinaryCharSigned_GateIdentically()
    {
        var (okBare, bareErrors, _) = EditionHarness.CompileFull(
            Prog("DSKBARE1", "01 WS-C PIC S9(4) BINARY-CHAR SIGNED."), 2023);
        var (okFull, fullErrors, _) = EditionHarness.CompileFull(
            Prog("DSKFULL1", "01 WS-C PIC S9(4) USAGE IS BINARY-CHAR SIGNED."), 2023);
        Assert.False(okBare, "bare BINARY-CHAR SIGNED must not compile silently");
        Assert.False(okFull, "USAGE IS BINARY-CHAR SIGNED must not compile silently");
        EditionHarness.AssertHasDiagnostic(bareErrors, "BINARY-CHAR");
        EditionHarness.AssertHasDiagnostic(fullErrors, "BINARY-CHAR");
        EditionHarness.AssertHasDiagnostic(bareErrors, "COBOLNET0899");
        EditionHarness.AssertHasDiagnostic(fullErrors, "COBOLNET0899");
    }
