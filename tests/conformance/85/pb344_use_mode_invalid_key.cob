      ...
      *> I-O status is set, 9.1.13.1), so each "DECL-" line precedes the status line of the case that raised it.
       ...
       FILE-CONTROL.
           SELECT F ASSIGN TO "pb344k85f.dat"
               ORGANIZATION IS INDEXED
               ACCESS MODE IS DYNAMIC
               RECORD KEY IS F-K
               FILE STATUS IS F-ST.
           SELECT G ASSIGN TO "pb344k85g.dat"
               ORGANIZATION IS INDEXED
               ACCESS MODE IS DYNAMIC
               RECORD KEY IS G-K
               FILE STATUS IS G-ST.
           SELECT OPTIONAL OPTI ASSIGN TO "pb344k85i.dat"
               ORGANIZATION IS INDEXED
               ACCESS MODE IS RANDOM
               RECORD KEY IS OI-K
               FILE STATUS IS OI-ST.
           SELECT OPTIONAL OPTR ASSIGN TO "pb344k85r.dat"
