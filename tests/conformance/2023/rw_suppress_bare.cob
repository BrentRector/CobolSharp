      *> ISO 5.2.3 — PRINTING is printed WITHOUT an underline in the 14.9.45.2 general format (measured on
      *> page 795: SUPPRESS carries an underline rule of 45.3 pt, PRINTING carries none), so it is an OPTIONAL
      *> WORD and a bare `SUPPRESS` is conforming. The grammar required it until 2026-07-27.
      *> This is rw_suppress with PRINTING OMITTED and NOTHING ELSE CHANGED, and it pins the SAME expected
      *> output byte-for-byte — which is the actual claim of 5.2.3: omitting an optional word is a different
      *> SPELLING of the same statement, not a different statement.
      *> ISO §14.9.45 SUPPRESS statement. A USE BEFORE REPORTING procedure
      *> (§14.9.49 Format 2) on the detail group executes SUPPRESS
      ...
      *> conditionally per GENERATE, so the surrounding shown rows print.
       ...
       FILE-CONTROL.
           SELECT RPT ASSIGN TO "rw-suppress-bare.rpt".
           SELECT RBACK ASSIGN TO "rw-suppress-bare.rpt"
