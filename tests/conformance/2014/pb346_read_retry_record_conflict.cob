      ...
      *>
      *> THE COBOL-2014 TWIN of 2023/pb346_read_retry_record_conflict, byte
      *> for byte the same expectations: Annex E lists no change to 14.7.9
      *> or to 14.9.30.4 GR9 between the two editions, so the inventory row
      *> GR-14.9.30.4-9 claims both and both are evidenced. (The RETRY
      *> phrase is a COBOL-2002 introduction; its gating diagnostic below
      *> 2002 is COBOLNET0900, pinned by negative/retry_below_2002.)
       ENVIRONMENT DIVISION.
       INPUT-OUTPUT SECTION.
       FILE-CONTROL.
      *> Format 2 (ACCESS MODE RANDOM) — the keyed post-read governance path.
           SELECT RA ASSIGN TO "pb346rt4-r.dat"
           ...
           SELECT RB ASSIGN TO "pb346rt4-r.dat"
               ORGANIZATION IS RELATIVE
               ACCESS MODE IS RANDOM
      ...
      *> Format 1 (ACCESS MODE SEQUENTIAL) — the pre-read conflict leg.
           SELECT SA ASSIGN TO "pb346rt4-s.dat"
               ORGANIZATION IS SEQUENTIAL
               ACCESS MODE IS SEQUENTIAL
               SHARING WITH ALL OTHER
               LOCK MODE IS MANUAL
               FILE STATUS IS SA-ST.
           SELECT SB ASSIGN TO "pb346rt4-s.dat"
               ORGANIZATION IS SEQUENTIAL
               ACCESS MODE IS SEQUENTIAL
               SHARING WITH ALL OTHER
               LOCK MODE IS MANUAL
