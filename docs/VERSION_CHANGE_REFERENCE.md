...
- `<!-- behavior:BEHAVIOR-ID -->` — the row is a per-edition BEHAVIOUR change (its gating action is
  `gate-behavior-by-dialect`) gated by that `CobolNet.Runtime.DialectBehaviors` row. It is a separate anchor
  kind because such a row can never be a `constructs.json` row: the source is legal at every edition, only the
  ANSWER differs, so there is no diagnostic for a construct row to carry. `DialectBehaviorRegistryDriftTests`
  holds the two registers together in both directions (kb/Work PB344);
  ⛔ **THE ADMISSION TEST** (kb/Work PB344, earned by a gate that had to be withdrawn): Annex E is
  INFORMATIVE and §E.1 scopes it to "a list of the substantive changes between the previous COBOL standard
  and this Working Draft International Standard" — ONE prior edition. An E.2 item earns a `behavior:` anchor
  only when it says what the prior rule STATED. Where its justification says instead that the previous
  standard "was not clear or missing processing", the prior text was SILENT, silence is not a prohibition,
  and the row is `ref-only`: the 2023 rule stands at every edition;
...
| 25 | Item 19a — INVALID KEY processing: declarative now executed when phrase absent | INVALID KEY; USE declaratives (E.2 item 19a) | 2014→2023 | **Old:** no INVALID KEY phrase + invalid-key condition → INPUT/OUTPUT/I-O/EXTEND declarative NOT executed (apparent error). **New:** such a declarative is now executed. ⚠ **DETERMINATION — NOT GATED (kb/Work PB344).** Annex E is INFORMATIVE and §E.1 scopes it to "a list of the substantive changes between the previous COBOL standard and this Working Draft International Standard" — ONE prior edition — while item 19's own justification says the previous standard "was not clear or missing processing of some I-O exceptions" and the sub-item adds "This appears to be an error in previous COBOL standards". A silence is not a prohibition, so there is no prior behaviour to serve; the 2023 rule stands at 85/2002/2014 as well. Its sibling (row 26) is refuted outright for 1985, and nothing distinguishes the two halves' evidence. Pinned by conformance:{85,2002,2014,2023}/pb344_use_mode_invalid_key, whose four copies expect the same lines. | Yes | ref-only — clarification of defective wording, no gate obligation <!-- ref-only --> |
| 26 | Item 19b — READ processing: declarative now executed for non-invalid-key/non-at-end exceptions | READ; USE declaratives (E.2 item 19b) | 2014→2023 | **Old:** READ exception that is not invalid-key/at-end → INPUT/I-O declarative NOT executed (apparent error). **New:** such a declarative is now executed (part of the inline-exception-checking PERFORM enhancement). ⚠ **DETERMINATION — NOT GATED (kb/Work PB344).** The same §E.1 scope and "not clear or missing processing" justification as row 25, PLUS a positive refutation for 1985: NIST CCVS SQ122A, SQ136A, SQ137A and SQ138A require the INPUT declarative to run for a '46' READ and SQ148A the OUTPUT one for a '47' READ — SQ137A fails with the literal remark "INPUT DECLARATIVE NOT EXECUTED" against ANSI X3.23-1985 VII-2 1.3.5, VII-51 4.6.4(5). A gate here made five NIST programs RED on CI (train 29, 2026-09-13) and was withdrawn. Pinned by conformance:{85,2002,2014,2023}/pb344_use_mode_read_exception, whose four copies expect the same lines. | Yes | ref-only — clarification of defective wording, no gate obligation <!-- ref-only --> |
...
| 29 | Item 22 — READ PREVIOUS immediately after OPEN now raises at-end — **on the INDEXED organization only** | READ … PREVIOUS following OPEN on an INDEXED file (E.2 item 22) | 2014→2023 | **Old:** conflicting rule/note: rule said the first record is retrieved; note said at-end normally exists. **New:** at-end condition occurs for READ PREVIOUS following OPEN. ⚠ **SCOPE (kb/Work PB343):** Annex E is INFORMATIVE and item 22 names no organization, but the amendment it describes was made in §14.9.30.4 GR21's **indexed** sub-rule block alone — d.3, "If no such record is found or PREVIOUS is specified and the previous operation on the file was an OPEN statement, the at end condition exists". The **relative** and **sequential** blocks print rule b) unamended — "the first existing record that is selected is made available, regardless of whether NEXT or PREVIOUS is specified" — so on those two organizations the after-OPEN behaviour is IDENTICAL at 2002, 2014 and 2023 and their connectors take no edition parameter (asserted by conformance:{2002,2014,2023}/pb334_read_previous_sequential and .../pb343_read_previous_relative). The INDEXED ≤2014 leg IS gated — `DialectBehavior.IndexedReadPreviousAfterOpenAtEnd`, pinned by conformance:{2002,2014,2023}/pb344_read_previous_indexed. It is the one E.2 I-O item that passes the admission test in the legend above: item 22 states what the prior RULE said ("the rule itself stated that the first record would be retrieved"), where rows 25/26 record only that the prior text was missing (kb/Work PB344). | Yes | gate-behavior-by-dialect <!-- behavior:indexed-read-previous-after-open-at-end --> |
