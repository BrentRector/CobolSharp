...
- **▶ LIVE STATE (2026-09-13 — TRAIN 30 IS ON MAIN):** **the P14 traceability inventory is 4,348 rows at GAP 2533.** **Landing train 30 carried PB829 + PB367b + PB418 + PB356 + PB256 onto main in ONE landing** — five cluster commits plus one closing DEVLOG/§0 commit (DEVLOG 1596) — **GAP 2546 → 2533 with the denominator unchanged at 4,348**, 26 verdict records over six batch files, of which 15 CONFORMS (all 15 GAP-closing), 10 PARTIAL and 1 DIVERGES. All five clusters landed; none was dropped. ⛔ **`~Nist` is now in the landing union filter by name** — train 29's finding — and the 23-term union gate ran the 356 `NistDifferentialTests` cases GREEN, alongside the UNFILTERED `Cobol.Net.Tests.Unit` assembly and the legacy `CobolSharp.Tests.Integration` assembly. **TWO diagnostic codes are claimed — COBOLNET1970** (a word run inside a closed general format's CLAUSE list: the FD §13.4.5.2, the sort-merge FD §13.4.6.2, the file control entry §12.4.5.1, I-O-CONTROL §12.4.6.2 and SPECIAL-NAMES §12.3.7.2) **and COBOLNET1971** (a paragraph header the closed PARAGRAPH lists of §12.3.2 and §11.2.1 do not admit); COBOLNET1972 was reserved and returned unused, so the **next free code is COBOLNET1978** (src-grep max = catalog max = COBOLNET1977, unchanged — 1970/1971 fill holes below it). Notes flipped: **PB367b, PB356 and PB256 to `landed`**; **PB829 and PB418 stay `half`** (a canonical status — PB829 on the four general-format rows that stay PARTIAL, PB418 on the GR7 determination and PB415's category-name set); **PB322 gained two rows** (`GR-14.9.49.4-7`, `-12`) moved off PB367b so no landed note holds a defective row. ⚠ **A NEW OWNER QUESTION, #18:** does §14.9.20.4 GR7's unconditional dynamic-length zeroing override an explicit VALUE (GR6a3) or a REPLACING sender, or only the category default? Behaviour is untouched pending the answer. ⚠ **Eleven leads raised by the five implementer reports are recorded in DEVLOG 1596 and are NOT yet in `kb/Work/`** — no registrar has run since train 29; the most valuable are a named SUM COUNTER rejected in the procedure division (COBOLNET1639 against §13.18.54.4 GR5/GR12 — rejects legal source), the ambient checking flags leaking across a CALL (`ProgramTable.CallProgram` saves the activator mask and the PERFORM depth but not `_checking` — a silent wrong answer), INITIALIZE of a whole level-01 OCCURS table aborting the run unit (§14.9.20.4 GR5b2), and NUMVAL-C/TEST-NUMVAL-C with LOCALE accepting any grouping positions where §15.68.3 r5 b) 6 names `mon_grouping`. ⚠ **Register consequences for the orchestrator:** PB499's mechanism is RESOLVED by PB418 (re-measure and re-scope, never dispatch as written), PB420 needs re-measuring, and PB499/PB415/PB420 all still name the now-deleted `InitializeSender`.
