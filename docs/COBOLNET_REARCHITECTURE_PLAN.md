...
- **⛔ BATTERY REFERENCE — CURRENT: battery #62, 2026-09-06, on `7cc21232` — trains 19 (PB742+PB743, PB301, PB751+PB752, PB750, PB770) and 20 (PB725, PB712, PB753, PB353, PB348) — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#61, superseded by #62 above): battery #61, 2026-09-06, on `61d13d3e` — train 18 (PB728+PB575, PB704, PB735, PB740 with its three §9.1.15 rows recorded PARTIAL, PB761) plus the #60 record and the notes 8–9 commit after it — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ Every compiler leg GREEN — Conformance **6232 / 6232**, Unit **22886 / 22886** (the GPL corpus is present in the battery worktree, so both external-population drift tests ran and passed), characterization **33 / 33**, guard witnesses green, NIST **353 MATCH / 0 REGRESSION** and audit CLEAN (⚠ the guard's NIST leg still drives the LEGACY compiler until PB750 lands in train 19 — its line is not evidence about COBOL.NET), citations 0, differential **1323 cases** with **2 PER-CASE FLIP(S)**, both attributed by inspection: `run_file:4729` and `run_file:5016` (GnuCOBOL's LINAGE / LINAGE-COUNTER sample and its EXTFH twin) WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT — at #60 both died in the BACKEND with a generated-C# build failure on `01 data-record.` carrying `88 endofdata VALUE HIGH-VALUES` above its `02` subordinate, a figurative condition-name VALUE on a GROUP subject, which PB728+PB575 (train 18 row 1) now classes once for a group too; an improvement, both rows re-baselined BY HAND in `tests/external/gnucobol-verdict-baseline.tsv`. Battery #62 at train 19's head must be ALL GREEN with 0 flips.
