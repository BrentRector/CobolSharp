...
- **▶ LIVE STATE (2026-09-13 — TRAIN 31 IS ON MAIN):** **the P14 traceability inventory is 4,348 rows at GAP 2517.** **Landing train 31 carried PB337 + PB398 + PB640 + PB530 + PB344 onto main in ONE landing** — five cluster commits plus one closing DEVLOG/§0 commit (DEVLOG 1598) — **GAP 2533 → 2517 with the denominator unchanged at 4,348**, 20 verdict records over six batch files, of which 17 CONFORMS (16 of them GAP-closing; the seventeenth, `SR-13.18.40.3-24`, already read CONFORMS and was re-verdicted only so its evidence names the check that closed its SECOND sentence) and 3 PARTIAL. All five clusters landed; none was dropped. The 23-term union gate — `~Drift|~EditionGate|~Read|~Return|~Corpus|~Move|~Sort|~Diagnostic|~Evaluate|~Condition|~Collat|~VersionMatrix|~Call|~ExceptionCondition|~Nist|~DataClause|~Picture|~FileIo|~Declarative|~Use|~Indexed|~Vcr|~Io` — ran GREEN over Conformance (the 356 `NistDifferentialTests` cases included), the UNFILTERED `Cobol.Net.Tests.Unit` assembly, Characterization and the legacy `CobolSharp.Tests.Integration` assembly; semgrep unchanged at 3 / 46 / 2 / 416. **SEVEN diagnostic codes are claimed — COBOLNET1984 / 1985** (§13.18.40.3 SR25 sentence 2's EDITING phrase order and SR24 sentence 2's at-most-two extended symbols, PB530), **1994 / 1995** (the INTO phrase's §14.9.30.3 SR1/SR2 = §14.9.34.3 SR2/SR3 admissibility and strong-receiver record COUNT, PB337) **and 1997–1999** (the `IN alphabet-name-1` resolver shared by §14.9.13.3 SR3 and §13.18.63.3 SR31, PB398); the codes were checked for collision ACROSS clusters, a hazard a single-cluster landing never has. The **next free code is COBOLNET2000** (src-grep max = catalog max = COBOLNET1999); the holes below it are 1967–1969, 1972–1975, 1978–1983, 1986–1993 and 1996. Notes flipped: **PB337, PB398, PB640, PB530 and PB344 to `landed`**; **`kb/Work/PB834` is NEW and `open`** (MAJOR, codegen/call, crashes + rejects-legal-source) and holds `GR-14.2.3-9` / `GR-14.2.3-10`, which PB640 transferred to it — a nested CALL passing a USAGE POINTER or OBJECT REFERENCE item emits uncompilable C# because the callee-side formal carrier for those categories is the STRING carrier. ⚠ **THREE DETERMINATIONS the owner may overturn in one line each**, all recorded in DEVLOG 1598 and in their notes: PB530 reads §13.18.40.3 SR25 sentence 2's "leftmost symbol in character-string-1" as the leftmost OF THE TWO EXTENDED SYMBOLS (phrase ORDER only — the reading Annex D.24's own `PIC IS L9999.99F` survives); PB398 writes NO introduction gate for EVALUATE's partial expression and the range's `IN alphabet-name-1`, because the 2002+ edge is inferred rather than stated and gating on an inference rejects legal source; and PB344 holds the §9.1.14 contract EDITION-INVARIANT, withdrawing the Annex E.2 item 19 a)/b) gate that had made five NIST CCVS programs red on CI, with the register's new ADMISSION TEST — an Annex E item earns a `behavior:` anchor only when it says what the PRIOR RULE STATED — written on the enum, in the VERSION_CHANGE_REFERENCE legend and in the files design. ⚠ **A NEW OWNER QUESTION, #18** (still open from train 30): does §14.9.20.4 GR7's unconditional dynamic-length zeroing override an explicit VALUE (GR6a3) or a REPLACING sender, or only the category default? Behaviour is untouched pending the answer. ⚠ **Thirteen leads from train 31's five implementer reports are recorded in DEVLOG 1598, and train 30's eleven in DEVLOG 1596, and NONE of them is in `kb/Work/` yet** — no registrar has run since train 29; the most valuable of the new ones are a signed image-carried numeric FORMAL losing its operational sign across a CALL (`CobolArgAdapt.Text` / `TextValue` hard-code `Signed = false` against §14.2.3 GR11 — a silent wrong answer), EVALUATE's `IN alphabet-name-1` being unreachable when identifier-4 is written (COBOLNET1639 — rejects legal source), a bare user class-name EVALUATE object binding as identifier-2 and erroring, an FD/SD multi-01 shared record area diagnosed as a REDEFINES, and `SortEmitter.EmitInputFile` hooking the USE after §14.9.30.4 GR12 c)'s implicit CLOSE. ⚠ **Register consequences carried forward from train 30:** PB499's mechanism is RESOLVED by PB418 (re-measure and re-scope, never dispatch as written), PB420 needs re-measuring, and PB499/PB415/PB420 all still name the now-deleted `InitializeSender`.
