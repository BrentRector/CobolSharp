...
- **⛔ BATTERY REFERENCE — CURRENT: battery #53, 2026-09-05, on `070b054f` — landing train 9 (PB231's pointer third, PB319, PB317, PB321; DEVLOG 1510) plus the note commit behind it — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#52, superseded by #53 above): battery #52, 2026-09-05, on `0f6165c6` — landing train 8 (PB647, PB300, PB305, PB303, PB316; DEVLOG 1504) plus the note commits behind it — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ **Green on the compiler on every leg; `=== BATTERY: NOT GREEN (rc=1) ===` as printed only because the GnuCOBOL differential moved ONE case in the spec-licensed direction** — `run_misc:11291` ("CALL with program prototypes": `PROGRAM-ID. a AS "blah?Sdk".` resolved through a REPOSITORY `PROGRAM d AS "blah?Sdk"` entry) WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT, which is exactly PB303's `AS literal` phrase (§11.10.2 Format 1, §8.3.2.2's externalized name) making legal source compile where COBOLNET0901 had refused the word `AS`; rebaselined in the commit that records this battery, attributed to PB303. Measured: FULL greenfield Conformance **5946 / 5946** (9 m 51 s; +42 for train 8's fixtures) · Unit **22383 / 22383** (2 m 13 s; PB647's `FloatLandingModeDriftTests` alone adds ~17,000 theory rows) · characterization **33 / 33** · code citations **0** · doc citations **0 misfiled** · guard witnesses **ALL GREEN** · NIST **353 MATCH / 0 REGRESSION**, audit **CLEAN**, guard verdict **ALL GREEN** · GnuCOBOL differential **1323 cases**, **`=== DIFFERENTIAL: 1 PER-CASE FLIP(S) ===`** = the PB303 case above. Totals 579/467/219/58 (WE_REJECT_THEY_ACCEPT / AGREE_ACCEPT / AGREE_REJECT / WE_ACCEPT_THEY_REJECT) — one row moved from the first column to the second.
