...
- **⛔ BATTERY REFERENCE — CURRENT: battery #83, 2026-09-22, on `c982251df` — trains 44 + 45 — run in an ISOLATED WORKTREE pinned at the batch head, never on main.**
  ✅ ALL GREEN ON EVERY COMPILER LEG — Conformance **7776 / 7776**, Unit **28643 / 28643** (the GPL corpus was copied into the battery worktree, so the external-population tests MEASURED their population rather than being red by absence), characterization **33 / 33**, guard evidence-rule witnesses AND the compiler-identity watchdog green, NIST **364 MATCH / 0 REGRESSION** with the audit CLEAN over a declared population of 376 (the shipped `cobol` compiler), the guard's legacy legs 1203 / 1203 and 503 / 504, all three static audits 0 (`audit_code_citations` 0 findings over 4,656 files · `audit_doc_citations` 0 MISFILED / 0 ELIDED over 529 checked / 479 correct · `audit_evidence_supersession` 0 UNMARKED), differential **1323 cases** with **2 ATTRIBUTED FLIPS — BOTH CONFORMANCE FIXES**. ⛔ **#82's record required #83 to be "ALL GREEN with 0 flips" and this run has TWO — recorded as a MISS, attributed and licensed, not rounded down to green.** (1) `run_fundamental:9066` "SPECIAL-NAMES CLASS" `AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT` on COBOLNET0844 — the DIVERGENCE-ADDING shape, so it was read from the case first: the program declares `CLASS EVEN IS 49 51 53 55 57` and writes `IF NUM-1 IS EVEN` over `01 NUM-1 PIC 9(01)`, and **§8.8.4.4.3 SR4** — "ALPHABETIC, ALPHABETIC-LOWER, ALPHABETIC-UPPER, or class-name-1 shall not be specified if the category of the data item referenced by identifier-1 is boolean, numeric, or numeric-edited", re-derived here and `--check`ed as rule **4** (NOT SR3, which is the usage rule) — forbids that source. Train 44's PB571 + PB590 landed the §8.8.4.4.3 operand rules as ONE table; the old screen returned early unless the operand's category was boolean, which is exactly why SR4 had never been asked here. (2) `syn_definition:556` "Redefinition of program-name by other programs" `WE_ACCEPT_THEY_REJECT → AGREE_REJECT` on COBOLNET2213 + COBOLNET2214 — PB660: two OUTERMOST `PROGRAM-ID. prog.` definitions in one compilation group (**§8.3.2.2**, `--check` rule 2) and duplicate contained program-names (**§8.4.6.3**), and GnuCOBOL's own case comments say "This should cause an error". **The standard licenses both new answers**, so the baseline was regenerated with `--write-baseline` on the same tree and its diff is EXACTLY those two rows, tier unchanged (`DEFAULT_DIALECT`), zero other churn; totals move 579/467/236/41 → **580/466/237/40**. **Complement measured, not assumed — and the FIRST selector was WRONG, which is recorded here rather than quietly fixed:** `corpus_sweep` asserts the population first (1323 cases == 1323 baseline rows); the first sweep then spelled the clause `CLASS <name> IS` and returned exactly ONE external program — the case that flipped — which that selector makes true BY CONSTRUCTION, because **the CLASS clause's `IS` is OPTIONAL**. Dropping it returns **THREE** of the 1,611 extracted external programs: `run_fundamental:9066`, `syn_misc:869` and `syn_misc:7276`. ⭐ **The corrected population is the STRONGER evidence, not the weaker.** `syn_misc:869` ("CLASS duplicate values") declares `CLASS CHECK-VALID 'a' THRU 'z' 'A' THRU 'Z' 'cdef'` with no `IS` and writes `IF X IS CHECK-VALID` over `01 X PIC X(8)` — a class-name-1 condition whose identifier-1 is of category **alphanumeric**, which SR4 does NOT reach — and it stayed **`AGREE_ACCEPT` in both batteries**; `syn_misc:7276` is a false positive (a VALIDATE *data-description* `CLASS IS ALPHABETIC-UPPER`, `WE_REJECT_THEY_ACCEPT` on the A.4.14 VALIDATE decline in both). So the external corpus holds ONE class-name-1 class condition the standard PERMITS and ONE it FORBIDS, and the new table moved the second while leaving the first alone — which is the probe on the axis the flipped case holds fixed, and shows the screen discriminating on the operand's CATEGORY exactly as SR4 is written rather than refusing class-name-1 wholesale. A one-case population could never have shown that. COBOLNET0844's other seven corpus cases are its older §8.8.1.1 "not a numeric operand" role and not one of them moved; and of the **60** external programs carrying two or more PROGRAM-ID paragraphs, COBOLNET2213/2214 fired on the **one** whose names actually collide (53 NIST programs are also multi-program, and NIST is 364 MATCH / 0 REGRESSION). ⚠ **The batch's other nine new codes fire on ZERO corpus cases** — COBOLNET2197, 2198, 2199, 2200, 2201, 2202, 2205, 2210, 2211 — stated rather than left as a clean-looking silence: the external corpus is not evidence about them either way, and their evidence is their own goldens. ⭐ **Both test populations reproduce train 45's own whole-assembly gate EXACTLY** (7,776 and 28,643 on a different worktree and a different build). ⭐ **And the Unit delta over #82 (+4,278) is ATTRIBUTED PER CLASS, which REFUTED the lander's explanation of it:** train 45's report put its 28,643 down to the GPL corpus being fetched into its fresh worktree, but #82 ran WITH the corpus and measured 24,365, and a per-class diff of the two `unit.trx` files shows **+4,226 in `FloatLandingModeDriftTests` alone** (16,897 → 21,123 — `LegalShapesAndModes` is 528 shape pairs × 8 `CobolRounding` members = 4,224 cases per `[MemberData]` theory and PB653 added a fifth) plus 52 cases over thirteen other classes, no class shrinking and none corpus-sized; `DEVLOG.md` 1631 and §0 say only that the fetch makes the two `ExternalCorpusPopulationDriftTests` real passes, which is true, so no repo document carried the refuted sentence. Conformance over #82: **+67** (7709). No corpus drift, no case without a compiler verdict, no harness failure. Batch: trains 44 and 45 (DEVLOG 1630, 1631), the wave-42 close (1628) and PB950's `push-main.sh` repair (1629) — eleven new diagnostic codes (COBOLNET2197–2202, 2205, 2210, 2211, 2213, 2214), 54 new goldens of which 34 negative, GAP **2310 → 2276**. 893 s wall. DEVLOG 1632. Battery #84 at the next train's head must be ALL GREEN with 0 flips.
- **BATTERY REFERENCE — PREVIOUS (#82, superseded by #83 above): battery #82, 2026-09-21, on `c774def44` — train 43 — run in an ISOLATED WORKTREE pinned at the batch head.**
  ✅ ALL GREEN ON EVERY COMPILER LEG — Conformance **7709 / 7709**, Unit **24365 / 24365**, characterization **33 / 33**, guard evidence-rule witnesses and the compiler-identity watchdog green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN, all three static audits 0, differential **1323 cases** with **1 ATTRIBUTED FLIP**: `run_file:3586` `AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT` on COBOLNET2186 — PB433's PERFORM-range-across-declaratives screen, the DIVERGENCE-ADDING shape read from the case and licensed by **§14.9.28.3 SR11** (both procedure-names must name procedures in the SAME declarative section; GnuCOBOL accepts only under its own `-fno-section-exit-check`). Re-baselined in that commit, diff exactly that one row. DEVLOG 1627.
