...
- **⛔ BATTERY REFERENCE — CURRENT: battery #65, 2026-09-07, on `835a3677` — train 23 (PB802: no sidecar of any type; fixed-file attributes validated from each file's own format; the framed store's required §9.1.6 header; the `COBOLNET_KEYCHECK` opt-out) — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#64, superseded by #65 above): battery #64, 2026-09-06, on `a7a99860` — train 22 (PB785 frozen-evidence audit, PB793 ASCII/EBCDIC code-names + CODE-SET conversion) over the question-23 CI gate — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ ALL GREEN — Conformance **6526 / 6526**, Unit **23042 / 23042** (the GPL corpus is present in the battery worktree), characterization **33 / 33**, guard witnesses green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN (the shipped `cobol` compiler), all three static audits 0, differential **1323 cases** with **0 PER-CASE FLIP(S)** — `configuration:885` and `run_misc:5406`, re-baselined by hand to AGREE_ACCEPT in train 22, hold. The GitHub runs on this head (34077093442 on the landing branch, 34077906771 on main) are GREEN — the first battery over a tree that reached main through the required `ci-gate` check. Battery #65 at the next train's head must be ALL GREEN with 0 flips.
