...
- **⛔ BATTERY REFERENCE — CURRENT: battery #81, 2026-09-20, on `c701786d` — train 42 — run in an ISOLATED WORKTREE pinned at the batch head, never on main.**
  ✅ ALL GREEN ON EVERY COMPILER LEG — Conformance **7632 / 7632**, Unit **24291 / 24291** (the GPL corpus is present in the battery worktree), characterization **33 / 33**, guard evidence-rule witnesses AND the compiler-identity watchdog green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN (the shipped `cobol` compiler), all three static audits 0, differential **1323 cases** with **3 ATTRIBUTED FLIP(S) — ALL THREE FIXES, none a regression**: `run_misc:470` and `run_misc:513` `WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT` (PB511's `IS EXTERNAL [ AS literal-1 ]`, §13.18.22.2 general format + §13.18.22.3 SR1 — legal source we used to refuse) and `syn_definition:1132` `WE_ACCEPT_THEY_REJECT → AGREE_REJECT` on COBOLNET2121 (PB466's ambiguous procedure-name reference, §8.4.2.2.1 — rule 1 false because another name has the identical spelling, rule 6 false because the referencing section contains no such paragraph). Every clause `--check`ed, never inherited; the baseline was regenerated with `--write-baseline` in the same commit and its diff is exactly those three rows. Population over #80: Conformance **+270**, Unit **+129**. DEVLOG 1624. Battery #82 at the next train's head must be ALL GREEN with 0 flips.
- **BATTERY REFERENCE — PREVIOUS (#80, superseded by #81 above): battery #80, 2026-09-13, on `bdbe660a` — train 38 — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ ALL GREEN — Conformance **7362 / 7362**, Unit **24162 / 24162** (the GPL corpus is present in the battery worktree), characterization **33 / 33**, guard witnesses green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN (the shipped `cobol` compiler), all three static audits 0, differential **1323 cases** with **2 ATTRIBUTED FLIP(S)** — see the DEVLOG entry for the attribution; the baseline row was re-verdicted by hand in the same commit. Battery #81 at the next train's head must be ALL GREEN with 0 flips.
