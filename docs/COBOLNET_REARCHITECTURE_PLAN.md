...
| ☐ | 07 | R | HIGH | 06 | Exhaustive visitor dispatch + binder/emitter god-class decomposition — **Steps 1–10 DONE (both god classes dissolved); RESUME AT Step 11 (structural `Place`) → Step 12 (FUNCTION-arg grammar)** | [PHASE-07](rearchitecture/PHASE-07-visitor-dispatch-emitter-decomposition.md) |
...
| **◐ D (NOW)** | P7 Steps 1–12 | **Binder/emitter god-class decomposition DONE (both dissolved). RESUME AT structural `Place` (Step 11, no C# text in the bound tree) → FUNCTION-arg grammar (Step 12) → the `ICodeGenBackend` seam.** | Needs B (`BoundCompilation`) + A (visitor). |
