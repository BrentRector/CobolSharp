...
- **⛔ BATTERY REFERENCE — CURRENT: battery #85, 2026-09-22, on `0abe2c80f` — trains 47 + 48 — run in an ISOLATED WORKTREE pinned at the batch head, never on main.**
  ✅ **ALL GREEN ON EVERY COMPILER LEG; the differential's 3 per-case flips are all conformance FIXES, each attributed and re-baselined, and the re-run over the corrected baseline prints `=== DIFFERENTIAL: 0 PER-CASE FLIP(S) ===`.** The script's own verdict was `=== BATTERY: NOT GREEN (rc=1) ===` on the flips alone — recorded as a MISS against #84's 0-flip bar, the same way #83 recorded its two. Conformance **7932 / 7932** (full, unfiltered), Unit **28746 / 28746** (GPL corpus copied in first: 1323 cases from 36 `.at` wrappers), characterization **33 / 33**, guard evidence-rule witnesses AND the compiler-identity watchdog green, NIST **364 MATCH / 0 REGRESSION** with the audit CLEAN over a declared population of 376 (the shipped `cobol`), legacy legs 1203 / 1203 and 503 / 504 (1 skipped), all four static audits clean (`audit_code_citations` 0 findings over 4,807 files · `audit_doc_citations` 0 MISFILED / 0 ELIDED over 563 checked / 512 correct · `audit_evidence_supersession` 0 UNMARKED, 11 non-gating drifts · `audit_witness_loss` 0 unexcused — and, because the battery's own leg compares against the merge-base and so measured nothing here, it was ALSO run `--base ec095d120` over the whole two-train span: 0 unexcused, 2 retired, 3 re-sited, GREEN), solution build 0 warnings / 0 errors. **The three flips, every one `WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT` (was `COBOL0001`), each compiled AND run here with GnuCOBOL's expected output:** `run_extensions:2101` and `run_misc:190` — a header-less `PROGRAM-ID.`, admitted by **§11.2.1**'s bracketed `[ IDENTIFICATION DIVISION. ]` (PB829 residue, train 48, gated below 2002 as `identification-header-optional-2002`); `run_extensions:2101` is also a containee named like its outermost program, which neither **§8.4.6.3** nor **§11.10.3 SR3** forbids — both constrain the programs CONTAINED within the outermost program, and the outermost is not among them; `run_misc:3430` — `SORT ROW1` with the statement KEY phrase omitted over tables whose OCCURS carry KEY phrases, admitted by **§14.9.40.3 SR15** and ordered by **§14.9.40.4 GR21** (PB846, train 47; output `0403020101020304`). Re-baselined, diff exactly those three rows. Totals **577 / 469 / 237 / 40** (#84: 580 / 466 / 237 / 40); no NEW or REMOVED case, no case without a compiler verdict. Both test populations reproduce train 48's own whole-assembly gate EXACTLY (7,932 and 28,746); over #84 that is Conformance **+109** and Unit **+58**. **Complement:** of the nine codes trains 47 + 48 added (COBOLNET2234, 2237–2243, 2247) only COBOLNET2241 (§12.3.7.3 SR8 implementor names, PB716/PB862) fires on the corpus — once, on `syn_misc:1583`, which stays `AGREE_REJECT`; COBOLNET0900 (the header's pre-2002 gate) cannot fire at the corpus's 2023. Fifteen cases kept their verdict and changed their reason: seven gained only the COBOL0307 hint beside their parse error, one (`syn_misc:1583`) gained COBOLNET2241, and seven parse errors became deeper rejects now that the grammar admits the header-less and KEY-less shapes, every one carrying its own citation (e.g. `run_extensions:2149` → COBOLNET1795, §11.10.3 SR2; `syn_misc:6499` → COBOLNET1757, SR15's refusing half). Wall **1,230 s** (Conformance 12 m 15 s). Batch: DEVLOG 1640–1644 — train 47 (GAP **2266 → 2208**), registrar #11, train 48 (GAP **2208 → 2198**); `git diff --stat ec095d120 0abe2c80f -- src/ tests/` is 337 files, +9,563 / −1,944. DEVLOG 1646. Battery #86 at the next train's head must be ALL GREEN with 0 flips.
- **BATTERY REFERENCE — PREVIOUS (#84, superseded by #85 above): battery #84, 2026-09-22, on `ec095d120` — train 46 — run in an ISOLATED WORKTREE pinned at the batch head.**
  ✅ ALL GREEN, 0 PER-CASE FLIPS (`=== BATTERY: ALL GREEN ===`, exit 0) — the first battery in five to meet the bar without an attributed miss. Conformance **7823 / 7823**, Unit **28688 / 28688**, characterization **33 / 33**, guard witnesses and watchdog green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN (population 376), all four static audits clean (first battery to carry `audit_witness_loss`), differential **1323 cases**, totals 580 / 466 / 237 / 40, no baseline row rewritten. Complement: train 46's COBOLNET2215 / 2216 fire on zero corpus cases; COBOLNET0819's 16 cases held. 1,777 s wall (Conformance 18 m 29 s, host contention unmeasured). DEVLOG 1640.
