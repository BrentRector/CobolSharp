...
- **▶ LIVE STATE (2026-09-12 — TRAIN 28 IS ON MAIN):** **the P14 traceability inventory is 4,348 rows at GAP 2561.** **Landing train 28 carried PB492 + PB452/PB817/PB497 + PB459 onto main in ONE landing** (three cluster commits, one commit re-applying the three verdict batches on the merged tree, and one closing DEVLOG/§0 commit — DEVLOG 1592) — **GAP 2589 → 2561 with the denominator unchanged at 4,348**, no cluster dropped, 36 records re-verdicted across three batches (30 CONFORMS · 6 PARTIAL · 18 re-adjudicated), which is exactly the 4 + 17 + 7 the three implementer reports predicted. The train is THREE rows rather than five on the owner's quota instruction. **FIVE diagnostic codes are claimed — COBOLNET1955** (PICTURE EDITING literal class, §13.18.40.3 SR9), **1958–1960** (prototype-pointer restriction / signature / ADDRESS OF FUNCTION operand) and **1961** (§8.4.2.3.3 SR4, an index-name subscripting a table it is not INDEXED BY) — each inside the range the manifest assigned its cluster with no cross-cluster collision; COBOLNET1956, 1957, 1962 and 1963 were reserved and never used and return to the pool, so the **next free code is COBOLNET1962**, src-grep max = catalog max = COBOLNET1961. Two 0899 staged-loud descriptors (`usage-function-pointer`, `program-pointer-restricted`) and the two GREEN negative fixtures that PINNED them were DELETED rather than left inert. `docs/CONFORMANCE.md` §7 gains three rows: **DOC-A.1-174** and **DOC-A.1-210** discharged with the rules they ride, and **DOC-A.1-128** (the implementor index range) written as a DETERMINATION only — its VERDICT is deliberately left to kb/Work PB460 and is not in any batch. Notes flipped to `landed`: **PB492, PB452, PB817, PB497, PB459**; **PB533 stays OPEN** with its finding F4 recorded as discharged (SR8's `char.IsLetter` half is what keeps it open); **PB609 NARROWS** to its CALL-side row `SR-14.9.4.3-14`; **PB646** claims the six rows PB492 leaves PARTIAL on the one shared Phase-4a national-form staging leg, so no landed note orphans a defective row. `scripts/semgrep/baseline.json` moved 418 → 417 on `cobolnet-raw-diagnostic-code-literal` (a DECREASE, locked in). ⚠ **Nine leads raised by the three implementer reports are recorded in DEVLOG 1592 and are NOT yet in `kb/Work/`** — no registrar ran before the quota reset; the most valuable is that §8.4.3.2 admits `function-pointer-name-1` as a function-identifier operand and there is no surface for it, so a program can now BUILD a function-pointer and still cannot call through it.
