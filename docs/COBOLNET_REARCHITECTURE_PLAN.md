...
- **⛔ BATTERY REFERENCE — CURRENT: battery #63, 2026-09-06, on `5bf5a7f1` — train 21 (PB792, PB732 re-landed, PB794) over the CI fix (PB795+PB796) — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#62, superseded by #63 above): battery #62, 2026-09-06, on `7cc21232` — trains 19 (PB742+PB743, PB301, PB751+PB752, PB750, PB770) and 20 (PB725, PB712, PB753, PB353, PB348) — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ Every compiler leg GREEN — Conformance **6506 / 6506**, Unit **23032 / 23032** (the GPL corpus is present in the battery worktree), characterization **33 / 33**, guard witnesses green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN — ⭐ the first battery whose guard NIST leg drives the SHIPPED `cobol` compiler (PB750): 364 of 376 programs compared, the eleven former LEGACY_DIVERGENT among them, the rest NO BASELINE — citations 0. ⛔ NOT GREEN on the differential: **1323 cases** with **12 PER-CASE FLIP(S)**, eleven attributed by inspection and re-baselined BY HAND, ONE a REGRESSION left in the baseline: `run_extensions:5006` (`>>SOURCE FORMAT UNKNOWN` and `>>SOURCE FORMAT "literal"`) AGREE_REJECT → WE_ACCEPT_THEY_REJECT — PB725's one directive gate consumes the `>>SOURCE FORMAT` line in `ReferenceFormatProcessor` and no longer diagnoses an operand that is neither FIXED nor FREE, where the old path left the line to the parser (COBOL0001); filed as PB794 and fixed alone, battery #63 must show it AGREE_REJECT again. The eleven: `syn_definition:1849`, `syn_definition:1872` WE_ACCEPT_THEY_REJECT → AGREE_REJECT (PB770: §12.3.7.3 SR14 a) duplicate characters in an ALPHABET literal phrase — the over-acceptance battery #59 exposed, closed); `configuration:885`, `run_misc:5406` AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT (PB770: `ALPHABET x IS EBCDIC` — SR15's implementor code-name set is EMPTY here; GnuCOBOL defines EBCDIC; owner question 22 / PB793 decides whether to follow it — a latitude difference, not a defect); `syn_file:1325` WE_ACCEPT_THEY_REJECT → AGREE_REJECT (PB742: a RELATIVE KEY clause on an INDEXED file, §12.4.5.1 Format 2); `syn_file:1352` WE_ACCEPT_THEY_REJECT → AGREE_REJECT and `run_file:6878`, `run_file:7104`, `syn_file:1557`, `syn_file:1970`, `syn_file:2220` AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT (PB742+PB743: a category-NUMERIC record or alternate key, §12.4.5.12.3 SR2 / §12.4.5.6.3 SR2 — numeric keys are a GnuCOBOL EXTENSION, and five of its programs rely on it; a dialect leniency for that extension is a candidate note, never a relaxation of the strict verdict). Battery #63 at train 21's head must be ALL GREEN with 0 flips.
