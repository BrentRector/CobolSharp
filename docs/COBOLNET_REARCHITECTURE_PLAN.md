...
- **▶ LIVE STATE (2026-09-13 — TRAIN 29 IS ON MAIN):** **the P14 traceability inventory is 4,348 rows at GAP 2546.** **Landing train 29 carried PB500 + PB771 + PB445 + PB488 onto main in ONE landing** (four cluster commits, one commit re-applying the four verdict batches on the merged tree, and one closing DEVLOG/§0 commit — DEVLOG 1594) — **GAP 2561 → 2546 with the denominator unchanged at 4,348**, 16 GAP-closing rows over four disjoint batches. ⛔ **PB344 WAS DROPPED FROM THE TRAIN and is NOT on main**: the five-cluster train passed a 25-term local union gate GREEN and then `scripts/push-main.sh` held it off main because **CI run 34755793553 went RED** — five `NistDifferentialTests` goldens (**SQ122A, SQ136A, SQ137A, SQ138A, SQ148A**, all sequential-I/O CCVS) plus the guard's `nist=5 audit=1`, on Linux AND Windows. Bisected by cluster: 5 red at PB344's checkpoint, **0 of 349 at PB500's**. `kb/Work/PB344.md` stays `open` with those five programs as its new acceptance evidence, its three rows stay GAP, and its codes COBOLNET1967–1969 stay free. ⚠ **The lesson is the FILTER:** `NistDifferentialTests` matches none of the terms any of the five reports named, so the NIST differential ran in no cluster's gate and in no union of them — **`~Nist` is now part of the landing union filter** and belongs in every IO/declarative wave-local gate. **FIVE diagnostic codes are claimed — COBOLNET1964–1966** (the SEARCH ALL Format-2 operand rules, §14.9.37.3 SR7–SR13, split by the operand a violation is about) and **1976–1977** (the §13.16.3 SR24 condition-name association: the lettered exclusion, and the missing conditional variable); COBOLNET1967–1969, 1973–1975 and 1978–1981 were reserved and never used and return to the pool, so the **next free code is COBOLNET1978**, src-grep max = catalog max = COBOLNET1977. ⛔ **COBOLNET1528 is RETIRED and must never be reallocated** (the COBOLNET1518 precedent): PB500 deleted the refusal it carried, which was a FORMAT 2 general rule (§13.18.63.4 GR16) applied to a FORMAT 1 VALUE clause. Notes flipped to `landed`: **PB500, PB771, PB445, PB488**, plus **PB740**'s residue bullet closed by PB771; **`kb/Work/PB871` — an id the PB771 implementer had to allocate out of band because `DefectiveRowCoverageDriftTests` requires a live claimant for every defective row — is RENUMBERED to `PB833`** across its file name, frontmatter, body and both references, and it owns the still-PARTIAL `GR-9.1.15-2`. `scripts/semgrep/baseline.json` moved 417 → 416 on `cobolnet-raw-diagnostic-code-literal` (a DECREASE, locked in). ⚠ **Eleven leads raised by the five implementer reports are recorded in DEVLOG 1594 and are NOT yet in `kb/Work/`** — no registrar ran before train 30; the most valuable are a cross-run-unit sharing refusal answering '30' where §9.1.13.9 1) a) requires '61' on all three organizations, and a SORT/MERGE USING read failure whose USE declarative sees the implicit CLOSE's status and so never runs.
