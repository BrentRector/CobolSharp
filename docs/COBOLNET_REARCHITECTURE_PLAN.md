...
- **⛔ BATTERY REFERENCE — CURRENT: battery #45, 2026-09-05, on `b3aad3fc` — landing train 2 (PB473, PB204, PB205, PB206, PB207, PB208; DEVLOG 1468) — run in a DETACHED WORKTREE cut at the batch head, the first battery with PB473's guard evidence witnesses (phase 2a).**
  ✅ **Every compiler leg GREEN, the guard's 21 evidence witnesses GREEN, the NIST guard CLEAN; of the differential's five flips, two are rebaselined as spec-licensed rejections, one is an over-rejection registered as `kb/Work/PB598`, and two are battery #44's known pair owed to `PB485`.** Measured: FULL greenfield Conformance **5629 / 5629** (9 m 17 s) · Unit **5339 / 5339** (2 m 7 s) · characterization **33 / 33** · code citations **0** · doc citations **0 misfiled** · guard witnesses **ALL GREEN** · NIST **353 MATCH / 0 REGRESSION**, audit **CLEAN**, guard verdict **ALL GREEN** · GnuCOBOL differential **1323 cases**, **`=== DIFFERENTIAL: 5 PER-CASE FLIP(S) ===`** — `run_fundamental:6286` and `syn_value:325` (AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT) are PB206's §13.18.63.3 SR4 size and SR13 category rejections of source the standard forbids and GnuCOBOL warns-and-truncates, rebaselined in this recording commit (exactly those two rows); `syn_misc:388` (AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT) is an OVER-REJECTION — SR4/SR5/SR10's size sentences bind "an elementary item" with "an explicit PICTURE clause", which a level-88 subject is not and has not, so `88 C VALUE 'cd'` on `PIC X` is a legal, permanently-false condition (`kb/Work/PB598`; the row stays AGREE_ACCEPT until the fix lands); `syn_misc:5093` and `syn_move:453` were battery #44's PB485 pair — **PB485 landed (`f37da577`): both flips returned to their committed rows and five older leaks moved toward AGREE_REJECT, rebaselined.** Totals 580/466/212/65. `=== BATTERY: NOT GREEN (rc=1) ===` as printed; green on the compiler once attributed, one flip owed to PB598 and two to PB485. ⭐ **RETIRED 2026-09-05 — `PB598` LANDED with landing train 4 (DEVLOG 1478) and `syn_misc:388` is back on its committed AGREE_ACCEPT row**, measured on the merged tree at `=== DIFFERENTIAL: 0 PER-CASE FLIP(S) ===`; nothing in this battery is owed to PB598 any more.
- **BATTERY REFERENCE — PREVIOUS (#44, superseded by #45 above): battery #44, 2026-09-04, on `1d949007` — landing train 1 (PB203, PB201, PB470, PB193; DEVLOG 1460) plus the resume-day filings — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ **Every compiler leg GREEN, the NIST guard CLEAN, and the differential's two flips are ONE pre-existing hole, registered as `kb/Work/PB485`.** Measured: FULL greenfield Conformance **5589 / 5589** (10 m 47 s) · Unit **5336 / 5336** (2 m 7 s) · characterization **33 / 33** · code citations **0** · doc citations **0 misfiled** · NIST **353 MATCH / 0 REGRESSION**, audit **CLEAN**, guard verdict **ALL GREEN** (the compare arm did not flake this run; PB473's fix lands with train 2) · GnuCOBOL differential **1323 cases**, **`=== DIFFERENTIAL: 2 PER-CASE FLIP(S) ===`** — `syn_move:453` WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT and `syn_misc:5093` AGREE_REJECT → WE_ACCEPT_THEY_REJECT, both attributed exclusively to `5082cffc` (PB201), and NEITHER is a fix: both programs carry a `78 name VALUE literal` entry (a MicroFocus/GnuCOBOL extension; §13.18.33.3 SR5 admits only 66, 77, 88 and 1–49) that the typed-native front end never validates and binds as a phantom group — PB201 removed the uncompilable-C# crash that had been masking it, so the cases now compile and abort at run time. ⛔ The baseline rows are NOT rewritten: both are already the ISO-correct verdicts, and PB485's fix restores them for the right reason (three more corpus cases — `syn_misc:5499`, `syn_occurs:103`, `run_file:9847` — were leaking before train 1 and move toward REJECT with it; rebaseline those then, as spec-licensed movement). Totals 577/469/212/65. `=== BATTERY: NOT GREEN (rc=1) ===` as printed; green on the compiler once attributed, the two flips owed to PB485.
