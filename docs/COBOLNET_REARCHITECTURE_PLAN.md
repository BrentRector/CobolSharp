...
- **⛔ BATTERY REFERENCE — CURRENT: battery #51, 2026-09-05, on `6111270a` — landing train 7 (PB231's NATIONAL third, PB623 + PB90, PB297, PB639, PB635; DEVLOG 1498) plus the note commits behind it — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#50, superseded by #51 above): battery #50, 2026-09-05, on `6c3d6a91` — landing train 6 (PB253, PB254, PB288, PB292; DEVLOG 1491) plus the note commits behind it — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ **Green on the compiler as printed on every leg; `=== BATTERY: NOT GREEN (rc=1) ===` only because the code-citation audit found ONE phantom clause in a register NOTE** (`kb/Work/PB646.md:66` cited `§8.8.4.1.3` for the class condition, which is §8.8.4.4 — corrected in the commit that records this battery). Measured: FULL greenfield Conformance **5854 / 5854** (9 m 29 s; +24 for train 6's fixtures) · Unit **5434 / 5434** (2 m 11 s) · characterization **33 / 33** · code citations **1 phantom** (a note, fixed) · doc citations **0 misfiled** · guard witnesses **ALL GREEN** · NIST **353 MATCH / 0 REGRESSION**, audit **CLEAN**, guard verdict **ALL GREEN** · GnuCOBOL differential **1323 cases**, **`=== DIFFERENTIAL: 0 PER-CASE FLIP(S) ===`** — train 6's fourteen closed rows moved nothing in the external oracle. Totals 580/466/219/58 (WE_REJECT_THEY_ACCEPT / AGREE_ACCEPT / AGREE_REJECT / WE_ACCEPT_THEY_REJECT), identical since #47.
