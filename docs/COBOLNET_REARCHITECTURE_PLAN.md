...
- **⛔ BATTERY REFERENCE — CURRENT: battery #47, 2026-09-05, on `4af4132c` — landing train 4 (PB233, PB598, PB237, PB238; DEVLOG 1478) — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#46, superseded by #47 above): battery #46, 2026-09-05, on `dbdcb0d2` — landing train 3 (PB231's USAGE BIT third, PB485, PB230, PB236, PB235; DEVLOG 1474) — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ **Every compiler leg GREEN, the guard's evidence witnesses GREEN, the NIST guard CLEAN, and the differential's ONE flip is the level-88 over-rejection `kb/Work/PB598` fixes, landing with train 4.** Measured: FULL greenfield Conformance **5738 / 5738** (10 m 27 s) · Unit **5368 / 5368** (2 m 15 s) · characterization **33 / 33** · code citations **0** · doc citations **0 misfiled** · guard witnesses **ALL GREEN** · NIST **353 MATCH / 0 REGRESSION**, audit **CLEAN**, guard verdict **ALL GREEN** · GnuCOBOL differential **1323 cases**, **`=== DIFFERENTIAL: 1 PER-CASE FLIP(S) ===`** — `syn_misc:388` AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT (COBOLNET1740 on a condition-name; PB598's fix on its worktree measured the case back to AGREE_ACCEPT with the baseline row untouched). Battery #44's PB485 pair and the five rows PB485 rebaselined hold; PB236's two spec-licensed moves from train 3 (`syn_move:374` → AGREE_REJECT, `run_misc:3544` → WE_REJECT_THEY_ACCEPT, an EBCDIC vendor extension) hold. Totals 582/464/219/58. `=== BATTERY: NOT GREEN (rc=1) ===` as printed; green on the compiler once attributed, the one flip owed to PB598.
