...
- **⛔ BATTERY REFERENCE — CURRENT: battery #48, 2026-09-05, on `6eaa98cf` — landing train 5 (PB248, PB250, PB251, PB252; DEVLOG 1484) — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ **Green on the compiler once attributed: every compiler leg GREEN, the guard's evidence witnesses GREEN, the GnuCOBOL differential at ZERO flips; `=== BATTERY: NOT GREEN (rc=1) ===` as printed because the NIST guard scored ONE FALSE REGRESSION** — IF137A `DIFF — REGRESSION!` with a preserved print file byte-identical to the golden after the guard's own normalization (17/17 tests passed; serial re-run MATCH 3/3; 12-wide contention probe MATCH 12/12), the audit's one finding being the same line: a compare-arm verdict corrupted under full-fan-out contention, `kb/Work/PB642` (the third face of PB473/PB478). Measured: FULL greenfield Conformance **5806 / 5806** (10 m 58 s) · Unit **5413 / 5413** (2 m 22 s) · characterization **33 / 33** · code citations **0** · doc citations **0 misfiled** · guard witnesses **ALL GREEN** · NIST **352 MATCH / 1 REGRESSION** (IF137A, false — PB642), audit **1 finding** (the same) · GnuCOBOL differential **1323 cases**, **`=== DIFFERENTIAL: 0 PER-CASE FLIP(S) ===`** — train 5's ten closed rows moved nothing in the external oracle. Totals 580/466/219/58 (WE_REJECT_THEY_ACCEPT / AGREE_ACCEPT / AGREE_REJECT / WE_ACCEPT_THEY_REJECT), identical to #47.
- **BATTERY REFERENCE — PREVIOUS (#47, superseded by #48 above): battery #47, 2026-09-05, on `4af4132c` — landing train 4 (PB233, PB598, PB237, PB238; DEVLOG 1478) — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ **`=== BATTERY: ALL GREEN ===` as printed — every compiler leg GREEN, the guard's evidence witnesses GREEN, the NIST guard CLEAN, and the GnuCOBOL differential at ZERO flips**, confirming on a full run what the train-4 lander measured on the merged tree (its one spec-licensed rebaseline, `syn_misc:3951`, holds). Measured: FULL greenfield Conformance **5787 / 5787** (10 m 29 s) · Unit **5380 / 5380** (2 m 16 s) · characterization **33 / 33** · code citations **0** · doc citations **0 misfiled** · guard witnesses **ALL GREEN** · NIST **353 MATCH / 0 REGRESSION**, audit **CLEAN**, guard verdict **ALL GREEN** · GnuCOBOL differential **1323 cases**, **`=== DIFFERENTIAL: 0 PER-CASE FLIP(S) ===`** — battery #46's one flip (`syn_misc:388`, the level-88 over-rejection) is closed by PB598 in this train. Totals 580/466/219/58 (WE_REJECT_THEY_ACCEPT / AGREE_ACCEPT / AGREE_REJECT / WE_ACCEPT_THEY_REJECT).
