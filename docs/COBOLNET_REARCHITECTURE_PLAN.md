...
- **⛔ BATTERY REFERENCE — CURRENT: battery #59, 2026-09-06, on `1e7b5085` — trains 15 and 16 (twelve clusters; the denominator repair to 4,348 rows) and PB741's solo landing (the NC215A fix), plus the note commits after them — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ **Every compiler leg GREEN — FULL Conformance 6192/6192 (NC215A restored), Unit 22572/22572, Characterization 33/33, guard witnesses green, NIST 353 MATCH and audit CLEAN (⚠ the guard's NIST leg still drives the LEGACY compiler until PB750 lands — its line is not evidence about COBOL.NET; the greenfield NIST evidence is the Conformance partitions), citations 0 — and SEVEN differential flips, all attributed by inspection with four general-format diagrams rendered (the b59 attribution report): five WE_REJECT → AGREE_ACCEPT are conforming optional-word acceptances (PB695 families 1–2: `TO` in `ALL TO VALUE`, the VALUE clause's WHEN/SET/TO/IS, `CHARACTERS` in SYMBOLIC CHARACTERS; PB693's CLASS overlaps are legal); `syn_file:1265` WE_ACCEPT → AGREE_REJECT is PB699's key rules matching GnuCOBOL item for item (§12.4.5.13.3 SR1–SR3) — an improvement; and `syn_definition:1849` AGREE_REJECT → WE_ACCEPT_THEY_REJECT is an ILLEGAL program (§12.3.7.3 SR14 a), duplicate characters in an ALPHABET literal phrase) that a correct relaxation EXPOSED — the over-acceptance predates train 15 (the pinned 4af4132c compiler accepts the same duplicates once the falsely-demanded IS is supplied), the old agreement was coincidental, and it is filed as PB770 (MAJOR: the ALPHABET clause's alphanumeric literal-phrase arm never got the validator its national twin has; an unrecognized code-name becomes an alphabet made of its letters; a 65,536 ordinal is masked into a 256-wide table). All seven rows re-baselined BY HAND in `tests/external/gnucobol-verdict-baseline.tsv` (never `--write-baseline`). Battery #60 at train 17's head must be ALL GREEN with 0 flips.**
- **BATTERY REFERENCE — PREVIOUS (#58, superseded by #59 above): battery #58, 2026-09-06, on `f86a5764` — landing train 14 (PB678, PB350, PB692, PB346, PB676, PB688; DEVLOG 1544) plus the note and repair commits after it — run in a DETACHED WORKTREE cut at the batch head.**
  ⛔ **`=== BATTERY: NOT GREEN (rc=1) ===` — Unit 22528/22528, Characterization 33/33, guard ALL GREEN (witnesses, NIST 353 MATCH, audit CLEAN), citations 0, differential counts 578/468/221/56 — and TWO findings, both attributed by inspection (the b58 attribution report; PB741):** (1) **a REAL wrong answer that reached main** — FULL Conformance 6084/6085: `NistDifferentialTests_P0("NC215A")` prints `FAIL*` for `SEQ-TEST-GF-6` (and GF-7 behind it): `IF NINE-DU-9V0-1 < SPACE` under `PROGRAM COLLATING SEQUENCE IS THE-WILD-ONE` loses the sequence, because train 14's PB678 (`f798397f`) folded the comparison's collate argument into `CollatingSelection.Of`, whose numeric arm answers the SORT-key question (§14.9.40.4 GR5, no sequence for a numeric key) where the comparison question (§8.8.4.2.5 → §8.8.4.2.7, the program collating sequence) was being asked — one classifier, two questions; bisected by build (green at c34f01ad, red at f798397f). **FIXED on main — PB741 landed ALONE on purpose as a blocking cluster (DEVLOG 1554), ahead of every other train so they rebase onto a correct main: the comparison-class rule is split out of the SORT-key classifier into `CollatingSelection.ForComparison(left, right)` and the one-argument `EmitContext.CollateArgFor` overload is DELETED, so half a comparison is unexpressible. `NistDifferentialTests_P0("NC215A")` passes (4/4 under `DisplayName~NC215A`) and the CCVS report is byte-identical to `tests/nist/valid/NC215A.txt`. PB728 — the same mechanism at the level-88 and EVALUATE THRU arms, which read `Pic` where a group has none — landed with it; PB575's binder half stays open.** **The guard's NIST leg did not see it because `guard-fast.sh`/`guard.sh` drive the LEGACY compiler (`src/CobolSharp.CLI`) — `guard NIST: 353 MATCH` has never been evidence about COBOL.NET's codegen (PB750); the greenfield NIST evidence is the Conformance assembly's `NistDifferentialTests` partitions.** (2) differential flip `syn_file:1943` WE_ACCEPT_THEY_REJECT → AGREE_REJECT: PB692's COBOLNET1858 enforces §12.4.5.5.2 SR2 (DYNAMIC/RANDOM shall not be specified for a sequential file) and GnuCOBOL agrees — an improvement, re-baselined by hand. Battery #59 at train 16's head with PB741's fix must be ALL GREEN with 0 flips.
