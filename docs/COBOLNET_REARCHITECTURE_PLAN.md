...
- **⛔ BATTERY REFERENCE — CURRENT: battery #66, 2026-09-09, on `16073192` — train 24 — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#65, superseded by #66 above): battery #65, 2026-09-07, on `835a3677` — train 23 (PB802: no sidecar of any type; fixed-file attributes validated from each file's own format; the framed store's required §9.1.6 header; the `COBOLNET_KEYCHECK` opt-out) — run in a DETACHED WORKTREE cut at the batch head.**
  ✅ ALL GREEN — Conformance **6526 / 6526**, Unit **23057 / 23057** (the GPL corpus is present in the battery worktree), characterization **33 / 33**, guard witnesses green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN (the shipped `cobol` compiler), all three static audits 0, differential **1323 cases** with **0 PER-CASE FLIP(S)** — the relative/indexed store header and the deleted sidecar change nothing the GnuCOBOL corpus observes; `configuration:885` and `run_misc:5406` hold. The GitHub runs on this head (34151967197 on the landing branch, 34152899374 on main) are GREEN. Battery #66 at the next train's head must be ALL GREEN with 0 flips.
