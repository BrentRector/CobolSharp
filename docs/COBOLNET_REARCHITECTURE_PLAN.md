...
- **⛔ BATTERY REFERENCE — CURRENT: battery #58, 2026-09-06, on `f86a5764` — landing train 14 (PB678, PB350, PB692, PB346, PB676, PB688; DEVLOG 1544) plus the note and repair commits after it — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#57, superseded by #58 above): battery #57, 2026-09-06, on `c34f01ad` — landing train 13 (PB334, PB345, PB332, PB352+PB354, PB341+PB342, PB364; DEVLOG 1537) plus the seven note/ledger commits after it — run in a DETACHED WORKTREE cut at the batch head.**
  ⛔ **`=== BATTERY: NOT GREEN (rc=1) ===` — every compiler leg green (FULL Conformance 6063/6063, Unit 22508/22508, Characterization 33/33, guard witnesses green, NIST 353 MATCH and audit CLEAN, differential counts 578/468/220/57) and THREE findings, all attributed to train 13 by inspection and repaired in the commit that records this (the b57 attribution report; PB717):** (1) the guard's legacy Integration leg red — `FileIOTests.FileIO_Start_PositionsForReadNext` wrote `START … KEY IS IX-KEY`, which §14.9.41.2 prints with the relational operator UNBRACKETED (folio 754 rendered), so PB332's tightened `startKeyPhrase` rejects it CORRECTLY; the test line now reads `KEY IS EQUAL TO IX-KEY`; (2) differential flip `syn_file:459` WE_ACCEPT_THEY_REJECT → AGREE_REJECT: PB352+PB354's COBOLNET0862 enforces §14.9.41.3 SR2 (FIRST or LAST required on a sequential-organization file) and GnuCOBOL agrees — an improvement; (3) flip `syn_misc:1152` WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT: PB332 made USE's AFTER, STANDARD, PROCEDURE and ON optional as §14.9.49.2 prints them (folio 774), removing an over-rejection — an improvement. Both rows re-baselined BY HAND in `tests/external/gnucobol-verdict-baseline.tsv` (never `--write-baseline`). Lesson (PB717): `OptionalWordSubsetDriftTests` measures what a format must ACCEPT and is blind to what a tightening newly REJECTS, and the only suite compiling the removed spelling is run by the guard, not the wave-local gate — the lander brief now gates the full Unit assembly and the legacy Integration assembly on every train. The first launch of this battery died in phase 0 on a corrupt `obj/…/ref/Cobol.Net.Editions.dll` left by an interrupted build; wiping the worktree's `obj`/`bin` fixed it. Battery #58 at train 14's head must be ALL GREEN with 0 flips.
