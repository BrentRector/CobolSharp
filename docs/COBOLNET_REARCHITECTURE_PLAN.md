...
- **▶ LIVE STATE (2026-09-25 — DOC-ROWS-2 IS ON MAIN; CLOUD GOLDEN LANE #1, DOC-ROWS-1 AND REGISTRAR-1 ON TOP OF TRAINS 64, 63, 62, 61, 60, 59 AND 58–40):** **the P14 traceability inventory is 4,347 rows at GAP 1241 — EVERY row adjudicated (0 rows never adjudicated) — and §15 — INTRINSIC FUNCTIONS, 557 rows — IS AT ZERO GAP.** **doc-rows-2 landed (DEVLOG 1707):** the last 51 Annex A.1 determinations written as §7 rows and recorded with them (DIVERGES 24 · PARTIAL 14 · CONFORMS 11 · NOT-IMPLEMENTED 1 · DOCUMENTED-NON-SUPPORT 1), so §7 discharges **182 of 182** documentation obligations; 21 of 51 items overturned by their refuters and corrected; owner decisions R47 (file encoding Latin-1, refuse above U+00FF: '71' line sequential / '91' record sequential and report) and R48 (STOP RUN in a hosted program ends the run unit only); w60b merged (FMT-5.2 retired, §15.78.2 REVERSE diagram restored); GAP 1248 → 1241; kb/Work PB1534 landed and PB1522 holds only DOC-A.1-201; `adjudication/doc-rows-2/LANDING.md`. **Train 63 landed (DEVLOG 1704):** w59j PB1548/PB1506/PB1524/PB1428 — the standard class BASE (`OoStandardClasses`, runtime `BASE`/`BASE__FACTORY`); New exists only through INHERITS FROM BASE (COBOLNET2448 `new-without-base` on every receiver arm), EC-OO-RESOURCE per §16.2.1.2 GR2; every OO golden, negative, test and version-matrix sample that NEW'd a root class now inherits BASE; GAP 1254 → 1248. **Train 62 landed (DEVLOG 1702):** R43 w59a+w59h (derived selectors mcs-only, float-formats-not-provided, record-key-source-only, a1-condition-absent; the §8 `definitional` arm; the FLOAT-DECIMAL clause decline COBOLNET2424; the VLG-vs-fixed override refusal COBOLNET0829; the generator %TEMP% race PB1564), w59k PB1557/PB1558 (full-set ALPHABET ordinals, SPECIAL-NAMES dependency-order bind), w59l PB1565/PB1566 (variadic exact argument lists, EXP/EXP10 result-range size error), GAP 1334 → 1254; the CITATIONS audit is green. **Train 61 landed (DEVLOG 1701):** w59c PB1036 (the EC-SORT-MERGE statement conditions raise), w59g PB1556 (one elementary record-area decode), w59i PB1549 (fatal EC raise sites share `EmitSelection`), w59m PB1569 (TYPE subject level-1 bit alignment; GR-13.18.57.4-2 → CONFORMS), GAP 1345 → 1334. **Golden lane #2 (local) batch 3 landed (DEVLOG 1700); the lane is done at 165 rows:** 30 rows witnessed (misc-p33…p38; 31 goldens), GAP 1375 → 1345; GR-13.18.57.4-2 re-verdicted PARTIAL on PB1569; the overturned/held rows are listed in `adjudication/golden-lane-1/LANDING.md`. **Golden lane #2 (local) batch 2 landed (DEVLOG 1698):** 78 rows witnessed (misc-p23…p32; 70 goldens), GAP 1453 → 1375. **Train 60 landed (DEVLOG 1697):** w59b PB1033/PB1058 (one saturating integer narrowing, COBOLNET2427), w59d PB1041/PB1310 (exact-argument intrinsics), w59e PB1053 (record extent tables), w59f PB1066 partial (GR14 DEFINE scope; SOURCE FORMAT arm open), GAP 1458 → 1453. **Golden lane #2 (local) batch 1 landed (DEVLOG 1695):** 57 rows witnessed (dns-witness, misc-p16…p22; 66 goldens), GAP 1515 → 1458. **Cloud golden lane #1 landed (DEVLOG 1688):** 111 CONFORMS/DNS rows witnessed by 126 spec-derived goldens over four waves, GAP 1627 → 1515; ~229 witness-owed rows remain for a later lane (drafts in `adjudication/golden-lane-1/`). **PB1522 step 3 landed (DEVLOG 1686):** the cloud wrote 67 Annex A.1 determination rows into `docs/CONFORMANCE.md` §7 (60 new, 2 corrected — DOC-A.1-72's was wrong — 5 'Not provided.'), each writer-probed and refuter-checked, and recorded 68 of the 126 held A.1 rows (CONFORMS 51 — 24 closed, 27 test-needed; PARTIAL 12; DOCUMENTED-NON-SUPPORT 5), GAP 1652 → 1627; A.1 audit 125 of 184 obligations met. **58 A.1 rows stay verdict-less:** 45 OMIT — the code is wrong against the spec, so the behavior is not documented, each claimed by a defect note (new PB1537–PB1545) — and 13 awaiting an owner answer (PB1536, PB1099, PB690, PB643). **Every inventory row is now ADJUDICATED at least once** (kb/Work PB1522, DEVLOG 1684): the last 208 verdict-less rows were adjudicated in claude.ai/code cloud sessions on the $250 cloud credit (two waves, each with independent refuters); cloud registrar-1 recorded 22 of them (11 non-A.1 + 11 A.1 items of declined modules closed DOCUMENTED-NON-SUPPORT under owner decision R42, made structural as Annex A.1 arms of the screen and commit/rollback selectors), GAP 1664 → 1652. **186 rows stay verdict-less by design:** 126 A.1 rows adjudicated but HELD because §7 carries no determination row for them (owner question PB1535 — the fix is writing the rows, PB1522 step 3) and 60 NEEDS-OWNER-DECISION rows each claimed by an owner note (PB1536, PB1198, PB1099, PB468, PB579, …). New defects PB1523–PB1533 (crashes PB1523/PB1524; wrong answers PB1526/PB1527/PB1529/PB1532). **Registrar 15d recorded adjudication batch 15d** (DEVLOG 1676): 26 subjects (the OO identification paragraphs, §9.3 objects / polymorphism / conformance, §6 reference format, §9.1.13 I-O status, §9.1 files, end markers, the compilation group, the OPTIONS clauses), 185 records, GAP 1723 → 1664 (+59), 29 CONFORMS rows test-needed for the golden lane; 27 notes filed (PB1491–PB1520 less four drafts folded into 15a/15c's PB1359, PB1361/PB1362, PB1383, PB1449), 23 extended, owner questions PB1517 / PB1518 / PB1519 and PB579 (extended). **Registrar 15c recorded adjudication batch 15c** (DEVLOG 1675): 27 clause-8 subjects (names and scope, qualification, national and numeric literals, NULL, object property / object view / SELF-SUPER, the program-address-identifier, reference modification, report counters, reserved words, subscripts, types, variable-length items, the relational operators and the seven simple conditions), 168 records, GAP 1762 → 1723 (+39), 39 CONFORMS rows test-needed for the golden lane; 29 notes filed (PB1441, PB1445, PB1446, PB1448–PB1450, PB1452–PB1460, PB1464–PB1477), 18 extended, nine drafts folded into 15b's PB1393 / PB1427 / PB1425 / PB1412; two message-tag rows added to the open owner question PB1198. **Registrar 15b recorded adjudication batch 15b** (DEVLOG 1674): 28 clause-8 subjects (literals, character sets, COBOL words, class and category, conditional / boolean / concatenation expressions, the identifier and address-identifier formats, dynamic-capacity tables, figurative constants, levels, lexical elements, LINAGE-COUNTER), 190 records, GAP 1812 → 1762 (+50), 24 CONFORMS rows test-needed for the golden lane; 41 notes filed (PB1390–PB1394, PB1397–PB1432), 22 extended, four drafts folded into PB547 / PB1246 / PB1228 / PB1353; no owner question. **Registrar 15a recorded adjudication batch 15a** (DEVLOG 1673): 28 §7.2 text-manipulation / §7.3 compiler-directive subjects (COPY, REPLACE, text-words, compile-time expressions, the constant conditional expression and the 17 directives), 233 records, GAP 1845 → 1812 (+33), 47 CONFORMS rows test-needed for the golden lane; 35 notes filed (PB1350–PB1384 — wrong answers PB1350/PB1351/PB1352 (the shared text-word tokenizer, literal comparison, directive lines rewritten by REPLACING), PB1354 (COPY SUPPRESS REPLACING loses the REPLACING), PB1355 (a missing copybook becomes a comment), PB1361/PB1362 (reference-format recognition), PB1380 (RENAMES ref-mod dropped), PB1382 (READ never raises EC-I-O-WARNING)), six open notes extended (PB1067, PB806, PB807, PB1119, PB1229, PB372).**Registrar 14f filed the wave-58/59 leads** (DEVLOG 1672): 15 notes PB1310–PB1324 (PB1310 binary64 ANNUITY/ACOS cancellation, PB1317 EC-BOUND-OVERFLOW + RESUME abandons growth, PB1318 DISPLAY LINAGE-COUNTER crash, PB1323 duplicate USING operand reaches Roslyn, PB1316 constant-name report SOURCE rejected), eight open notes extended, six CONFORMS rows re-verdicted PARTIAL, GAP 1839 → 1845, and ten landed closes_rows claims on those rows withdrawn. **Registrar 14e recorded adjudication batch d1-14e** (DEVLOG 1671): 22 data-division subjects (OCCURS through WORKING-STORAGE, incl. the report writer's PAGE, PRESENT WHEN, RD, report group, SOURCE, SUM, TYPE and VARYING clauses), 220 records, GAP 1875 → 1839 (+36), 32 CONFORMS rows test-needed for the golden lane; 50 notes drafted (PB1260–PB1309), two folded at landing into 14b's PB1129 (RD CODE) and PB1133 (RecursiveContainedWs) (48 open), and five open notes extended. **Registrar 14d recorded adjudication batch d1-14d** (DEVLOG 1670): 23 data-division subjects (ANY LENGTH through LOCAL-STORAGE, incl. the report writer's CODE, COLUMN, CONTROL, GROUP INDICATE and LINE clauses), 147 records, GAP 1900 → 1875 (+25), 26 CONFORMS rows test-needed for the golden lane; 45 notes drafted (PB1210–PB1255, incl. the owner note PB1255), eight folded at landing into 14b/14c's PB1129, PB1153, PB1132, PB1115, PB1187, PB1167, PB1188 and PB1171 (37 open), and five open notes extended. **Registrar 14c recorded adjudication batch d1-14c** (DEVLOG 1669): 22 statement subjects (RAISE through WRITE, ROUNDED, RETRY, RETURNING items, scope of statements), 218 records, GAP 1955 → 1900 (+55), 63 CONFORMS rows test-needed for the golden lane; 35 notes filed (PB1160–PB1199, incl. the owner note PB1198; five drafts folded into 14a/14b’s PB1072, PB1148, PB1140 and PB1139) and 15 open notes extended. **Registrar 14b recorded adjudication batch d1-14b** (DEVLOG 1668): 23 procedure-division subjects (§14.2, §14.6–§14.8, ACCEPT, FREE, GENERATE, INITIATE, INSPECT, INVOKE, MERGE, MULTIPLY), 218 records, GAP 1989 → 1955 (+34), 66 CONFORMS rows test-needed for the golden lane; 45 notes filed (PB1110–PB1154, incl. the owner note PB1151) and 12 open notes extended. **Registrar 14a recorded adjudication batch d1-14a** (DEVLOG 1667): 23 environment-division / file-control subjects, 163 records, GAP 2026 → 1989 (+37), 40 CONFORMS rows test-needed for the golden lane; 29 notes filed (PB1072–PB1100, incl. the owner note PB1099 with three questions). **Landing train 59 carried FIVE clusters and TEN notes to `landed` in one landing** (DEVLOG 1666): LOG/trig beyond binary64 without BigInteger and PRESENT-VALUE’s exact discount base (PB999 + PB1000), the SET Format-7 pointer screen, EC-BOUND-SET and one mnemonic map (PB548 + PB460 + PB454), one lexer token per figurative spelling and PICTURE SR7 (PB510 + PB569; COBOLNET2418–2419), SEARCH’s grammar spelling its two formats and SR4 (PB444 + PB446; COBOLNET2409), and §8.3.3.6.3 SR3 for multi-character ALL literals (PB422). **GAP 2037 → 2026** (−11), gated on the WHOLE Conformance assembly (8,484 cases), unfiltered Unit (29,190), Characterization and the legacy integration assembly (503 + 1 skipped); no cluster dropped. Before it: **Landing train 58 carried FIVE clusters and FOURTEEN notes to `landed` in one landing** (DEVLOG 1665), all five editing `DataBinder*.cs`: one data-clause placement table for SR11/SR5/SR6/SR7 and the BWZ/JUSTIFIED subject rules (PB507 + PB512 + PB518 + PB519; COBOLNET2403–2405), the VALUE clause placement and all-formats literal screens (PB515 + PB550 + PB551 + PB586; COBOLNET2406), the SIGN subject screen, CONTROL FINAL and unrepeated format elements (PB483 + PB537; COBOLNET2421–2423), the data-name-n ref-mod screen over SORT/MERGE/READ and the LINAGE operand rules (PB481 + PB524), and the run-time boolean width plus one condition-name qualifier (PB589 + PB567). **GAP 2060 → 2037** (−23), gated on the WHOLE Conformance assembly (8,441 cases), unfiltered Unit (29,139), Characterization and the legacy integration assembly (503 + 1 skipped); no cluster dropped. Before it: **Landing train 57 carried THREE clusters and THREE notes to `landed` in one landing** (DEVLOG 1661): the RD GLOBAL clause with USE GLOBAL Format 2 selected per statement (PB369; COBOLNET2392), index-name uniqueness through one resolution over per-table index declarations (PB919), and EXTERNAL FDs with out-of-line records carried in the run-unit cell’s dynamic slots (PB1026). **GAP 2069 → 2060** (−9), gated on the WHOLE Conformance assembly (8,350 cases), unfiltered Unit (29,097), Characterization and the legacy integration assembly (503 + 1 skipped). JD (PB999, LOG/trig beyond binary64) was DROPPED because it raised semgrep `cobolnet-no-biginteger` 46 → 66; PB999 stays open. Before it: **Landing train 56 carried SIX clusters and NINETEEN notes to `landed` in one landing** (DEVLOG 1660): the OPEN/START screens and the unnarrowed START LENGTH (PB357 + PB333 + PB318 + PB330 + PB417; COBOLNET2371–2372), the declaratives structural rules through the one procedure-name funnel (PB361 + PB362 + PB363; COBOLNET2375–2378; PB369 stays open), RANDOM on the run unit, REM's own zero-divisor rule and usage-national CONCAT (PB307 + PB304 + PB306), the FileRegistry Require sweep and the sort-merge flow ECs (PB360 + PB349), the §5.5 nonzero integer-n screen, ENTRY-CONVENTION and the ALPHABET SR11 residue (PB859 + PB910 + PB232 + PB226; COBOLNET2385–2386; PB919 stays open), and character-channel storage images with the Format-1 --permissive vendor lane (PB992 + PB162; GR-14.2.3-8 closed; PB165 stays open). **GAP 2092 → 2069** (−23), gated on the WHOLE Conformance assembly (8,329 cases), unfiltered Unit (29,075), Characterization and the legacy integration assembly (503 + 1 skipped). Before it: **Landing train 55 carried FOUR clusters and FIVE notes to `landed` in one landing** (DEVLOG 1659): ADDRESS OF a method formal and method DECLARATIVES with one USE-machinery emitter (PB1019 method arm + PB1010), record keys after a dynamic-length member through one key-window reader (PB1025; PB1026 stays open), the SORT/MERGE implicit-transfer dispositions from one rule table (PB993), and the reference resolver's closed `RefResolution` answer (PB1030; COBOLNET2363–2364). **GAP 2095 → 2092** (−3), gated on the WHOLE Conformance assembly (8,286 cases), unfiltered Unit (29,008), Characterization and the legacy integration assembly (503 + 1 skipped). Before it: **Landing train 54 carried SEVEN clusters and EIGHT notes to `landed` in one landing** (DEVLOG 1658): contained-program GLOBAL LINKAGE carriers and bridges (PB1009; PB1019 program arm), the qualified OCCURS KEY (PB1018; COBOLNET2353), propagated-condition pickup before NOT ON EXCEPTION and per-evaluation property GETs (PB606 + PB987), refusal nodes that carry their diagnostic (PB1029; COBOLNET2362), the one §8.9 reservation gate for both parse paths and grammar-decided format words (PB655 + PB764), the POINTER storage image BY CONTENT (PB970 arm 2), and pointer/object-leafed group transfer (PB244 shape (a); PB244 stays open). **GAP 2101 → 2095** (−6), gated on the WHOLE Conformance assembly (8,271 cases), unfiltered Unit (28,980), Characterization and the legacy integration assembly (503 + 1 skipped). Before it: **Landing train 53 carried FIVE clusters and ELEVEN notes to `landed` in one landing, retired PB1006 and DROPPED PB999** (DEVLOG 1657): GR14 implicit PUSH/POP ALL and PUSH/POP ALL placement (PB1004 + PB1005; COBOLNET2344), ACCEPT sized by the run-time receiver and function temporaries through the MOVE chain (PB1013 + PB1007), the report LINE clause NEXT PAGE and SR3/SR5/SR7/SR8 (PB1001 + PB1002), address-identifier operands, the WRITE ADVANCING count and REPOSITORY SR1 (PB1021 + PB1023 + PB1017; COBOLNET2365), and CONSTANT LENGTH OF subscripts and range IN alphabet (PB1016 + PB1015). PB999 conflicts with PB952's one domain screen and stays open. **GAP 2114 → 2101** (−13), gated on the WHOLE Conformance assembly (8,246 cases), unfiltered Unit (28,952), Characterization and the legacy integration assembly (503 + 1 skipped). Before it: **Landing train 52 carried EIGHT clusters and EIGHTEEN notes to `landed` in one landing, and PB970 to half** (DEVLOG 1656): the Conformance runner's compiled-program cache (PB985; `CompilationInputs`, warm re-gate 1m43s against 20m14s cold, OFF in CI and the battery), numeric storage images across the CALL and INVOKE boundary (PB970 arm 1; PB968 discharged), ONE operand-class screen for class-closed identifier positions (PB210 + PB211 + PB212; COBOLNET2324–2326), index data items only where §13.18.60.3 SR10 lists them (PB215; PB213/PB214 closed), CALL address-identifiers, static ADDRESS-OF cells and the CANCEL cascade (PB239 + PB234 + PB240), OO class-unit BASED data, repeated IMPLEMENTS and windowed crossings (PB956 + PB521 + PB946 + PB187; COBOLNET2333), ODO ref-mod direction and dynamic-table element images (PB202 + PB189; PB244 open) and NUMVAL-C locale grouping (PB835). **GAP 2130 → 2114** (−16), gated on the WHOLE Conformance assembly (8,206 cases), unfiltered Unit (28,927), Characterization and the legacy integration assembly (503 + 1 skipped); no cluster dropped. Train 52 claimed COBOLNET2324–2326 and 2333; next free diagnostic on main stays COBOLNET2320 (codes come from the central allocation). Before it: **Landing train 50 carried SEVEN clusters and FIFTEEN notes to `landed` in one landing** (DEVLOG 1651), the first train merged and gated while its predecessor was still in CI, landed after train 51 and rebased onto it: group senders (PB943 + PB944; `PlaceRenderer.SendingGroupValue`), report-writer relative operands and NEXT GROUP (PB951 + PB957; COBOLNET2284), float edges (PB961 + PB952; `IntrinsicSig.Domain`), receiving-operand roles (PB881 + PB856 + PB923; `ResolveSending`), SORT transfer I-O status and the UInt128 key column (PB837 + PB186), >>PUSH/>>POP directive state (PB941; COBOLNET2297) and general formats (PB706 + PB778 + PB924). **GAP 2161 → 2130** (−31, the same rows as on train 49's inventory), gated on the WHOLE Conformance assembly (8,135 cases), unfiltered Unit (28,877), Characterization and the legacy integration assembly (503 + 1 skipped). No cluster dropped. Train 50 claimed COBOLNET2284 and 2297; next free diagnostic on main stays COBOLNET2320. Before it: **Landing train 51 carried SIX clusters and FOURTEEN notes to `landed` in one landing, ahead of train 50** (DEVLOG 1650; train 50 stopped before push-main and now rebases onto it): ARG-OMITTED raised at the reference (PB971), UNSTRING receivers through the MOVE chain and the one all-or-nothing class rule (PB979 + PB980; determination D-UN3), grammar optional words, empty parentheses and REPOSITORY AS phrases (PB983 + PB969 + PB974), OO naming, positions and RAISING rule 9 (PB973 + PB975 + PB972), the SPECIAL-NAMES FOR phrase (PB976 + PB977) and bare-operand conditions, qualified data references and out-of-line FD records (PB982 + PB978 + PB981; determination D-FRA). **GAP 2175 → 2161**, gated on the WHOLE Conformance assembly (8,084 cases), unfiltered Unit (28,832), Characterization and the legacy integration assembly (503 + 1 skipped); no cluster dropped. Next free diagnostic on main: COBOLNET2320 (train 51 claimed 2306, 2309, 2315, 2318 and 2319; later waves take codes from the central allocation). Before it: **Landing train 49 carried EIGHT clusters and TWELVE notes to `landed` in one landing, and PB965 to half** (DEVLOG 1648): CALL boundary delivery (PB962; PB965 half, the `ICobolProgram.Call(CobolArg[], CobolArg?)` ABI), activations in expressions (PB892, activation-identity staging; `siteHandlesPropagation` deleted), the BoundUnsupported split (PB909 + PB681 + PB938; `BoundRejected`), the merged grammar formats (PB849 + PB894 + PB662; the program prototype), multi-target ASSIGN and DYNAMIC LENGTH STRUCTURE (PB829; determination D-DL3), the citation audit's prose half (PB388; RULE/SUBITEM now GATE in docs/ and kb/ too), line-sequential advancing (PB964; seven NIST goldens re-baselined) and MOVE CORRESPONDING through a level-66 alias (PB966). **GAP 2198 → 2175**, gated on the WHOLE Conformance assembly (8,021 cases), unfiltered Unit (28,794), Characterization and the legacy integration assembly (503 + 1 skipped). **Cluster V (PB655 + PB764) was DROPPED**: its token-level §8.9 gate has no arm in the legacy `Compilation.LexAndParse`, so eight integration programs that name `COL` at the legacy's '85 failed to parse. Next free diagnostic: COBOLNET2275 (train 49 claimed 2256–2258, 2269–2270 and 2272–2274; 2253–2255, 2259–2268 and 2271 were returned unused). Before it: **Landing train 48 carried SIX clusters and EIGHT notes to `landed` in one landing** (DEVLOG 1644): the §8.9 list-end predicate (PB805), grammar sinks (PB830; PB829 stays half), description copiers (PB888 + PB889 + PB836), the citation audit's code half (PB388 stays half; RULE/SUBITEM/DIAG-UNQUALIFIED now GATE under src/tests/scripts), ACCEPT through the one MOVE chain (PB887) and EVALUATE operands decided by symbol (PB843). **GAP 2208 → 2198**, gated on the WHOLE Conformance assembly (7,932 cases), unfiltered Unit (28,745 + one attributed PB376 temp-dir race), Characterization and the legacy integration assembly; no cluster dropped, and the train claimed no diagnostic code. Before it: **Landing train 47 carried wave 46's SIX clusters and EIGHTEEN notes to `landed` in one landing, and retired PB825** (DEVLOG 1642). The clusters were printed optional words (PB846 + PB847 + PB848), the RAISING tuple (PB815 + PB814), the method omitted-argument ABI (PB757), parameterized classes and REPOSITORY EXPANDS (PB759), SPECIAL-NAMES implementor names (PB716 + PB790 + PB862), CALL argument carriers (PB873 + PB615 + PB834), report-entry clause presence and OCCURS operands (PB853 + PB885) and the receiving store and float IS editing (PB871 + PB866). PB825 was retired under owner decision R13. **GAP 2266 → 2208**. The gate was the WHOLE Conformance assembly (7,903 cases), unfiltered Unit (28,730), Characterization and the legacy integration assembly, and no cluster was dropped. Next free diagnostic: COBOLNET2253 (the orchestrator's wave-47 allocation; train 47 claimed 2234, 2237–2243 and 2247). Before it: **Landing train 46 carried SEVEN clusters and TWENTY notes to `landed` in one landing** (DEVLOG 1639) — conditions (PB225 + PB760 + PB823 + PB705), the ambient exception state (PB891 + PB841 + PB893), file I-O status (PB810 + PB860 + PB854 + PB864), data-division views (PB572 + PB907 + PB960), verdict recording (PB959: `record_verdicts` now MERGES witnesses and `audit_witness_loss --check` guards retirements), MOVE validity (PB878 + PB879 + PB880; PB887 stays open) and the EVALUATE condition subject (PB842 + PB912; PB843 stays open) — **GAP 2276 → 2266**, gated on the WHOLE Conformance assembly (7,823 cases), unfiltered Unit (28,688), Characterization and the legacy integration assembly; no cluster dropped. Next free diagnostic: COBOLNET2217 (train 46 claimed 2215–2216). Before it: **Landing train 45 carried FIVE clusters and THIRTEEN notes to `landed` in one landing** — the printed formats (PB453 + PB563 + PB594), MOVE and operand rendering (PB584 + PB664 + PB394), float scale and the arithmetic gates (PB653 + PB654), the file connectors' OPEN phrases and lock visibility (PB668 + PB669) and program identity and linkage carriers (PB663 + PB660) — **GAP 2288 → 2276** on eleven `record_verdicts` batches re-applied in landing order on the merged tree and then compared ROW BY ROW against the text-merged inventory, zero rows differing. Gated on the WHOLE `Cobol.Net.Tests.Conformance` assembly UNFILTERED (the filter term is the assembly's own root namespace, so it selects all 7,776 cases — never a union of the implementers' terms) plus unfiltered Unit, Characterization and the legacy integration assembly, with no cluster dropped (DEVLOG 1631). The whole assembly answered `Failed: 0, Passed: 7776, Total: 7776`, unfiltered Unit `Failed: 0, Passed: 28643`, Characterization `Failed: 0, Passed: 33` and the legacy assembly `Failed: 0, Passed: 503, Skipped: 1` — the GnuCOBOL corpus fetched, so the population drift pair is a real pass. Five codes claimed — COBOLNET2205, 2210, 2211, 2213, 2214 — and eight distinct codes returned unused (2203, 2204, 2206–2209, 2212, 2215), so **the next free code is COBOLNET2215**. ⚠ The cross-cluster code-range check EARNED ITS KEEP: COBOLNET2205 had been allocated to TWO clusters and bit nothing only because the second used none of its four. ⚠ PB669's record-lock visibility is proven on Windows only by this train's own battery; CI's Linux leg on the landing sha is its real verdict (kb/Work PB796). **Landing train 44 carried THREE clusters and NINE notes to `landed` in one landing** — the report writer's second repetition axis (PB484 + PB429 + PB541 + PB565), the class and relation conditions (PB571 + PB590 + PB649) and the intrinsic argument intake (PB620 + PB636) — **GAP 2310 → 2288** on 30 records across seven `record_verdicts` batches re-applied in landing order on the merged tree (29 inventory rows changed, 22 of them closing the GAP and 7 already-CONFORMS §15 rows strengthened; none added, none removed) (cluster 3's two batches both record `AR-15.17.3-2`, so the shape guard refuses them in one run and they went in as separate runs, later over earlier), gated on the WHOLE `Cobol.Net.Tests.Conformance` assembly UNFILTERED plus unfiltered Unit, Characterization and the legacy integration assembly, with no cluster dropped (DEVLOG 1630). Six codes claimed — COBOLNET2197, 2198, 2199, 2200, 2201, 2202 — and two returned (2203, 2204), so **the next free code is COBOLNET2205**. ⚠ A BEHAVIOUR CHANGE this train ships: `EVALUATE X IS ALPHANUMERIC` no longer parses — ALPHANUMERIC is not one of §8.8.4.4.2's fourteen class-condition alternatives, and it was a non-ISO extension of a duplicate grammar rule this train deleted. **Landing train 43 carried SIX clusters and TWENTY-EIGHT notes to `landed` (thirty-two touched) in one landing** — SET-TO-FALSE and the SET formats (PB455 + PB561 + PB920 + PB890 + PB559 + PB503 + PB450), INITIALIZE value occasions (PB415 + PB418 + PB577 + PB576 + PB933 arm 1), exception checking and PERFORM ranges (PB433 + PB595 + PB441 + PB395), the data-description screens (PB587 + PB585 + PB570 + PB527 + PB501 + PB921), the MOVE operands (PB886 + PB896 + PB542 + PB922 + PB602) and citation fidelity (PB900 + PB838 + PB932 + PB388) — **GAP 2339 → 2310** on 29 records across twelve `record_verdicts` batches re-applied to the merged tree (rows changed 0: the merge had preserved them exactly), gated on the WHOLE `Cobol.Net.Tests.Conformance` assembly UNFILTERED plus unfiltered Unit, Characterization and the legacy integration assembly, with no cluster dropped (DEVLOG 1625). Six codes claimed — COBOLNET2176, 2177, 2186, 2187, 2191, 2196 — and the returned ones (2178–2185, 2188–2190, 2192–2195) are never reused, so a mechanical catalog-ceiling probe is always BELOW the allocator. The rest of that wave-43 allocation, COBOLNET2197–2202, was NOT returned: it went to the two groups that land in train 44 above, and all six are in the catalogue now. Clusters 1 and 6 MERGE their suspended wave-42 predecessor branches `worktree-wf_c18987cb-f4c-1` and `-6`, which therefore carry nothing this train does not and must never be landed separately. **Landing train 42 carried SIX clusters and twenty notes in one landing** — the SET finisher train 40 dropped (PB449 + PB456 + PB458 + PB462), the PB511 group (PB511 + PB544 + PB553 + PB549), BLANK WHEN ZERO (PB566 + PB509), PB245's `closes_rows` back-link (PB245 + PB875), the VALUE-clause group (PB560 + PB556 + PB557 + PB558 + PB499) and printed-format fidelity (PB412 + PB421 + PB402) — **GAP 2366 → 2339** on 29 records across eight `record_verdicts` batches re-applied to the merged tree, gated on the WHOLE `Cobol.Net.Tests.Conformance` assembly UNFILTERED plus unfiltered Unit, Characterization and the legacy integration assembly (DEVLOG 1622). **Wave 40 closed the nine remaining §15 rows as ONE group of seven notes**, every one of them a rule whose SECOND arm had never been screened: PB249 (TRIM's argument-2 REPEATS and the schema had no variadic tail, so from the second one onward there was no class, no width and no cross-argument screen; and r3's syntactic antecedent — "when no argument-2 is specified" — was decided at run time on an emptiness test, re-applying r3's SPACE default where its antecedent is FALSE) · PB247 (§15.87.3 r3's argument-2 walk sat BELOW `BindSubstitute`'s table(ALL) early return, so one enumeration switched the whole rule off; it is now one method on both exits, advancing a pair-SLOT counter by each operand's STATIC element count) · PB246 (SQRT implemented NEITHER of its two rules: §15.84.3 r2's domain rode on a NaN artifact that an SDIDI underflow to −0.0 defeats, and §15.84.4 r4's "absolute value" — the phrase IEC 60559's `sqrt(−0) = −0` exists to make load-bearing — had no `Math.Abs` anywhere on the path) · PB257 (VARIANCE's two carriers disagreed about an empty argument list, one answering a silent 0 and the other indexing `xs[0]`; ONE shared `RequireArguments` assertion now carries the §15.3 invariant across all ten variadic statistical functions in both carriers, and the §15.97/§15.85 clause numbers heading those bodies were another function's) · PB255 (TEST-FORMATTED-DATETIME's offset bounds were justified from §15.3.3.3, which governs the common time format; §15.3.3.6.2 governs them, and the short-argument-2 and lowercase-NOTE answers are now determinations D-TFD1/D-TFD2) · PB190 (REVERSE's three "throws at run time" shapes RE-MEASURED on a build of the tree — all three return §15.78.4 r1's value; PB25 and PB164 had closed them and neither refreshed the row) · PB245's ROW (`RV-15.4.1-4` closes on a §1.1 derivation after the enumeration it was waiting for was done mechanically: of the fourteen §15 clauses carrying an equivalent arithmetic expression, NOT ONE makes its result explicitly implementor-defined, so NOTE 1 item 4's antecedent is unpopulatable in the STANDARD — `DRV-RV-15.4.1-4`). ⚠ **PB245 itself stays OPEN and is an owner decision:** its subject is the missing `closes-rows:` back-link from a landed fix to the inventory rows it closes, and wave 40 is a fourth data point for that mechanism rather than a fix for it. DEVLOG 1620 carries the forensics. **Landing train 41 CARRIED SIX clusters and LANDED ALL SIX, twelve notes, in ONE landing** — PB461 (one figurative-constant classifier) · PB465 (SET pointer GR19 and GR20 as two checks) · PB554+PB564+PB513+PB536 (the data-description screens) · PB491+PB531+PB532+PB568 (PictureAnalyzer repetition factors and the EDITING phrase at GR14 widths) · PB882+PB883+PB840+PB852 (the report-writer sum counter) · PB428 (§8.4.3.1.2 Format 4 inline method invocation) — DEVLOG 1619 carries the forensics. **Landing train 40 CARRIED FIVE clusters and LANDED FOUR, eleven notes, in ONE landing** — PB413 + PB466 (`ProcedureNameMap` REMEMBERS a repeated procedure-name where five `Dictionary.TryAdd` sites discarded it — after registration the compiler could not even REPRESENT an ambiguity — and the program, method and USE-FOR-DEBUGGING resolution arms became one body; COBOLNET2121 for §8.4.2.2.1 rule 1, COBOLNET2122 for §8.4.2.2.3 SR7, both reported at the REFERENCE because both rules are conditioned on "explicitly referenced") · PB431 + PB432 + PB437 + PB439 (PERFORM’s §14.9.28.2 Format-2 head is ONE bracket over three alternatives and the varying-phrase operands carry seven lettered obligations — one cardinality screen ABOVE the format split, one operand classification, one GR12 window table, and GR12’s per-augment window stops being shipped as a REFUSAL of conforming source; COBOLNET2117–2120) · PB457 + PB467 (the reference resolver’s two scope questions: a CAPACITY register carries its §13.18.38.3 SR30 position and every ill-formed reference names its own rule — COBOLNET2125/2126/2127 — instead of the false COBOLNET1639 "is not defined"; and `PtrResolveBased`’s unit-wide `ByName` bypass is deleted for `ReferenceResolver.DeclarationOf`, so §14.9.39.3 SR18 / §14.9.3.3 SR1 stop being wrong in BOTH directions inside a method) · PB897 + PB898 + PB899 (the GnuCOBOL corpus fetch, which had been failing on every Git-for-Windows host and training every agent to ignore two permanent drift reds, now stages-then-swaps and goes RED with `POPULATION UNMEASURED` on failure — **this train is the witness: the corpus fetched into a fresh worktree and `ExternalCorpusPopulationDriftTests` ran GREEN**; `StandardStreams.EnsureUtf8()` is called by BOTH entry points; `LEGACY_DIVERGENT` is derived from `corpus.tsv` through `scripts/guard-population.sh` in both guards) — **GAP 2407 → 2396** with the denominator unchanged at 4,348, **eleven verdict records over four batch files re-applied on the MERGED tree in landing order**, every one closing its row. ⛔ **THE FIFTH CLUSTER, PB449 + PB456 + PB458 + PB462 (SET formats), WAS DROPPED BY THE TRAIN’S OWN GATE, AND `~OoSpine` IS THE TERM THAT SAW IT**: its own gate ran `~CorpusRunner|~Drift|~EditionGate|~Nist` and the SET statement’s OO arm lives in `OoSpineTests`, which none of those select. `SET N4 TO U` (a class-object sender into a `PIC 9(4)`) reported COBOLNET0844 from the ARITHMETIC screen where §14.9.39.3 SR8’s COBOLNET0867 is due, and `SET EXCEPTION-OBJECT TO E` reported COBOLNET1639 "‘EXCEPTION-OBJECT’ is not defined" — false about a name the standard declares — where §8.4.3.6 SR1’s COBOLNET0848 is due. Bisected on each checkpoint’s own build: base `00bd0a01` Passed 6/6, the cluster-1 checkpoint alone Failed 2 of 6. Dropped WHOLE and returned to its implementer, NOT repaired by the lander; **PB449, PB456, PB458 and PB462 stay `open`**, `GR-14.9.39.4-6` stays PB462’s, and **COBOLNET2112 and COBOLNET2113 are reserved for it — do not reallocate them.** Dropping it also removed the train’s one `SpecTraceabilityInventoryDriftTests` red, a pure COMPOSITION defect of train 39’s kind: cluster 4’s batch pinned `SR-14.9.39.3-29` at `SetBinder.cs#DynTryBindSetCapacity` and cluster 1 had folded that method away — neither cluster’s own gate could see it, only the merged tree held both facts. ⛔ **No generated or list file was three-way merged**: the four corpus manifests were merged as SETS by a script comparing each list’s element COUNT before and after (85 96 → 97, 2014 99 → 100, 2023 606 → 607, negative 1144 → 1157, with `pb17-function-subscript-varying-by` REMOVED — it pinned conforming source as a rejection). That mechanism is not decoration: the first attempt took both sides of the `85/manifest.json` conflict textually and produced a file with TWO `"enabled"` keys, which `json.load` silently resolves to the last — ninety-six goldens would have vanished with every marker check and both validators green, the train-33 shape exactly. `docs/DIAGNOSTICS.md` was REGENERATED by `scripts/gen-diagnostics-doc.ps1`, the two `Cobol.Net.Editions/*.g.cs` from `constructs.json`, and the inventory came only from the batches. The 16-term union gate — every term measured LIVE by `filter_population.py`, five named INERT against Conformance and run in the UNFILTERED Unit assembly rather than believed — ran Conformance **5446 / 5446**, the UNFILTERED Unit assembly **24212 / 24212**, Characterization **33 / 33** and the legacy `CobolSharp.Tests.Integration` assembly **503 / 504** (1 skipped). ⛔ Both `ExternalCorpusPopulationDriftTests` were GREEN in a fresh worktree — the first landing train since PB897 for which the GnuCOBOL corpus fetched at all. Semgrep no rule up; `work.py check` 939 items; `audit_code_citations.py`’s gating arm **⛔ 0 findings**, its non-gating rule-ordinal arm measures **208**, owned by PB388’s sweep. **NINE codes are claimed — COBOLNET2117, 2118, 2119, 2120, 2121, 2122, 2125, 2126, 2127**; COBOLNET2114–2116, 2123, 2124 and 2128–2131 were allocated centrally and came back UNUSED, and **the next free code is COBOLNET2151** — do not reuse the returned ones, and COBOLNET2112/2113 are RESERVED for the dropped SET cluster when it lands. ⛔ **THE FIRST CI RUN WAS RED (35473257133)**: the `rest` shard — Conformance minus `VersionMatrixTests` and `CorpusRunner`, Windows AND Linux — failed `GobackStatusArmParityTests.RaisingLastInAPerformWhenPhrase_IsAcceptedOnBothArms` with cluster 3's new COBOLNET2118. TRAIN 39 landed that test; its fixture writes `PERFORM UNTIL 1 = 1 … WHEN EC-SIZE …` and §14.9.28.2 Format 3 prints `PERFORM [ WITH LOCATION ]` and nothing else, so the compiler is right and the FIXTURE was repaired — the same correction cluster 3 had already made to twelve `ExitPlacementContextDriftTests` rows, in a class that did not exist on its base. `push-main.sh` held main untouched throughout. **A screen NEW to a train must be measured against every test that writes the construct it screens, and the union term for that is the VERB.** ⚠ For the next BATTERY: `guard.sh`/`guard-fast.sh` now derive `LEGACY_DIVERGENT` from the manifest for the first time, and `SQ212A` is expected LEGACY DIVERGENT, not a regression. Notes flipped to `landed`: **PB413, PB466, PB431, PB432, PB437, PB439, PB457, PB467, PB897, PB898, PB899**. **REGISTRAR #10 ran on 2026-09-22 over the seven wave-45 implementer results (groups A–G) and the train-46 lander report, and the register is 1000 notes**: seven leads filed as **PB961–PB967** — a binary32 signaling NaN quieted by the float→double image lane (PB961), a RETURNING delivery that aborts a conforming pair whose content is not a digit image (PB962), the unwitnessed >>TURN file-name-1 arm and GR5 phrase rule (PB963, analysis), a line-sequential AFTER write welded to the next plain WRITE (PB964), a fixed-length group passed BY REFERENCE to a variable-length formal arriving EMPTY (PB965), MOVE CORRESPONDING refusing a THROUGH alias (PB966) and the audits no CI leg runs (PB967, process). Every probe re-run on main `f503888da`; four leads dropped as already owned or behaviour-neutral. **REGISTRAR #13 ran on 2026-09-23 over the leads of waves 53–57 and trains 51–57, and the register is 1104 notes**: thirty-nine mechanisms filed as **PB1033–PB1071** (DEVLOG 1662 has the grouped list — the silent wrong answers among them: integer operands narrowed to int at emit (PB1033), the unraised sort-merge EC family (PB1036), SIN/COS/TAN of an exact argument (PB1041), a method's USE BEFORE REPORTING (PB1044), D-FRA's round trip (PB1053), INVOKE copy-out of a non-numeric image (PB1064), >>DEFINE surviving GR14's POP ALL (PB1066), a copybook's >>SOURCE FREE (PB1067); and a compiler crash on an oversized report-writer integer (PB1058)) and five notes extended (PB310, PB674, PB244, PB165, PB651). Every probe re-run on main `c54434a8d`; the w55c pending batch applied against PB1036; seven CONFORMS rows are contradicted by the new notes and claimed by them; **GAP unchanged at 2060**. **REGISTRAR #12 ran on 2026-09-22 over the 86 leads of waves 48–52 and the train-49 lander, and the register is 1065 notes**: forty-seven mechanisms filed as **PB986–PB1032** (DEVLOG 1649 has the one-line list) and seven notes extended — PB606 is now a measured MAJOR defect (CALL NOT ON EXCEPTION runs before the propagated condition), PB605's premise is corrected by §14.9.4.3 SR3 (the refusal is right, the diagnostic wrong), and **PB965 landed** with GR-14.2.3-8 re-homed to PB970 and PB992. Every probe re-run on main `37f19e028`; three leads did not survive re-measurement. Seven CONFORMS rows are contradicted by the new notes and claimed by them; **GAP unchanged at 2175**. **REGISTRAR #11 ran on 2026-09-22 over the 56 leads of waves 46–47, the wave-45 finishers and train 47, and the register is 1017 notes**: seventeen mechanisms filed as **PB968–PB984** — RESUME into a failed hoisted function activation's carrying statement (PB968), the keyword-omitted empty argument list (PB969), the CALL channel's missing storage image (PB970), *-ARG-OMITTED never raised under checking in any activation kind (PB971), §9.3.8.2.3 rule 9 and the duplicated override pass (PB972), a formal named N crashing the backend (PB973), REPOSITORY specifiers without AS literal (PB974), positionless OoClassTable diagnostics (PB975), the ignored CLASS FOR phrase (PB976), the postfix FOR phrase (PB977), first-match qualified lookups (PB978), UNSTRING's static examination size (PB979), UNSTRING SR3 (PB980), the FD shared record area over imageless records (PB981), a bare word as a condition crashing at run time (PB982), the optional IN of a THROUGH range (PB983) and the duplicated DOC-A.1-56 row (PB984); PB715 and PB454 extended. Every probe re-run on main `6871b6e10`; GAP unchanged at 2208. **GAP 2276 before and after** (DEVLOG 1636). **REGISTRAR #9 ran on 2026-09-22 over the four wave-43 implementer reports (PB484, PB571, PB620, PB450) and the train-44 lander report, and the register is 991 notes**: eight leads filed as **PB951–PB958** (the report-writer grammar's `+` operand, ACOS/ASIN's non-total domain artifact, `record_verdicts.py`'s replace-not-merge write, the recovery-`PicInfo` second diagnostic, the §8.3.3.3.2-for-an-ITEM citations, the OO cell/bridge BASED residue, the unowned NEXT GROUP clause and a π lost from §15.10.4's transcription), plus **five notes EXTENDED rather than duplicated** — the ⭐ seven floating-point class conditions were ALREADY OWNED by PB225, whose recorded posture was stale and is now re-measured (a compile-time refusal at 2014+, not a clean compile that dies at evaluation). ⚠ **Two of the four leads the lander handed forward did not survive re-measurement as stated**: the report `+` defect is real but its claimed SCREEN-SECTION twin is not — `CobolScreen.g4` already admits `PLUS`/`+`/`MINUS` — and the ACOS/ASIN finding's "§15.3 requires 0" is not in §15.3, which makes the result implementor-defined only when checking is NOT enabled. ⭐ **Two REGISTER defects, both of which put a phantom at the top of the harm-ranked queue**: PB857 and PB590 were the same defect filed twice and PB590's landing flipped only its own note, so PB857 ranked as an open MAJOR `rejects_legal_source` for nine days; and PB450 held `SR-14.9.39.3-18` for a mechanism it did not own. Both are `landed` now, the row belongs to PB956, and the durable answer to the first is PB245's `closes_rows` back-link. The pass moved NO verdict — **GAP is 2276 before and after** (DEVLOG 1632). ⛔ What is OPEN is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). — **PRIOR (2026-09-19 — LANDING TRAIN 39 ON TOP OF REGISTRAR #6 AND TRAIN 38):** **the P14 traceability inventory is 4,348 rows at GAP 2407.** **Landing train 39 carried FOUR clusters and NINE notes in ONE landing** — PB425 (the finisher train 38 dropped) + PB430 (`MoveClassifier.CanBeZeroLengthItem` writes §8.5.4 down ONCE, so `NeedsLengthFreeze` stops asking a SHAPE question where §14.9.25.4 GR1 asks whether identifier-1 IS a zero-length item and stops re-rounding every function-result MOVE; `IsGroupPlace` and `OperandText.NonElementaryMoveSender` are the single group/RENAMES decision and the single codec both arms and both emitter paths ask) · PB392 + PB423 (`ItemCategory.GroupKindsOf` returns the five group kinds as a [Flags] SET where §14.9.44.3 SR6 and §14.9.2.3 SR6 name a SET and a boolean stood; `MoveTable16.Sr1ClassRefusal` is §14.9.25.3 SR1 asked of ONE operand's class, so the RECEIVING arm stops pattern-matching one hand-written usage — `MOVE P TO Y` printed `Y=[CobolNet]`, the CLR carrier's type name, into a user's PIC X(8)) · PB399 (EVALUATE's four unscreened syntax rules, `ScreenObjectCount` COBOLNET2106 in BOTH directions and `ScreenRangeOperands` COBOLNET2107 over the range PAIR; the §8.8.4.2 Format-3 band MOVED out of one CALLER into `StatementValidation.CheckFormat3Relation` at the `CheckedRelational` checkpoint, so IF, EVALUATE, SEARCH WHEN, PERFORM UNTIL and abbreviated relations all get it) · PB404 + PB407 + PB409 + PB410 (PB403's `EnclosingContext` gains its missing ASKER — `PlacementRules.RefusedInGlobalDeclarative` COBOLNET2102 and `RefusedRaisingLastHere` COBOLNET2103, one method per RULE SHAPE — and GOBACK's general format is RE-DERIVED from the printed page at 300 dpi, PDF 661 and 653, because an OCR'd diagram is lossy toward falsely-restrictive syntax and six legal spellings were being rejected) — **GAP 2423 → 2407** with the denominator unchanged at 4,348, **seventeen verdict records over four batch files re-applied on the MERGED tree in landing order**, sixteen closing their row and `GR-14.9.25.4-1` left PARTIAL on purpose (its residue is PB394's variable-extent group sender). ⛔ **THE TRAIN FOUND A COMPOSITION DEFECT THAT LIVES IN A STRING, AND ONE TEST OUT OF 27,772 SAW IT**: `SpecTraceabilityInventoryDriftTests.EveryCodeLocation_ResolvesInTheTree` reported `SR-14.9.25.3-1: MoveBinder.cs#MoveFigurativeEditionGates — no longer in that file`. Cluster 2's batch pinned SR1's MoveBinder site at the name that method carried on PB423's base; cluster 1's finisher RENAMED it `MoveOperandClassChecks` in the same train, and was right to — once the `StoreAsImage` marking moved out to `MarkFillImageStorage`, what is left in it IS SR1 and nothing else. Both clusters' own gates were GREEN; only the merged tree holds both facts, and only a test that RESOLVES a reference rather than pattern-matching it can say so. Repaired through a correction BATCH (verdict untouched, GAP unmoved), never by hand-editing the inventory, and SWEPT: `kb/Work/PB422` and `PB424` are OPEN and their pins now name the live symbol, `PB348` (landed, and its sentence describes the CURRENT architecture) takes the live name, `PB423`'s BEFORE-state narrative keeps its history with the rename annotated inline, and `docs/PHASE4_RECONCILIATION.md` is left alone because its own header declares it a FROZEN HISTORICAL LEDGER. No `.cs` file names the dead symbol. ⛔ **No generated or list file was three-way merged**: the five corpus manifests were edited by a script asserting each list's element COUNT before and after (85 92 → 94, 2002 285 → 289, 2014 98 → 99, 2023 604 → 606, negative 1127 → 1144 — nine positive goldens and seventeen negatives), `docs/DIAGNOSTICS.md` was REGENERATED by `scripts/gen-diagnostics-doc.ps1`, the two `Cobol.Net.Editions/*.g.cs` were regenerated from `constructs.json`, and the inventory came only from the batches; the corpus is now **1,083 positive goldens and 1,145 negative fixtures**. ⛔ **THE FIRST CI RUN WAS RED (35468418522) AND A FILTER TERM NO GATE IN THIS TRAIN RAN IS WHY**: the `matrix-constructs` shard failed on Windows AND Linux, three cells of ONE row — `VersionMatrixTests.Construct_MatchesEditionExpectation("statement-raising-2002", 2002 / 2014 / 2023)` — with cluster 4's new COBOLNET2103. That row's sample writes `GOBACK RAISING LAST EXCEPTION` in an ORDINARY PARAGRAPH and §14.9.18.3 SR5 admits the LAST phrase only in a declarative or a PERFORM WHEN phrase (`cite.py --check` OK on the GOBACK wording, which is NOT the near-identical §14.9.14.3 SR6 EXIT wording). The compiler is RIGHT and the version matrix had been pinning non-conforming source as ACCEPTED at three editions, so the SAMPLE was repaired — into a `USE AFTER EXCEPTION CONDITION` declarative, the EC model the row is itself about; `RAISING exception-name` (binds as SR4's object-reference form) and a Format-3 PERFORM WHEN (2023-only, would break the 2002/2014 compile cells) were both measured and rejected. `push-main.sh` held main untouched throughout. **The lander union carries `~VersionMatrix` from now on** — it alone selects 2,330 Conformance cases and NO cluster's gate and no earlier union term touched one of them; train 37 learned this as `~CorpusRunner` and train 38 as the eight goldens only `~CorpusRunner` could see. The final **19-term** union gate — every term measured LIVE by `filter_population.py`, with `~GroupKindDriftTests` and `~MoveOperandClassDriftTests` named INERT against Conformance and then measured POSITIVELY in Unit (7 and 12) rather than believed — ran Conformance **5783 / 5783**, the UNFILTERED Unit assembly **24189 / 24189**, Characterization **33 / 33** and legacy `CobolSharp.Tests.Integration` **503 / 504**. Semgrep no rule up (`cobolnet-raw-diagnostic-code-literal` 417 → 415, and **the baseline IS tightened to 415 this train** — no concurrent train is in flight to be reddened by the ratchet); `audit_code_citations.py`'s gating arm **⛔ 0 findings** and its non-gating rule-ordinal arm measures **205**, up from 203 because of registrar #6's own new notes, owned by PB388's sweep; `work.py check` 939 items. **FIVE codes are claimed — COBOLNET2102, 2103, 2104, 2106, 2107**; COBOLNET2099–2101 and 2105 were allocated centrally and came back UNUSED, and **the next free code is COBOLNET2108** — do not reuse the returned four. ⚠ `COBOLNET0712` and `COBOLNET0868` were EMITTED by `src` on main and had no `DiagnosticCatalog` row, so `docs/DIAGNOSTICS.md` documented neither; both gained descriptors here. Measured tree-wide: **162 codes emitted but uncatalogued on main, 160 here** — pre-existing, already owned by the OPEN `kb/Work/PB424`, and NO new note or list was opened for it (rule 8). ⚠ Three of the clusters' inherited leads DIED on re-measurement (PB423's `CorrEligible` claim, PB399's p12 — whose premise was wrong, an `OCCURS … DEPENDING ON` group is NOT a §8.5.1.12.1 variable-length group and the ODO form is now pinned LEGAL — and PB410's sweep item 3). Notes flipped to `landed`: **PB430, PB392, PB423, PB399, PB404, PB407, PB409, PB410**; **PB425** stays `landed` with a "Landed — the FINISHER" section. No cluster was dropped. **REGISTRAR #7 ran on 2026-09-20 over the wave-39/40/41 reports and the train-40/42 lander reports and the register is 971 notes**: forty-eight leads, seven already owned and dropped, **PB907–PB938** filed (thirty-two mechanisms, one of them the owner decision PB935 on `SET pp TO ENTRY` vs §13.18.60.4 GR25), seven notes EXTENDED rather than duplicated (PB388, PB212, PB880, PB706, PB529, PB533, PB160) and thirteen given `cluster:` back-links. Fifty-seven probes re-run on its own build and sixty-two citations re-derived; **three reported findings did not survive the re-run** (the `GO TO DEPENDING` stage defect is fixed by the wave-41 PB412 group, the qualified capacity register by PB457, and the SET `U+2026` claim is half PB899's) and **one report's cited quotation is in no clause at all**. It moved NO verdict — GAP is 2339 before and after (DEVLOG 1623). **REGISTRAR #8 ran on 2026-09-21 over the six wave-42 implementer reports (PB455, PB415, PB395, PB587, PB394, PB388), the train-43 lander report, the battery-81 record and DEVLOG 1622–1625, and the register is 982 notes**: twenty-two leads, **PB939–PB949** filed (eleven mechanisms — two of them already written down inside `kb/Work/PB933`'s PROSE, where no ranker could see them), eleven notes EXTENDED rather than duplicated (PB498, PB362, PB388, PB450, PB212, PB929, PB515, PB933, PB376, PB608, PB921) and eleven more given `cluster:` back-links. **PB388's three orphan rows have owners** — `GR-14.9.39.4-12`/`-13` to PB450, `SR-14.9.39.3-2` to PB212, `SR-14.9.39.3-10` to PB929's golden round — and PB941 claims SEVEN `>>PUSH`/`>>POP` rows that had no claimer at all. **Three reported findings did not survive the re-run**: the §13.18.63.3 SR6 Format-3 lead is CLOSED by PB921 (which landed in train 43), PB498's `value-clause-p2` premise is STALE since PB761, and `SR-14.9.39.3-10`'s stated reason is stale since PB496 — `objectReferenceUsage` spells FACTORY OF and ACTIVE-CLASS today. It moved NO verdict — GAP is 2310 before and after (DEVLOG 1626). ⛔ What is OPEN is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). — **PRIOR (2026-09-13 — TRAIN 38 AND REGISTRAR #6):** **the inventory stood at GAP 2423.** **REGISTRAR #6 ran on 2026-09-19 and the register is 939 notes** — the lead-filing pass over the twenty-four implementer/finisher reports and the train-33-38 lander reports filed **PB878-PB906** (twenty-nine mechanisms), extended PB388 and PB446, and gave fifteen existing notes `cluster:` back-links so the fill hands one implementer a whole group (owner decision 2026-09-13). It moved NO verdict — GAP is 2423 before and after. Three inherited leads died on re-measurement, one of them a fix PB391 had already landed; DEVLOG 1616 has the forensics. ⛔ What is OPEN is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). **Landing train 38 carried FIVE clusters and landed FOUR** — PB396 (an imperative-statement an unbracketed general format prints is REQUIRED: `statementBlock*` → bare `statementBlock` at NINE positions across four verbs, `WHEN OTHER` lifted into the single trailing phrase §5.2.7 says it is, `EvaluateBinder`'s LAZY subject slot forced before any arm binds for §14.9.13.4 GR3, and `Frontend.Parse`'s SLL/LL DOUBLE-REPORT fixed on the way; COBOLNET2072/2073) · PB411 (`ItemCategory.UsageOf` is THE ONE §8.5.2.1 usage reader — an alphanumeric group "is treated as though it had a usage of display", so `STOP RUN / GOBACK WITH ERROR STATUS <group>` stopped being rejected — and `CallBinder.DecodeGobackPhrases` reads §14.9.18.2's format ONCE before the GR2/GR4 fork, so a METHOD's GOBACK can no longer drop the 2023 status phrase in silence) · PB420 (a STALE deferral arm refused INITIALIZE's implicit MOVE into every COMP-1/COMP-2/FLOAT-* receiver and ABORTED the run unit; the arm is DELETED, not replaced, and `AcceptDisplayEmitter`'s Format-2 twin with it) · PB391+finisher (§14.9.25.3 SR8 **and** SR9 are asked under CORRESPONDING through ONE composite entry `MoveTable16.DataItemRefusal`, because §14.7.6 rule 2 defers to "the rules for the MOVE statement" plural and SR10 governs only "all other cases not described in Syntax rules 8 and 9") — **GAP 2435 → 2423** with the denominator unchanged at 4,348, sixteen verdict records over six batch files re-applied on the MERGED tree in manifest order (the two PB391 batches collide BY DESIGN — the finisher supersedes `pb391-rule2-rule4` on `SR-14.9.25.3-13` and `GR-14.9.25.4-11` — so they were applied as two runs, in order). ⛔ **THE FIFTH CLUSTER, PB425, WAS DROPPED BY THE TRAIN'S OWN GATE AND ONLY `~CorpusRunner` COULD SEE IT**: eight goldens MAIN ALREADY CARRIED went red (`Failed: 8, Passed: 3451, Total: 3459` — `pb65_dec_move_channel`, `pb60_numval_standard_decimal`, `pb60_numvalf_native_channels`, `pb251_numval_native_value_fixed`, `pb253_prose_float_standard_container`, `pb253_prose_float_sdidi_container` ×2 editions, `l1_numval_family_cap_discriminators`), every one a NUMVAL/float/decimal program no cluster wrote. Bisected by cluster on each checkpoint's own build: `MOVE FUNCTION E TO EW` prints the golden's `2718281828459045235360287471352` at the PB420 boundary and `2718281828000000000000000000000` one commit later. `MoveClassifier.NeedsLengthFreeze` answers true for `BoundComputedOperand { Expr: BoundIntrinsicCall }` against ANY numeric receiver, so `MoveBinder.BindMoveOf` materialises EVERY function-result MOVE into `SendingValueTemp`'s carrier and truncates the STANDARD-DECIMAL/SDIDI value — while §14.9.25.4 GR1's sentence names **identifier-1** being a §8.5.1.10 zero-length item, which a numeric function result can never be. The cluster was dropped WHOLE (patch, goldens, manifest entries, the D-B2 determination and its two verdict records; `kb/Work/PB425` stays `open`) and returned to its implementer with that attribution, NOT repaired by the lander. PB425's own gate ran `~ZeroLengthLiteralMove|~Move|~Drift|~EditionGate` — 533 Conformance cases and NO `~CorpusRunner` — so the 2,218-case complement where all eight reds live was never measured: an implementer gate without `CorpusRunnerTests` measures only what the change's author thought to name. ⛔ **No generated or list file was three-way merged**: the five corpus manifests were edited TEXTUALLY by a script asserting each list's element count and then re-parsing the JSON (85 89 → 92, 2002 283 → 285, 2014 97 → 98, 2023 602 → 604, negative 1121 → 1127 with `pb169-status-group-identifier` REMOVED — it pinned the rejection of legal source), `docs/DIAGNOSTICS.md` was REGENERATED by `scripts/gen-diagnostics-doc.ps1`, and the inventory came only from the batches. The union gate — 15 terms on the first run, **14** after the drop, every one measured LIVE by `filter_population.py` with `~RequiredImperativeStatement` and `~StaleDeferral` named INERT against Conformance (both classes live in the UNFILTERED Unit assembly) — ran Conformance **3445 / 3445 (run 2, after the drop — run 1 was 3451 / 3459)**, the UNFILTERED Unit assembly **24162 / 24162**, Characterization **33 / 33** and the legacy `CobolSharp.Tests.Integration` assembly **503 / 504**. Semgrep no rule up (`cobolnet-raw-diagnostic-code-literal` 419 → 417, and the baseline is TIGHTENED to 417 this train); `work.py check` 910 items; the non-gating rule-ordinal arm of `audit_code_citations.py` measures **203**, unchanged. **TWO codes are claimed, COBOLNET2072 and COBOLNET2073** (PB396's); COBOLNET2074 and 2087–2095 were allocated and returned UNUSED, and **the next free code is COBOLNET2099** — the central allocation runs to COBOLNET2098, so do not reuse it. ⚠ ONE repair the train made itself: the new `CobolControlFlow.g4` comment pinned its drift guard as `GrammarRequiredImperativeDriftTests`, a class that does not exist (`RequiredImperativeStatementDriftTests` does, and the design doc already says so) — a cross-reference you cannot follow is not a pin. Notes flipped to `landed`: **PB396, PB411, PB420, PB391**. ⛔ The worklist is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). — **PRIOR (TRAIN 37):** **Landing train 37 carried FIVE clusters in ONE landing** — PB408+PB406 (§14.9.18.4 GR1 b) asks the ACTIVATOR whether checking is enabled, so `BoundRaising.Enabled` is DELETED and every activating statement carries its own element's `EcCheckingProfile`; GR1 b) 3. a)'s EC-RAISING-NOT-SPECIFIED substitution lands with it) · PB447 (SEARCH is lowered ONCE PER FORMAT, because GR4 licenses the N+1 overshoot GR9 forbids; `EmitSerialScan` / `EmitAllScan` from one shared frame) · PB419 (a multi-operand statement IS N statements — `BoundImplicitSeries`, and `EcWrap` DISTRIBUTES the checked wrapper over its members) · PB427 ("same type" is §8.5.3.1's relation over `EquivalentTypeDeclarations`, spent by eight rules including §14.8.2.2 2)'s strongly-typed-argument sentence, which was implemented nowhere) · PB443 (SEARCH's identifier-1 is a WRITTEN reference resolved once, through `ReferenceResolver.ReadWritten` and `ResolveTableOperand`) — **GAP 2443 → 2435** with the denominator unchanged at 4,348, ten verdict records over five batch files re-applied on the MERGED tree in manifest order. Every cluster was based BEFORE the trains between it and main (`0b1b48f9`, `db68907f` ×2, `f8fa6452` ×2), so all five were three-way merged and every conflict resolved against the spec. ⛔ **No generated or list file was three-way merged**: the corpus manifests were inserted by a script asserting the `enabled` element COUNT before and after (2002 280 → 283, 85 87 → 89, negative 1110 → 1121), `docs/DIAGNOSTICS.md` was REGENERATED by `scripts/gen-diagnostics-doc.ps1`, and the inventory came from the batches. **THREE `D14`s collided in `COBOLNET_CONTROL_FLOW_DESIGN.md`** — PB403 keeps D14, PB447 landed as **D15** and PB443 as **D16**. ⛔ **THE TRAIN FOUND A COMPOSITION DEFECT NEITHER IMPLEMENTER COULD SEE**: PB408's `StampActivators` recursed only through `BoundSequence`, the one container shape that existed when it was written, while PB419 added `BoundImplicitSeries` and routed the UDF/OO desugars through its `Rewrap` — so a function activation hoisted out of an operand of a multi-operand CLOSE/FREE/INITIALIZE/INITIATE/OPEN/TERMINATE reached NEITHER arm and carried an empty profile, silently re-creating the very defect PB408 landed to fix; it now traverses both shapes, as `QueryFor` beside it always has. The 13-term union gate — `~Exception|~Goback|~Raising|~Search|~Initialize|~Resume|~Typedef|~Pointer|~Drift|~EditionGate|~CorpusManifest|~CorpusRunner|~Nist`, every term measured LIVE by `filter_population.py` — ran Conformance **3193 / 3193**, the UNFILTERED Unit assembly **24093 / 24093**, Characterization **33 / 33** and legacy Integration **503 / 504**. ⛔ **THE FIRST CI RUN WAS RED (34799104450) AND THE THIRTEENTH TERM IS WHY**: the `corpus-nist` shards, Windows and Linux, refused MAIN's own `85/pb877_subscripted_subscript` with PB443's new COBOLNET2075 — the golden wrote `SEARCH ALL T-E (T-N (1), TX)` and `TX` is T-E's own index, which §14.9.37.3 SR2 forbids on identifier-1 (§8.4.2.3.3 SR5 a) exempts a SEARCH subject from the subscript count for that reason). The screen is right, the golden was repaired to write only the enclosing occurrence, and its point — the SUBSCRIPTED SUBSCRIPT `T-N (1)` — is untouched. A screen NEW to a train must be measured against the WHOLE corpus and only `CorpusRunnerTests` executes it; the 12-term union ran 992 Conformance cases and none of main's goldens, the 13-term one runs 3193. `push-main.sh` held main untouched throughout; semgrep no rule up (`cobolnet-raw-diagnostic-code-literal` 419 → 417, baseline again left untightened); `work.py check` 910 items; the non-gating rule-ordinal arm of `audit_code_citations.py` measures **203**, unchanged. **ONE code is claimed, COBOLNET2075** (PB443's SEARCH identifier-1 operand position); COBOLNET2069–2071 and 2076–2086 were allocated and returned UNUSED, and **the next free code is COBOLNET2099** — the central allocation runs to COBOLNET2098, so do not reuse it. Notes flipped to `landed`: **PB408, PB406, PB447, PB419, PB427, PB443**; no cluster was dropped. ⛔ The worklist is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). — **PRIOR (THE PB877 BLOCKING-FIX TRAIN):** **the inventory stood at GAP 2443.** **Landing train PB877 carried ONE cluster in ONE landing, ahead of trains 37 and 38, because it is the regression battery #77 caught** — the GnuCOBOL differential case `run_initialize:298` had flipped AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT at train 35's head. §8.4.2.3.3 SR2 had been written down by halves (`DataItem.IsTable`, a table-RECOGNITION test, where SR2 also admits an item "subordinate to a data description entry that contains an OCCURS clause"), so a subscript that is itself a subscripted identifier was split off as an EXTRA subscript: loud on the receiving side (COBOLNET0899) and a RUN-TIME ABORT on the sending side. SR2 is now `DataItem.IsTableElement`, SR3 is `DataItem.SubscriptArity`/`SubscriptLevels()`, `ReferenceResolver.CannotBeSubscripted` is the ONE declaration-informed `'('` predicate passed by all three `SplitSubscriptTokens` callers, the five hand-written arity walks are one, and `ReferenceResolver.ScreenSubscriptArity` reports both rules at compile time on BOTH sides (COBOLNET2096/2097), firing only where subscripts are WRITTEN because SR5's seven exceptions are all about omitting them. **GAP 2456 → 2443 in this train's own arithmetic terms: 4 verdict records re-applied on the merged tree closed FMT-8.4.2.3.2, SR-8.4.2.3.3-2 and SR-8.4.2.3.3-3 and refreshed GR-8.4.2.3.4-1 — the row that had been CONFORMS for weeks on five goldens which every one varied the subscript's VALUE and none its SHAPE.** PB877's base was `ccf03eed` (train 35); train 36 touched six of its nineteen files but NOT `ReferenceResolver.cs`, and no JSON hunk was three-way merged — the inventory was reproduced by re-applying the batch, both corpus manifests by a script asserting the `enabled` element COUNT before and after (85: 86 → 87, negative: 1108 → 1110) and `docs/DIAGNOSTICS.md` by `scripts/gen-diagnostics-doc.ps1`. The 14-term union gate — `~Subscript|~Initialize|~Drift|~EditionGate|~Nist|~CorpusRunner|~Search|~Intrinsic|~Occurs|~Odo|~Report|~RefMod|~Reference|~Table`, every term measured live by `filter_population.py`, with `~SubscriptAdmission` measured INERT against Conformance and DROPPED — ran Conformance **3432 / 3432**, the UNFILTERED Unit assembly **24062 / 24062**, Characterization **33 / 33** and legacy Integration **503 / 504**; semgrep no rule up (`cobolnet-raw-diagnostic-code-literal` 419 → 417, baseline again left untightened); `work.py check` 910 items. The differential case was re-verified on the merged build, not inherited: compile exits 0 and the program prints NOTHING, which is its own success condition. **TWO codes are claimed, COBOLNET2096 and COBOLNET2097; the next free code is COBOLNET2099 — the orchestrator allocated COBOLNET2098 to PB877 centrally and it came back unused, so do not reuse it.** ⛔ **AND THE TRAIN REPAIRED THE PROBE THAT ANSWERS THAT QUESTION.** `scripts/session-probe.ps1`'s `diag` line printed `src-grep max COBOLNET1999 · catalog max COBOLNET1999 → next free = COBOLNET2000` on a tree whose catalogue already held COBOLNET2096/2097: its scan was pinned to the `COBOLNET1xxx` band, so the moment codes crossed 2000 the one mechanism this section tells you to allocate from ("claim the next free code by RUNNING `session-probe.ps1`, never by reading a list") began handing out a code that is already taken — the COBOLNET1535/1573 collision class, re-armed. The regex had been widened from a decade to a band once before, for exactly this reason, which is what makes a band the defect rather than the value: it is now pinned to no band at all (`COBOLNET[0-9][0-9][0-9][0-9]` in both scans) and reports `src-grep max COBOLNET2097 · catalog max COBOLNET2097 → next free = COBOLNET2098`. **PB877 → `landed`. The register is 910 notes.** ⛔ The worklist is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). — **PRIOR (TRAIN 36):** **Landing train 36 carried PB555 + PB401 + PB403 + PB388 + PB436 onto main in ONE landing** — five cluster commits plus one closing DEVLOG/§0 commit (DEVLOG 1609) — **GAP 2456 → 2446 with the denominator unchanged at 4,348**, 11 verdict records over four batch files re-applied on the MERGED tree in manifest order plus ONE the lander measured itself. A sixth cluster, PB391, was HELD OUT of the train by the orchestrator (it needs a finisher now that train 35's PB416 is on main) and neither its patch nor its batches were applied. PB555 was based on `b929f96e` and the other four on `d80cb676` — before trains 34 AND 35 — so every cluster was three-way merged onto train 35's head, and every conflict was resolved against the spec, not by taking a side: `SetBinder.BindSetCondition` keeps MAIN's PB390 §14.9.39.3 SR6 rejection over PB555's re-worded `BoundUnsupported` (and the local it fed was deleted with it); `StatementEmitter.Visit(BoundExitPerform)`'s `_ =>` arm keeps PB403's THROW while every other transfer site keeps train 35's PB405 idiom; `SetEmitter.EmitSet` keeps PB555's one-store-path block, whose comment already carries the §14.9.39.4 GR6/GR7 citation PB388 was correcting on the line it replaced; `PerformVaryingDifferentialTests`' summary keeps PB388's clause correction AND PB436's refutation of what the tests measure; the two conditions-and-exceptions design-doc conflicts are keep-both. ⛔ **No corpus `manifest.json` and no `traceability-inventory.json` hunk was three-way merged at all** — a conflict INSIDE one JSON list element is silently lossy (train 33's `cobol-words.json` row) — so every golden was inserted by a script that parses the JSON, asserts the `enabled` element COUNT before and after and asserts no duplicate (2002 276 → 280, 85 85 → 86, negative 1098 → 1108), the two generated `Cobol.Net.Editions/*.g.cs` were REGENERATED from `constructs.json` (236 → 238 rows) rather than merged, and all four verdict batches were re-applied from their batch files. **THE TRAIN FOUND AND FIXED A GATE DEFECT OF ITS OWN**: PB388's new FORMAT-NAME check went RED on `tests/conformance/negative/pb489-linage-operand-is-linage-counter.cob:6`, a CORRECT line — §8.4.2.2.2 Format 7 and §8.4.3.1 Format 10 are BOTH printed `qualified-linage-counter`, and the whole-line name test attributed the second citation's name to the first. The repair is structural, `_owning_cite` gives a spelled format NAME to the citation it stands NEXT TO, with the two-citation line added to `ORDINAL_SELF_TEST` as a silent twin beside a firing defect; `--self-test` PASS, `--check` **⛔ 0 findings**. The 19-term union gate — `~CorpusRunner|~CorpusManifest|~EnabledNegativeCase|~Drift|~EditionGate|~VersionMatrix|~ThroughRangeException|~Evaluate|~Exit|~ControlFlow|~Perform|~Resume|~Declarative|~PointerData|~Set|~Search|~Diagnostic|~Collation|~Nist`, extended beyond the clusters' own terms with `~CorpusRunner` (all five clusters add goldens and only `CorpusRunnerTests` executes them) and `~Collation` (PB555 and PB401 both reach the collation carrier), every term measured by `filter_population.py` — ran over Conformance, the UNFILTERED `Cobol.Net.Tests.Unit` assembly, Characterization and the legacy `CobolSharp.Tests.Integration` assembly: Conformance **5501 / 5501**, the full Unit assembly **24057 / 24057**, Characterization **33 / 33** and legacy Integration **503 / 504**. `~Collation` was measured INERT against Conformance (its 164 tests live in Unit, which this gate runs unfiltered) and is named rather than believed. **THE ONE ROW THE TRAIN MEASURED ITSELF**: `GR-14.9.37.4-1` was left PARTIAL by PB388 for the lander, because the clause holding it open — GR1 b) 2's transfer of control — was PB405's dispatcher-`break` defect, not a citation. Measured on the merged tree with a freshly built `cobol.exe`: `PERFORM 3 TIMES … SEARCH E … WHEN E(IX1) = 3 GO TO HIT-PARA … END-PERFORM` prints `LOOP` then `HIT-PARA` where the adjudication recorded `LOOP` then `NOT-REACHED-2`, and the isolating control (a bare `GO TO` out of an inline PERFORM, no SEARCH) prints `C1-OUT`. PARTIAL → CONFORMS, witnessed by PB405's `DispatcherTransferIdiomDriftTests`, whose fixture carries a `GO TO` inside a SEARCH `AT END` phrase and one inside an EVALUATE arm, both inside an inline PERFORM. Semgrep: no rule increased; `cobolnet-raw-diagnostic-code-literal` fell **419 → 417** and the baseline was again left UNTIGHTENED, for train 35's reason — a concurrent train's CI must not be reddened by a ratchet it did not cause. **TWO diagnostic codes are claimed: COBOLNET2048** (PB555, §13.18.63.3 SR27's literal-4-distinctness) and **COBOLNET2049** (PB555, §14.9.39.3 SR7's missing FALSE phrase); no other cluster needed one, and COBOLNET2051–2068 were allocated to clusters that did not use them and are returned FREE. The next free code AT TRAIN 36 was **COBOLNET2050** with the central allocation running to COBOLNET2095 (both superseded by the PB877 train above). ⚠ Pre-existing and NOT this train's: `COBOLNET1535` is allocated TWICE in `DiagnosticCatalog`; and the non-gating RULE/SUBITEM arm of `audit_code_citations.py` measures **203 rule-ordinal citations naming a rule number or sub-item their clause does not have** on the merged tree (PB388 reported 195 on its own base) — one sweep, owned in `kb/Work`. Notes flipped to `landed`: **PB555, PB401, PB403 and PB436**; **PB388 stays `half`** (GR-14.9.39.4-12/-13 DIVERGES on `setAddressStatement`'s single hard-coded receiver, SR-14.9.39.3-10 PARTIAL on PB389, SR-14.9.39.3-2 PARTIAL with SR2 written nowhere). No note was filed by the lander (the registrar owns PB877+). **The register was 909 notes at train 36.** ⛔ The worklist is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). ⛔ **THE FIRST CI RUN WAS RED (run 34791392702), AND IT FOUND A GATE THE WINDOWS BATTERY CANNOT RUN.** `push-main.sh` refused to touch main: the Guard job's `scripts/guard-fast.sh` reported `NC201A.txt has 1 FAIL*` and `NC201A.txt footer 001 TEST(S) FAILED` while its OWN NIST leg said `364 MATCH, 0 REGRESSION(S)` and the audit said CLEAN. The red is PB436's, and its mechanism is the duplication review dimension: `guard.sh` and `guard-fast.sh` each carried a hand-kept COPY of the baseline-cleanliness loop — `guard-fast.sh`'s comment said "parity with guard.sh" — and PB436 taught the CCVS allowance to `guard-verdict.sh`'s comparison arm and to NEITHER copy. Both copies are DELETED in favour of one `scripts/guard-baselines.sh`, which reads the allowance from `tests/nist/corpus.tsv`'s `CCVS-DEFECT` rows — the same declaration, from the same file, that `CorpusManifestTests` states as a set equality — refuses a declaration whose row is not `divergent` (its ISO citation would go unenforced), and measures the SELECTOR'S COMPLEMENT, so a declaration left behind after a golden is repaired is itself a defect. TWO of `guard-verify.sh`'s witnesses were stale against PB436's own change and are repaired with it: the MATCH message now names the golden's allowance, and a footer the GOLDEN ITSELF carries is a MATCH rather than a regression — that arm is reached only after a byte-for-byte match, so comparing the footer to the constant 0 was auditing the BASELINE and not the run. Six NEW witnesses fire every arm of the new audit (undeclared `FAIL*`, undeclared footer, declared-and-allowed, not-`divergent`, STALE declaration, 0-byte), each proved to refuse before it was trusted; `guard-verify.sh --witnesses` ALL GREEN, and the wave gate re-ran GREEN over `~CorpusManifest|~Drift|~EditionGate|~SpecPinnedNist` (Conformance 321/321, Unit 24057/24057, Char 33/33).
  ...
  **In flight now (fifth resume, 2026-09-04 21:00 PDT — every stream FRESH from its disk checkpoint):** **battery #45** ran in a worktree cut at `b3aad3fc` (landing train 2's head) and is **GREEN ON EVERY COMPILER LEG with the guard's new evidence witnesses and the NIST audit CLEAN**; of its five GnuCOBOL differential flips, two are PB206's spec-licensed rejections (rebaselined), one is an over-rejection PB206 completed — `kb/Work/PB598`, the VALUE-clause size sentences applied to a level-88 subject — and two were battery #44's known pair owed to `kb/Work/PB485`, **which landed with train 3 (`f37da577`): both returned to their committed rows** (the "Gates" bullet carries the numbers); lane-3 **batch 3 is REGISTERED — 226 rows over the eight data-description clauses, GAP 3005 → 2992, notes `PB486`–`PB580` plus five extensions** (DEVLOG 1466; verdicts PARTIAL 116 · DIVERGES 38 · NOT-IMPLEMENTED 36 · CONFORMS 36, of which 16 close their row and 20 are CONFORMS-but-untested and listed in `kb/Work/PB370` for golden-lane round 4; four rows carrying an OWNER verdict were deliberately NOT recorded and are the owner question in `kb/Work/PB579`); golden-lane **round 3** resumed from the two finished writers; the **fix lane carries no finished work** — train 5's four are landed — (train 1's four — PB203, PB201, PB470, PB193 — train 2's six — PB473, PB204, PB205, PB206, PB207, PB208 — train 3's five — PB231, PB485, PB230, PB236, PB235 — train 4's four — PB233, PB598, PB237, PB238 — and train 5's four — PB248, PB250, PB251, PB252 — are landed). ⚠ Batch 3 was also the measurement of the note-yield outside §14, which
  `kb/Work/PB469` names as the softest number in the whole projection, and it is now MEASURED: **0.42 notes per
  adjudicated rule and a CONFORMS share of 15.9 %** against §14's 0.29–0.46 and 26 % — the data-description
  clauses are about twice as defective as the statements, and the GAP delta per adjudicated rule (5.8 %) is half
  batch b2's (11 %). DEVLOG 1466 carries the three-point table.
...
- **▶ SESSION HANDOFF (2026-09-02 afternoon — superseded by the evening bullet above):** the
  ...
  **DONE — the lane-3 batch-1 registrar landed** (DEVLOG 1437): 179 rows of OPEN · READ · RELEASE · RETURN ·
  ...
  3379 → 3227, `PB260`/`PB302` landed, `PB301` at `half` — landed whole in train 19), the CITATION-REPAIR SWEEP (DEVLOG 1439, GAP
...
- **▶ SESSION HANDOFF (2026-08-17 — superseded by the 2026-09-02 handoff above; kept as history until the next §0 sweep):**
  ...
  **4183/4183** · Characterization 33/33 · NIST 353/0 audit-clean · differential 1323 cases with TWO attributed
  ...
  No gate is owed.
  **✅ PB62 CLOSED (DEVLOG 1298, 2026-08-18)** — the ALL subscript is ONE enumerating operand (`TableAllPlace` +
  `AllCount` Fixed/Odo/Capacity, `CobolTable.AllArgs` at run time — the standard's three ranges, nested dynamic
  capacities included), admissible only where the definition repeats an argument (`RepeatsAnArgument`,
  ...
  **NEXT, in order:** ⓪ **PB108 — the tooling batch the owner adopted BEFORE T1 (2026-08-19, DEVLOG 1329):**
  ...
  ③ ✅ **PB64 T5 LANDED (2026-08-19, DEVLOG 1334)** — the OBJECT-COMPUTER CHARACTER CLASSIFICATION clause binds
  (`ClassificationSpec` / `LocalePhrase`) and is RESOLVED AT EACH ACTIVATION into the module's `__CLASSIFY`
  (`CharacterClassification.Resolve` — §12.3.6.4 GR5/GR8, §14.6.6 r2); UPPER-CASE / LOWER-CASE map through a LOCALE
  phrase's named locale (r2, `BindCaseFunctionWithLocale`), the classification (r3) or the invariant tables (r4); the
  class tests ALPHABETIC / -LOWER / -UPPER classify per LC_CTYPE (GR3 b1/c1/d1 — letters only, space NOT alphabetic
  under a classification, documented); `LocaleFacts.Require` is the ONE §8.2.1 gate (EC-LOCALE-MISSING / -INVALID at
...
- **PER COMMIT — the WAVE-LOCAL filtered gate ONLY (~2 min):** the fix's own tests + immediate neighbours +
  ...
  ⛔ Do NOT run the full Conformance suite per fix (owner-corrected; §3 and `feedback_tiered_gates`).
  **The leg is ~10 min on the dev box** — it was ~12 min until the [[A13]] collection split (2026-09-02)
  took `VersionMatrixTests`/`NistDifferentialTests`/`CorpusRunnerTests` out of one serial xUnit
...
- **⛔ PRIOR BATTERY REFERENCE — the burn-down-cluster-A + seed-wave batch, battery #39, 2026-08-31.**
  ...
  **Next-free diagnostic code: `COBOLNET1728`** (1700–1704 were the cluster-B wave; 1705/1706 witness A,
  1707 the A.4.2 screen-statement surface, 1720 cluster E, 1724–1727 cluster F — the band has holes below its
  maximum). Never hand-read this — `session-probe.ps1` computes it from the catalog and the src grep,
...
- **▶ BURN-DOWN STATE after the 2026-09-02 WITNESS-CLUSTER-B2 landing (DEVLOG 1441), which followed fix-lane
  ...
  **GAP 3119 of 4311** (3636 → 3636 → 3618 → 3618 → 3467 → 3456 → 3407 → 3394 → 3379 → 3227 → 3223 → **3119**: cluster C was a
  ...
  A wrote the 49 witnesses that let lane 2a's A.4.8 + A.4.13 declines finally close, cluster F closed 13 on the
...
- **⛔ BATTERY REFERENCE — CURRENT: battery #86, 2026-09-23, on `c54434a8d` — trains 49 through 57 — run in an ISOLATED WORKTREE pinned at the batch head, never on main.**
  ✅ **ALL GREEN ON EVERY COMPILER LEG; the differential's 17 per-case flips are every one licensed by the standard, attributed to its landing and re-baselined, and the re-run over the corrected baseline prints `=== DIFFERENTIAL: 0 PER-CASE FLIP(S) ===`.** The script's own verdict was `=== BATTERY: NOT GREEN (rc=1) ===` on the flips alone — recorded as a MISS against #85's 0-flip bar. Conformance **8350 / 8350** (full, unfiltered; +418 over #85), Unit **29097 / 29097** (+351; GPL corpus copied in first: 1323 cases from 36 `.at` wrappers), characterization **33 / 33**, guard evidence-rule witnesses AND the compiler-identity watchdog green, NIST **364 MATCH / 0 REGRESSION** with the audit CLEAN over a declared population of 376, legacy legs 1203 / 1203 and 503 / 504 (1 skipped), all four static audits clean (`audit_code_citations` 0 over 5,164 files · `audit_doc_citations` 0 MISFILED / 0 ELIDED over 639 checked / 583 correct · `audit_evidence_supersession` 0 UNMARKED, 11 non-gating drifts · `audit_witness_loss` GREEN, and ALSO run `--base 0abe2c80f` over the nine-train span: 0 unexcused, 18 retired, 24 re-sited, GREEN), solution build 0 warnings / 0 errors. **The flips, every source extracted, compiled, the run-cases RUN (each printed GnuCOBOL's expected output), every citation `cite.py --check`ed:** EIGHT rejects → AGREE_ACCEPT — `run_fundamental:2459/2483/2507/2532/2577` (YYYYMMDD, YYYYDDD, INTRINSIC, RECURSIVE, AWAY-FROM-ZERO as data-names; **§8.10** context-sensitive words; PB655+PB764, train 54), `syn_functions:64` (`FUNCTION pi e intrinsic`, the **§12.3.8.2** intrinsic-function-specifier's name list; PB974, train 51), `run_fundamental:1317` (`f1()`, the **§8.4.3.2.2** optional argument list; PB969, train 51), `listings:1055` (`ASSIGN TO DISK name`, **§12.4.5.1** ellipsis + **§12.4.5.2 SR5** / DOC-A.1-71; PB829, train 49); FIVE WE_ACCEPT_THEY_REJECT → AGREE_REJECT — `syn_definition:2853` (**§14.9.49.3 SR1**, COBOLNET2377, PB361), `syn_file:999` (GO TO into a declarative, **§14.9.49.3 SR4**, COBOLNET2376, PB362), `syn_misc:619` (a bare literal / numeric item as a condition, **§8.8.4.2.1**, COBOLNET2318, PB982), `syn_occurs:359` (KEY qualified by a name that does not exist, **§13.18.38.3 SR3**, COBOLNET2353, PB1018), `syn_misc:4346` (ENTRY); FOUR hollow AGREE_ACCEPTs → WE_REJECT_THEY_ACCEPT — `run_extensions:6030/6100/6122` (INSPECT … TRAILING, which **§14.9.22.2** does not print) and `run_misc:4111` (ENTRY, which ISO/IEC 1989 does not define) compiled at #85 only as COBOLNET1756 staged run-time aborts, and PB909 (train 49) made them COBOLNET2269 compile-time refusals — GnuCOBOL's non-ISO extensions are not followed (CLAUDE.md rule 1). Re-baselined, diff exactly those 17 rows. Totals **573 / 473 / 242 / 35** (#85: 577 / 469 / 237 / 40); no NEW or REMOVED case, no case without a compiler verdict. **Complement:** 13 codes new to the corpus (COBOLNET2269 on 8 cases; 2318, 2353, 2375, 2376, 2377 …); COBOLNET1756 (staged refusal) 19 → 2 cases; 63 cases kept their verdict and changed their reason, mostly parse errors becoming named rules. **One LEAD, handed to the orchestrator:** `syn_misc:4346`'s ENTRY error masks its subject — a duplicate `PROCEDURE DIVISION USING p p o` alone reaches Roslyn and dies `CS0102 … '__lnk0'` where **§14.2.2 SR1** owes a COBOL diagnostic (row SR-14.2.2-1 is GAP). Wall **1,093 s** (Conformance 11 m 50 s). Batch: DEVLOG 1648–1661 — trains 49–57 plus registrar #12, GAP **2198 → 2060**; `git diff --shortstat 0abe2c80f c54434a8d -- src/ tests/` is 877 files, +36,993 / −5,513. DEVLOG 1663. Battery #87 at the next train's head must be ALL GREEN with 0 flips.
- **BATTERY REFERENCE — PREVIOUS (#85, superseded by #86 above): battery #85, 2026-09-22, on `0abe2c80f` — trains 47 + 48 — run in an ISOLATED WORKTREE pinned at the batch head.**
  ✅ Every compiler leg green; 3 per-case flips, all `WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT` conformance fixes (header-less `PROGRAM-ID.` by §11.2.1, table SORT without KEY by §14.9.40.3 SR15 / §14.9.40.4 GR21), re-baselined, re-run 0 flips — recorded as a MISS against #84's 0-flip bar. Conformance **7932 / 7932**, Unit **28746 / 28746**, characterization **33 / 33**, guard witnesses and watchdog green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN (population 376), all four static audits clean, differential **1323 cases**, totals 577 / 469 / 237 / 40. 1,230 s wall. DEVLOG 1646.
