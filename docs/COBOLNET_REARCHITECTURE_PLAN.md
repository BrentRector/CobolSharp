...
- **▶ LIVE STATE (2026-09-09 — TRAIN 27 IS ON MAIN; THE WITNESS-OWED BAND IS CLOSED):** **the P14 traceability inventory is 4,348 rows at GAP 2589.** **Landing train 27 — the owner's WIND-DOWN train — carried PB358 + PB165 + PB367 + PB523 + PB400 + PB490 onto main in ONE landing (six cluster commits plus one closing DEVLOG/§0 commit, DEVLOG 1590) — GAP 2609 → 2589 with the denominator unchanged at 4,348**, no cluster dropped, 28 rows re-verdicted across eight batches re-applied on the merged tree. **ONE diagnostic code is claimed by that train — COBOLNET1954** (the RECORD KEY / ALTERNATE RECORD KEY `SOURCE` decline, Annex A.3 item 40) — and thirteen return to the pool (COBOLNET1947–1953 and 1955–1960 were reserved and never used); the **next free code is COBOLNET1955**, src-grep max = catalog max = COBOLNET1954. ⚠ **FIVE NIST goldens were re-baselined to the conforming runs and are OWNER-VISIBLE**: `SQ212A` (PB367, 47 → 28 lines, its `corpus.tsv` row now `divergent` with the §14.9.49.3 SR1 citation — 16 CCVS sub-tests are no longer reached in that program, and the old golden had been baselined FROM the heuristic under repair by the very commit that invented it, `12f970df`), and `SQ201M` / `SQ208M` / `SQ209M` / `SQ210M` (PB523, margin blank lines and removed form feeds only — no PASS/FAIL line and no LINAGE-COUNTER value moved, each justified by the CCVS program's own printed visual-check text). Notes flipped to `landed`: PB358, PB293, PB818, PB367, PB523, PB400, PB490; `kb/Work/PB367b` is NEW and open; PB165 stays open on its fourth leg (the dynamic Format-1 lane's runtime per-formal DESCRIPTION registry, which P13 subsumes); PB398 gains two transferred rows. **PB818's row `SR-14.8.2.2-2` was deliberately left unadjudicated** — it and `SR-14.8.2.2-1` carry no verdict at all and §14.8.2.2 is territory the adjudication lane has never reached, so the cure is recorded and the verdict is not invented. The 2026-09-09 batches stamped the twelve ACCEPT Format-3 rows with their a42 witnesses (2748 → 2736, DEVLOG 1576) and closed three rows on owner-signed §8 derivations — OPEN GR12, INITIALIZE GR9, overlapping PERFORM GR2 (2736 → 2733, DEVLOG 1577; owner decisions Q25–Q27). **kb/Work PB245's fourteen §15 intrinsic rows then went to a GOLDEN ROUND** — every rule re-derived from the clause before anything ran, nine new spec-derived goldens and negatives written and measured on a build of the tree — and **thirteen closed CONFORMS while one (RV-15.4.1-4) was restated on its surviving residue (2726 → 2713)**; PB245's own MECHANISM, the missing back-link from a landed fix to the inventory rows it closes, stays open. **kb/Work PB258's seven CALL/DISPLAY/DIVIDE rows took the same round** (DEVLOG 1583): twelve new spec-derived goldens and negatives, and **six closed CONFORMS — including SR-14.9.11.3-1, which had been publishing a DIVERGES against a compiler PB148 already fixed — while SR-14.9.4.3-5 was restated (2713 → 2707)** on a residue no read would have found, the unwritable `ADDRESS OF` argument arm that SR-14.9.4.3-3 and -4 already hold. **Landing train 24 carried PB803 + PB366 + PB343 + PB592 onto main in ONE landing (four cluster commits plus one closing DEVLOG/§0 commit, DEVLOG 1579) — GAP 2733 → 2726 with the denominator unchanged at 4,348**, no cluster dropped, and **no diagnostic code was claimed** (COBOLNET1916–1917, 1918–1919 and 1922–1923 were reserved, never used, and return to the pool; the next free code is still **COBOLNET1859**). **Landing train 25 then carried PB440 + PB365 (reconciled with PB366) + PB389 + PB393 + PB495 (+PB520 +PB540) onto main in ONE landing — five cluster commits, one composition-repair commit and one closing DEVLOG/§0 commit, DEVLOG 1584 — GAP 2707 → 2659 with the denominator unchanged at 4,348**, no cluster dropped. **kb/Work PB451's ten SET format-5 rows and PB496's six USAGE OBJECT REFERENCE rows then took the golden round** (DEVLOG 1586): both notes predate PB389's descriptor and every measurement they rest on is now false, so all sixteen were re-derived from the clause, pinned by four new goldens and eleven new negatives, and **closed CONFORMS (2659 → 2643)**; the dead `oo-factory-object-reference` descriptor (COBOLNET0899's name for that use, never the CODE) is deleted now that no row cites it, and `docs/CONFORMANCE.md` gains **DOC-A.1-214**, the §13.18.60.4 GR22 a) object-reference storage determination A.1 required and nothing had written. ⛔ PB451's own repair 4 was WRONG about the standard — §14.9.39.3 SR10 d)'s "described with an IMPLEMENTS clause that references int-1" is the §11.4.4 / §11.8.4 GR2 relation, not a literal claim about the clause text — and implementing it would have made the compiler reject legal source; the withdrawal is recorded inline in the note. (Its batches were measured as −48 from 2713; the PB258 golden round moved main to 2707 while the train was gating, and the train was rebased onto it — the two sets of rows are disjoint and the merged inventory was re-measured, not assumed.) FIVE diagnostic codes were CLAIMED: **COBOLNET1920** (report-group-reference-ambiguous, PB365), **COBOLNET1924** and **COBOLNET1925** (object-reference ACTIVE-CLASS placement and the interface-alternative phrases, PB389), **COBOLNET1927** (usage-group-contradiction, PB495 — §13.18.60.3 SR2's first site ever) and **COBOLNET1931** (move-variable-length-incompatible, PB393); COBOLNET1921, 1926, 1928, 1929 and 1930 were reserved, never used, and return to the pool, so **the next free code is still COBOLNET1859**. **Landing train 26 then carried PB528 + PB452 + PB505 + PB487 onto main in ONE landing — four cluster commits, two train-level commits and one closing DEVLOG/§0 commit, DEVLOG 1587 — GAP 2643 → 2609 with the denominator unchanged at 4,348**, no cluster dropped. (Its batches were measured as −34 from 2659; the PB451/PB496 golden round moved main to 2643 while the train was gating, the train was rebased onto it, and the merged inventory was re-measured, not assumed — the two sets of rows are disjoint.) Every cluster edited the subsystem train 25 restructured, so they were brought in ONE AT A TIME with a solution build and that cluster's own drift tests between each; that is what surfaced the three composition seams, each resolved from the two reports and the spec — message-tag missing from `UsageFamilies.IsPictureless` against §13.16.3 SR8 (whose drift test had been holding it open as a forward obligation), the object-reference descriptor tuple PB389 introduced, and a §13.18.60.3 SR14 arm-B asymmetry that proved to be the cell **PB819** already registers for FUNCTION-POINTER, which gains the second cell. ELEVEN diagnostic codes were CLAIMED, each inside its cluster's assigned range with no collision across them: **COBOLNET1934/1935** (picture-composition, picture-precedence — PB528), **1938/1939/1940** (the three SET Format-15 screens — PB452), **1944/1945/1946** (the table-VALUE screens — PB505) and **1941/1942/1943** (unrecognized data clause, ALIGNED, MESSAGE-TAG — PB487); COBOLNET1936 and 1937 were reserved, never used, and return to the pool. ⛔ **PB452 stays `open` BY DESIGN**: SET Format 15 landed whole and Format 8 (function-pointer) did not, and that residue is PB817. ⛔ **THE WITNESS-OWED BAND IS CLOSED**: the last three owed rows landed as features — the DISPLAY over-punch representation and the valid sign set are now TWO conventions behind `--sign-encoding` (default `ibm` = IBM/Micro Focus compatibility) out of the ONE `ZonedSign` table model, with A.1 rows 177 and 178 filed (PB803, owner decisions Q28/Q29), and A.1 item 206 is declared **provided at the native range** with all eight BINARY-CHAR/-SHORT/-LONG/-DOUBLE bands (PB592, owner decision Q30) — `audit_annex_a1.py --check` reports no findings over a §7 A.1 table of 58 rows. ⛔ **A GOLDEN MAY NOW CARRY `*> options: key=value`** at the start of a comment line (`ConformanceCorpus.ApplySourceOptions`; unknown key or value throws), which is what lets a golden exercise a documented LATITUDE rather than one choice inside it. The train also landed two WRONG-ANSWER fixes: `USE AFTER EXCEPTION OBJECT class-name` now selects for a FACTORY exception object as well as an instance one (PB366 — a COBOL class is TWO disjoint emitted C# hierarchies and the selector tested one; GR14 b), the interface operand, is still COBOLNET0859 and stays PB365’s, so `GR-14.9.49.4-14` stays DIVERGES by design), and a relative `READ … PREVIOUS` after `OPEN INPUT` now makes the first existing record available instead of answering at-end (PB343 — the INDEXED block’s §14.9.30.4 GR21 d) 3 carve-out had been inherited through `VERSION_CHANGE_REFERENCE.md` row 29, **which named no organization**; row 29 is now scoped to INDEXED, the same wrong arm was swept out of the LEGACY oracle `RelativeFileHandler.ReadPrevious`, and a GREEN `SpecFixTests` case that PINNED the wrong answer was renamed and corrected). Train 24’s semgrep numbers are unchanged on every rule (3/46/2/423). **Landing train 23 carried PB802 ALONE onto main in ONE landing (one cluster commit plus one closing DEVLOG/§0 commit, DEVLOG 1574) — GAP 2748 → 2748 with the denominator unchanged at 4,348**, ⚠ **k = 1 BY OWNER INSTRUCTION** ("run one train (23 = PB802) thru completion, commit, push, then pause"), so the measured single-cluster landing cost was accepted deliberately; nothing was dropped, and **no diagnostic code was claimed** ('39' is an I-O status, and the patch carries zero `COBOLNET####` tokens; the next free code is still **COBOLNET1859**). ⛔ **THERE IS NO SIDECAR OF ANY TYPE** — owner decision Q24 (2026-09-07, verbatim: "Let's match GNUCobol's implementation in spirit. No sidecar of any type.") deletes the `.cbattr` catalog PB193 had landed, and §14.9.27.4 GR10's fixed-file-attribute comparison is now made against WHAT EACH ORGANIZATION'S PHYSICAL FORMAT ITSELF ENCODES, out of ONE virtual `FileConnector.FixedAttributeConflict` with three per-organization overrides sited beside the format each one reads — the hand-kept `MediumFixesRecordLayout` switch is deleted, and `EveryOrganizationStatesWhatItsOwnFormatEncodes` is the drift test that keeps a fourth organization automatic. ⛔ **THE RELATIVE/INDEXED STORE FORMAT CHANGED**: the framed store now opens with a REQUIRED §9.1.6 header (`"CBNFSTR"` magic + a format-version byte, organization, record type, minimum and maximum record size, a key count and one descriptor per key), decoded in ONE place, so **a relative or indexed file written by an EARLIER BUILD is refused '39' rather than silently misread** — the loud failure is why the header is REQUIRED and not optional. **Variable-length record-sequential files did NOT change format**, and neither did any other sequential shape: `RecordFraming`'s 4-byte record-length prefix was already in the data and always had been (§9.1.7.2's own "record length headers"), so the premise that a sidecar was needed to round-trip them was FALSE. ⚠ Two further OWNER-VISIBLE consequences: **PB193's original reproduction now opens '00'** — a RELATIVE file reopened INPUT through a SEQUENTIAL or LINE SEQUENTIAL FD, because a sequential file's format states nothing for the store to contradict (the reverse direction is still '39', and `RecordLayoutNotice` still reports on stderr) — and **'39' now precedes '37'** for an unwritable NON-STORE file opened I-O through a keyed FD, since §14.9.27.4's general rules apply in order and GR10 ends the statement. The key check ALONE has a documented runtime opt-out, **`COBOLNET_KEYCHECK=OFF`** (GnuCOBOL's `COB_KEYCHECK`; ⛔ `COBOLNET_`, never `COBOL_`, which IS `SwitchStore.Prefix` and would put it inside the §12.3.7 external-switch family), declared at `FileRegistry.KeyCheckVariable`, read once per run unit and registered in `RuntimeConfig.All`. `PB802` is filed `kind: decision` / `landed`, **`PB684` is CLOSED** because its subject — a sidecar that could not be rewritten — no longer exists, `PB193` is marked superseded in part and `PB701` is corrected. Train 23's semgrep numbers are unchanged on every rule (3/46/2/423). **Before it landing train 22 carried PB785 + PB793 onto main in ONE landing (two cluster commits plus one closing batch/DEVLOG/§0 commit, DEVLOG 1572) — GAP 2752 → 2748 with the denominator unchanged at 4,348**, no cluster dropped, and **no new diagnostic code was claimed** (COBOLNET1907's catalogue description was rewritten in place; 1912–1913 were reserved, never used, and return to the pool; the next free code is still **COBOLNET1859**). ⛔ **THE IMPLEMENTOR CODE-NAME SET IS NO LONGER EMPTY** — PB793 (owner question 22) makes `ASCII` (ISO/IEC 646 IRV, 128 ordinals, identity correspondence, native order) and `EBCDIC` (IBM CCSID 37, derived at bind time from .NET's in-box code page and never transcribed) supported as ALPHABET `code-name-1`, out of ONE table `ImplementorCodeNames` that both binder arms LOOK UP; the national set is still empty and says so from the same table. ⛔ **§13.18.13.4 CODE-SET NOW CONVERTS** — GR6 a/b had never been implemented while every non-identity set was refused, and is now a 256-entry permutation the compiler emits as a literal, applied at `NextFrame` · `EmitRecord`/`EmitRecordLine`/`OverwriteInPlace` · `RecordFraming`'s keyed payload, so a record area, a key value and a lock identity are always native. ⚠ `CONFORMANCE.md` DOC-A.1-129 was RE-DECIDED, not re-worded: the code set stays out of the validated fixed-file attributes under §14.9.27.4 GR10's latitude, no longer on the (now false) ground that its value is constant. ⛔ **`docs/rearchitecture/evidence/` IS FROZEN EVIDENCE** — PB785 (owner question 21): a claim the tree has refuted is MARKED with a `superseded_by` object, never repaired, and `scripts/spec/audit_evidence_supersession.py --check` is a THIRD static audit in `build-local.{sh,ps1}` and `battery.sh` PHASE -1 beside the two citation ones. Train 22's semgrep numbers are unchanged on every rule (3/46/2/423), and it is the first train to reach main through `scripts/push-main.sh` under the required `ci-gate` check. **Before it landing train 21 carried PB792 + PB732 + PB794 onto main in ONE landing (three cluster commits plus one closing DEVLOG/§0 commit, DEVLOG 1569) — GAP 2761 → 2752 with the denominator unchanged at 4,348**, no cluster dropped, and **COBOLNET1902 and COBOLNET1911 were claimed** (1903, 1908 and 1909 were reserved, never used, and return to the pool; the next free code is still **COBOLNET1859**). ⛔ **COBOLNET0883 AND COBOLNET1576 ARE RETIRED AND MUST NEVER BE REALLOCATED** — PB794 replaced the six per-directive malformed-operand codes with the ONE producer COBOLNET1911, driven off the new `directiveOperand` column of `constructs.json`; the retirement blocks are at `DiagnosticCatalog.cs:127` and `:1884`, the COBOLNET0875 / COBOLNET1518 precedent. ⛔ **PB732 — THE CLUSTER TRAIN 19 DROPPED — IS ON MAIN**, re-landed on top of PB792, which was the gate that had made its screen look wrong: `RW104A` now compiles at `--std 2023 --permissive` with ZERO COBOLNET1639, its `xxxxx049.txt` detail field at column 20 and that file byte-identical to the strict `--std 85` run. Train 21's semgrep numbers are unchanged on every rule (3/46/2/423). **Before it landing train 20 carried PB725 + PB712 + PB753 + PB353 + PB348 onto main in ONE landing (five cluster commits plus a batches commit and one closing DEVLOG/§0 commit, DEVLOG 1564) — GAP 2768 → 2761 with the denominator unchanged at 4,348**, no cluster dropped, and **COBOLNET1910, COBOLNET1914 and COBOLNET1915 were claimed** (1908–1909, 1911–1913 and 1916–1917 were reserved and stay free; the next free code is still **COBOLNET1859**). ⛔ **COBOLNET0875 IS RETIRED AND MUST NEVER BE REALLOCATED** — PB725 replaced >>TURN's bespoke below-2002 code with the one registry band COBOLNET0900; the retirement block is at `DiagnosticCatalog.cs:103-107`, the COBOLNET1518 precedent. Train 20's semgrep baseline moved DOWN (`cobolnet-raw-diagnostic-code-literal` 427 → 423) and was updated to lock it. **Before it landing train 19 carried PB742+PB743 + PB301 + PB751+PB752 + PB750 + PB770 onto main in ONE landing (five cluster commits plus one closing batches/DEVLOG/§0 commit, DEVLOG 1562) — GAP 2777 → 2768 with the denominator unchanged at 4,348**, and **COBOLNET1900, COBOLNET1906 and COBOLNET1907 were claimed** (1901 and 1904–1905 were reserved and stay free; the next free code is still **COBOLNET1859**). ⛔ **ONE CLUSTER WAS DROPPED — PB732** (`WHEN SET TO FALSE IS literal-4`): its report-writer arm of the VALUE literal-position screen rejects `03 VALUE "DETAIL LINE " COLUMN 20 PIC X(12).` in NIST `RW104A` — legal source, over-rejected because the VALUE clause's operand list absorbs the next clause's keyword and PB732 turned that silent swallow into COBOLNET1639. Three `VersionMatrixTests_P9/P10/P11.Cobol85Program_StillCompilesAtLaterEdition("RW104A")` legs went red; the cluster's own gate never ran them (its filter had no `~VersionMatrix` / `~Nist` term). Attributed mechanically — `ReportNonLiteralValueOperand`, the only emitter of that message, exists in no commit before PB732's. The finding is appended to `kb/Work/PB732`, which stays `open` — PB732 was dropped by train 19 and train 20 did not carry it. **Before that landing train 18 carried PB728+PB575 + PB704 + PB735 + PB740 onto main in ONE landing — the first FOUR-row train, and the second FINISHED BY A CONTINUATION LANDER after an API session limit killed the first one with every cluster committed but the gate unrun (five cluster commits — PB761's reconciliation, which the train itself forced, is the fifth — plus one closing batches/DEVLOG/§0 commit, DEVLOG 1559) — GAP 2780 → 2777 with the denominator unchanged at 4,348**, no cluster dropped, and **no diagnostic code was claimed by any of the four** (COBOLNET1892–1899 were reserved and none was needed; the next free code is still **COBOLNET1859**). ⛔ **PB740's three §9.1.15 rows landed PARTIAL, not CONFORMS, and that is a CORRECTION made at the landing:** the note's whole evidence is the SEQUENTIAL organization, while `RelativeConnector` and `IndexedConnector` hold no long-lived host handle at all — they reach the physical file only through `RecordFraming.ReadStore`/`WriteStore` over `HostFile.OpenAuxiliary`, which is fixed at `FileShare.ReadWrite`, and the CLOSE-side store is `FileMode.Create` — so the keyed organizations establish no §9.1.15 lock against another run unit and silently discard its writes on '00'. The in-run-unit half (Table 19) IS arbitrated for every organization and is what PB740 fixed; the keyed half is **kb/Work PB771**, which is why the train's GAP move is 3 and not 6. **Before it landing train 17 carried PB702+PB709 + PB714 + PB737 + PB708 + PB739 + PB695 family 3 onto main in ONE landing — the fifth six-row train, and the first FINISHED BY A CONTINUATION LANDER after an API session limit killed the first one mid-row-3 (six cluster commits + one closing DEVLOG/§0 commit, DEVLOG 1556) — GAP 2788 → 2780 with the denominator unchanged at 4,348**, no cluster dropped, and **no diagnostic code was claimed by any of the six** (COBOLNET1884–1891 were reserved and none was needed; the next free code is still **COBOLNET1859**). **kb/Work PB695 is LANDED WHOLE** — `audit_grammar_optional_words.py` reports **0 candidates** (42 → 12 → 8 → 0), re-verified on the merged tree — and PB714's decision is FILES design **D22** (D20 is PB683's, D21 PB339's). ⛔ **PB708 CHANGED THE GATE ITSELF:** every `FullyQualifiedName~` TERM is now put back to vstest before the legs run (`scripts/filter_population.py`), so a term naming no test in any assembly the invocation runs is DEAD and REFUSES the gate, and a term live only in an assembly the invocation runs UNFILTERED is INERT and stamps the verdict line — name a filter term in the assembly it filters, and keep `DisplayName~` selectors out of a `FullyQualifiedName` union. Before it **landing train 16 carried PB697 (+PB698) + PB339 + PB695 family 2 + PB338 + PB713 + PB699 onto main in ONE landing — the fourth six-row train (six cluster commits, one cross-row reconciliation commit, one closing DEVLOG/§0 commit, DEVLOG 1551) — rows 4,371 → 4,348 and GAP 2810 → 2788**, no cluster dropped, and **no diagnostic code was claimed by any of the six** (COBOLNET1872–1883 were reserved and none was needed; the next free code is still **COBOLNET1859**). ⛔ **THE ROW AND GAP MOVES ARE TWO DIFFERENT KINDS OF NUMBER AND MUST NOT BE READ AS ONE.** Thirteen of the twenty-two are PB698’s DENOMINATOR move — a **transcription repair, not progress**: the transcription had outdented lettered sub-items at printed page breaks (23 sites in 14 clauses, every ordinal re-measured on the canonical PDF; 23 of 23 print INDENTED), so restoring the printed structure folds 23 rows back into their parents and `cite.py` now answers `§14.9.41.3 6) b) 2.` where one path had stood for three items. **Nine are actual progress** (PB339 one, PB338 four, PB713 one, PB699 three). PB698 also closed a hazard the migration itself uncovered: a sub-list rule-id is POSITIONAL, so 34 ids disappeared, 11 appeared and SIX adjudicated rows kept their id while their text changed underneath — the migration was therefore done BY TEXT through batches, and `build_inventory.py` now stamps a derived `rule-digest` per row and **fails a new `--check`** when a carried verdict lands on changed text (proved to fire before it was trusted). PB339’s decision lands as FILES design **D21** (its D19 slot was taken twice in flight, by train 14’s PB692 and train 15’s PB683), PB699 FOLDS its material into PB692’s **D19** so that decision states one rule rather than two, and PB338 CORRECTED **D17** in place — its false premise that a keyed walk learns its record by reading it was the root of the defect. ⚠ **The train’s two reds were both row 3’s base predating train 15’s PB688**, which gated `ORGANIZATION IS LINE SEQUENTIAL` to COBOL-2023: family 2’s COBOL-2002 CODE-SET artefacts carried a LINE SEQUENTIAL file (repaired by taking PB695 family 3’s already-corrected golden verbatim), and a keep-both resolution of the 2014 corpus manifest re-added `pb329_line_sequential_charset_2014`, which PB688 had MOVED to 2023. The second is the general lesson and has its own commit: **a union resolution of an append-only list has to read what the other side DELETED, not only what it added**; `Manifest_CoversEveryProgram_NoOverlap` caught it. ⚠ **The gate’s remaining reds are neither this train’s nor new**: `NistDifferentialTests_P0("NC215A")` is train 14’s PB678 collating regression (**PB741**, blocking fix in flight) and the two `ExternalCorpusPopulationDriftTests` reds cannot pass in a worktree without the GPL corpus. Train-16 gate: Conformance union filter (39 terms) **4227/4227**, FULL Unit **22,564/22,566**, legacy `CobolSharp.Tests.Integration` **503 passed / 1 skipped / 0 failed**, Characterization **33/33**, NIST **351/352**, semgrep held at **427**, both citation audits at zero, `build_inventory.py --check` exit 0. **Landing train 15 carried PB691 (+PB144) + PB693 + PB703 (+PB364) + PB686 + PB683 + PB695 family 1 onto main in ONE landing — the third six-row train (six cluster commits, one closing inventory/DEVLOG/§0 commit, DEVLOG 1547) — GAP 2815 → 2810**, no cluster dropped, and **no diagnostic code was claimed by any of the six** (COBOLNET1852–1855 and 1864–1871 were reserved and none was needed; the next free code is still **COBOLNET1859**). The rows' bases ASCENDED (973460ba, 979b670c, then four at 2c8e32cb), so every row predated trains 13 and 14 and every conflict was resolved by keeping BOTH mechanisms: COBOLNET1720 now serves SIX rules (PB334's two joined by PB691's §14.9.51.3 SR2), `BoundStores` walks PB334's READ lists and PB691's WRITE/REWRITE lists, and `SequentialIoEmitter.EmitWrite`/`EmitRewrite` are PB683's unconditional governed call with PB691's §9.1.14 status snapshot after it. PB683's decision lands as FILES design **D20** (train 13's PB345 holds D18, train 14's PB692 D19). ⚠ **THE GATE'S ONE CONFORMANCE RED IS NOT THIS TRAIN'S AND IS ON MAIN NOW**: `NistDifferentialTests_P0.NistProgram_MatchesGolden("NC215A")` — `IF NINE-DU-9V0-1 < SPACE` under `PROGRAM COLLATING SEQUENCE IS THE-WILD-ONE` prints `FAIL*` where the golden requires `PASS` — bisected to **train 14's PB678** (`f798397f`, SORT/MERGE collating sequence per key class): GREEN at `c34f01ad`, RED at `f798397f`, and RED at pre-train-15 main `f86a5764` with none of train 15 applied. It is a WRONG-ANSWER regression on the program collating sequence in a plain relation condition and needs a kb/Work note from central id allocation. ⚠ `semgrep` `cobolnet-raw-diagnostic-code-literal` was re-baselined **423 → 427**, and the reason is that the baseline was BLIND: semgrep's C# front end `PartialParsing`d lines 7–467 of `ProcedureTableBuilder.cs`, and PB703's deletion of the nested primary-constructor class made the file parse, revealing four always-existing violations (zero findings sit on lines the train wrote). Recorded in `kb/Work/PB175` with the general point that a semgrep count is evidence about what the parser could READ. **Landing train 14 carried PB678 + PB350 + PB692 + PB346 + PB676 + PB688 onto main in ONE landing — the second six-row train (six cluster commits, one closing inventory/DEVLOG/§0 commit, DEVLOG 1544) — GAP 2820 → 2815**, no cluster dropped, and the **FULL Unit assembly** was gated rather than the union filter, which is train 13's lesson applied. **Its only diagnostic codes are PB350's COBOLNET1850 (`format-required-phrase-omitted`) and PB692's COBOLNET1858 (`access-mode-not-sequential-on-sequential-file`); COBOLNET1842–1843, 1851, 1856–1857 and 1859–1863 were reserved for the train's other clusters and none was needed.** Two cross-row collisions were decided on landing rather than by taking a side: PB692's FILE-CONTROL-entry decision lands as FILES design **D19** because train 13's PB345 took D18, and PB692's `ORGANIZATION IS LINE SEQUENTIAL` goldens became **2023** programs — PB688 dated the organization to 2023 from the Foreword's main-changes list, so below 2023 that source is rejected by COBOLNET0900 and §12.4.5.5.2 SR2 never gets to speak. ⚠ **The gate's ONE red was the same edition seam reaching a golden PB688's sweep could not see**: train 13's `2002/pb352_start_sequential_first_last` carries a LINE SEQUENTIAL connector and landed after PB688's base. It was SPLIT rather than moved — the 2002 program keeps the record-sequential legs, because START FIRST/LAST is a 2002 introduction and that program exists to pin §14.9.41.4 GR20/GR21 at the OLDEST edition that has it, and the line-sequential legs became `2023/pb352_start_sequential_first_last_line`, whose `.out` was proven load-bearing by poisoning it. The next free diagnostic code is **COBOLNET1859**. **Landing train 13 carried PB334 + PB345 + PB332 + PB352 (with PB354) + PB341 (with PB342) + PB364 onto main in ONE landing — the FIRST SIX-ROW train, the band's top (six cluster commits, one closing inventory/DIAGNOSTICS/DEVLOG/§0 commit, DEVLOG 1537) — GAP 2853 → 2820**, no cluster dropped; ⚠ **it was finished by a CONTINUATION lander after an API session limit killed the first one mid-landing**, and the continuation CONFIRMED rather than re-applied all six verdict batches (every dry run reported `rows changed: 0` and the rebuilt inventory was byte-identical). ⚠ **The gate's ONE red is a filter finding, not a cluster's design defect, and it is worth the owner's eye as a GATE lesson:** `HostFileProbeDriftTests` is in no cluster's test names and so in no union filter, and only the FULL Unit leg ran it — it named PB334's new `RecordFraming.FrameStarts` calling `File.Exists`, which PB323's determination forbids outside `HostFile.Probe`. The root was one level above the guard's own remedy: `FileSupport`'s contract asks the host once per OPEN in `FileConnector.Open` and never in an organization body, and `FrameStarts` is only reached from a backward READ on an already-open file, so it now takes the connector's own seekable STREAM (`FrameStarts(Stream)`) and asks the host nothing — no second handle under a deny-share mode, and no way to index a file other than the one being read. **A filtered Unit leg would have shipped it; a landing train should run the FULL Unit project, not the union filter, on the Unit assembly.** **The only diagnostic codes claimed are PB345's COBOLNET1836 (`record-less-fd-no-record-size`) and COBOLNET1837 (`file-description-record-required`); COBOLNET1822–1823, 1834–1835, 1838–1841 and 1844–1849 were reserved for this train's other clusters and none was needed, and the lowest unclaimed code is still COBOLNET1779.** PB334 gave the SEQUENTIAL READ arm the three sub-rules its binder never consumed — `readDirection`, `readKey` and `readInvalidKey` had no member on `BoundRead`, so `READ … PREVIOUS` bound as a FORWARD read at every edition, §14.9.30.3 SR10's KEY error fired only on the keyed arm, a NOT INVALID KEY block was compiled away, the COBOLNET0900 gate never fired and SR7 could not be enforced at all — over one `ReadKind` on BOTH bound nodes through a new `IBoundRead`, collapsing `VersionConformancePass.GateStatement`'s two arms to ONE, and reconciled on landing with train 12's PB340 so there is still exactly one governed Format-1 read. PB345 made §14.9.30.4 GR6's implied record area REAL at bind time (`DataBinder.MaterializeImpliedRecord`, an unnamed level-01 GROUP over one FILLER), so a record-less FD — legal on the sequential and relative organizations per §13.4.5.3 SR3, withdrawn only for INDEXED by SR7 — registers a connector instead of aborting the run unit with "a compiler defect", with COBOLNET1836/1837 as its two screens and the decision recorded as FILES design **D18** (renumbered on landing off train 12's D17). PB332 repaired `audit_grammar_optional_words.py`, a gate that had located ZERO general formats since de-paging and exited 0 over 596 rules, and then fixed the grammar only from what the repaired tool found (`startWithLength : WITH? LENGTH`, `useStatement`'s three formats, and the INVERSE finding that `startKeyPhrase`'s relational operator was OPTIONAL where the printed page brackets nothing) — 320 formats declared / located / measured, non-zero exit on a shortfall. PB352 deleted `KeyedIoBinder.BindStart`'s early return for every sequential file — which cited §14.9.41.3 SR2 as the reason for refusing what SR2 permits, killing the run unit on a .NET stack trace — with `SequentialConnector.StartFirstLast` written over the ONE framing walk `NextFrame` extracted out of `Read`, and §14.9.41.4 GR1's '47' written five times replaced by `FileConnector.StartOpenModeGuard`; PB354 split `RecordLayout.KeyIndexByPosition`, one method answering two different rules, into `KeyIndexOfKeyItem` and `GenericKeyIndex`. PB341 made the release ordinal a VECTOR (`KeyedRec.Ordinals`) where one `Arrival` had served every alternate key's duplicate ordering — a REWRITE touching one key silently reordered every other key's sequence and `CloseCore` baked it into the physical file — and PB342 made the file position indicator a KEY VALUE alone per §14.9.41.4 GR17 e) 1., where the pair (key, arrival) had lost two records in BOTH directions after a forward `START` into a duplicate set. PB364 replaced four hand-rolled `HashSet`s implementing FOUR INSTANCES OF ONE SENTENCE (§14.9.49.3 SR7/SR8/SR9/SR14) with one `UseOperandRegister<TKey>` keyed on the STATEMENT ordinal, retiring three over-rejections of legal COBOL. ⚠ **One train-level remedy worth the owner's eye:** the train's new emit sites pushed semgrep's `cobolnet-raw-diagnostic-code-literal` above its 440 baseline, and the remedy taken was to CONVERT rather than to adjudicate — COBOLNET0862 and COBOLNET0863 were given catalogue descriptors (`IoStatementOperandRule`, `FileKeyClauseRule`) and all twenty raw-literal emit sites converted, so the baseline moved DOWN to **423** (`kb/Work/PB175` carries the attribution). Before it **the denominator was corrected, alone, on 2026-09-05 (PB689) — the rise from 2794 to 2853 is a DENOMINATOR CORRECTION, not a regression:** `extract_rule_catalog.py` closed a clause's rule block at ANY markdown heading while `specs/ISO_COBOL.md` rendered nine of the standard's in-clause group labels (`SEQUENTIAL FILES`, `FORMAT n` — printed in body type, verified in the PDF text layer on pp146/463/609/680/820) as headings, so five clauses were short by SIXTY normative rules — §14.9.51.4 (WRITE) held 16 of its 42 general rules, GR23's line-sequential `'71'` among the missing — and every GAP figure published before this commit was measured against a total that flattered the project by that much; the transcription is repaired to its own plain-line convention, the group label is now its own line kind in the extractor (so a future heading-ified label truncates nothing and is reported), `CatalogCoverageDriftTests` counts the printed ordinals against the catalog per clause on every build, and `GR-14.9.51.4-23` was re-attributed to PB329's landed mechanism (2794 → 2854 → 2853). Before it **landing train 12 carried PB340 (+PB335) + PB328 + PB673 + PB329 onto main in ONE landing (four cluster commits, one inventory/DIAGNOSTICS/DEVLOG/§0 commit, DEVLOG 1525) — GAP 2802 → 2794**, no cluster dropped and no red that had to be attributed to a cluster. **NO diagnostic code was claimed — COBOLNET1824–1825, COBOLNET1828–1829, COBOLNET1830–1831 and COBOLNET1832–1833 were reserved for this train’s clusters and none was needed; `docs/DIAGNOSTICS.md` regenerates BYTE-IDENTICAL and the lowest unclaimed code is still COBOLNET1779.** All four clusters are the file I-O subsystem. PB340 re-split record-lock governance on a READ by FORMAT rather than by ORGANIZATION (design decision **D17** in `docs/COBOLNET_FILES_DESIGN.md`): one governed Format-1 read serves sequential, relative and indexed and owns §14.9.30.4 GR22’s skip-scan, where a relative or indexed `READ … ADVANCING ON LOCK` had answered ’51’ — the one status GR22 rules out — because the bound phrase was read by no emitter at all; §14.9.30.3 SR9’s implied-NEXT test came with it (**PB335 flipped to landed**) and `BoundIoPhraseConsumptionDriftTests` now fails whenever any phrase property of a bound I-O node goes unread by its emitter. PB328 gave the OPEN write-capability question ONE home, `HostFile.PermitsWrite` called once from `FileConnector.Open`, where the same read-only file had answered ’37’ sequentially and ’00’ keyed and the ’00’ was believed through READ and REWRITE to a ’30’ at CLOSE (§14.9.27.4 GR16 names no organization and Table 20 is why it cannot). PB673 deleted the connector-held ASSIGN … USING and LINAGE closures rather than guarding them — every OPEN and sequential WRITE entry now takes the executing element’s own operands with NO DEFAULTS, so a forgetful emitter path is a compile error — fixing an EXTERNAL connector that wrote into the LAST activator’s file at ’00’ (§12.4.5.3 GR3 b) and a recursive unit that read a returned activation’s LOCAL-STORAGE, and catching a two-arm dispatch where a FACTORY/OBJECT LINAGE FD had no page model at all; `docs/CONFORMANCE.md` gains the missing REQUIRED **DOC-A.1-72**. PB329 made the LINE SEQUENTIAL character-set determination the four rules that delegate to it had never had (**DOC-A.1-115**: U+0020 and above are members, the C0 controls are not, DEL and the Latin-1 supplement are), giving ’09’ its first producer and GR23’s WRITE arm its first existence over ONE `LineSequentialCharacterSet` and five call sites. ⚠ **Two train-level findings worth the owner’s eye.** (1) PB673’s no-defaults signature caught TEN call sites the earlier rows of the same train had introduced — the guard worked exactly as designed, and each was fixed by passing the operands, never by adding a default. (2) PB328 had reconciled PB323’s inventory row `GR-14.9.27.4-3` to its renamed drift-test class by editing the inventory DIRECTLY on its branch instead of through a verdict batch, which a landing that re-applies the BATCHES would have silently lost, leaving a dangling `test-ref`; it is landed here as a real batch and the per-cluster inventory delta now matches the four branches’ own deltas exactly. Before it **landing train 11 carried PB327 + PB331 + PB336 + PB347 onto main in ONE landing (four cluster commits, one inventory/DEVLOG/§0 commit, DEVLOG 1519) — GAP 2810 → 2802**, no cluster dropped and no red that had to be attributed to a cluster. **The only diagnostic code claimed is PB331’s COBOLNET1818 (`read-ignoring-with-lock`); COBOLNET1816–1817, COBOLNET1819–1821 and COBOLNET1826–1827 were reserved for this train’s clusters and none was needed, and the lowest unclaimed code is still COBOLNET1779.** PB327 made the FD/SD record area’s image its BYTES — NATIONAL was the last leaf kind shipping its carrier instead of its storage — which released three legal record shapes the one gate `GateNationalRecords` had refused, folded that gate into `ByteWindowResidueOf` (where PB231’s pointer third has already made it vacuous), put §14.9.30.4 GR15’s national space in `FitRecord`/`TrimRecordEnd` and scaled §14.9.41.4 GR13’s START LENGTH from national positions to bytes; PB331 split the READ statement’s TWO printed lock brackets — measured off the rendered page 722, where the transcription was already right and the GRAMMAR had drifted — one grammar object per bracket and two facts (contention, retention) from the parse tree through to `FileRegistry`, landing §14.9.30.3 SR3’s first real check with it and taking IGNORING LOCK off WRITE and REWRITE; PB336 lifted §14.9.30.4 GR21’s ’46’ above the absent-OPTIONAL arm that ARMS it and extracted the whole READ precondition chain into `FileConnector`, in the standard’s own order, once for all three organizations; PB347 deleted `SequentialIoBinder.FileOfRecord` — which answered containment where WRITE, REWRITE and RELEASE ask IDENTITY — and gave `record-name-1` one home in `StatementValidation.ResolveRecordName`, the containment walk surviving only as the repair message. ⛔ **One OWNER-VISIBLE item:** PB327 TOOK an owner-reserved determination rather than blocking on it — an ALPHANUMERIC group containing a national leaf now MOVEs, compares and DISPLAYs as its BYTES (§14.9.11.4 GR1/GR7 leave both readings legal; the one taken is what §13.18.29.4 GR2’s own NOTE predicts, and its remedy, GROUP-USAGE NATIONAL, is what the new `AsNat()` face implements). It is recorded on `kb/Work/PB546`, in D-N5 (now DISCHARGED) and in `docs/CONFORMANCE.md` DOC-A.1-57, whose “one sanctioned width divergence” is gone; reversing it is one arm in `PlaceRenderer.GroupImage` plus two goldens. Before it **landing train 10 carried PB323 + PB324 + PB325 + PB326 onto main in ONE landing (four cluster commits, one inventory/DIAGNOSTICS/prose commit, DEVLOG 1514) — GAP 2821 → 2810**, no cluster dropped. **The only diagnostic codes claimed are PB324’s COBOLNET1810 (`assign-using-not-alphanumeric`) and COBOLNET1811 (`assign-using-in-own-record`); the lowest unclaimed code is still COBOLNET1779.** PB323 gave the OPEN presence decision ONE probe with THREE answers (§14.9.27.4 GR3’s '37' for a present-but-refused file, where `File.Exists` had answered '35' — or '05' plus an invented empty file for an OPTIONAL one — on all three organizations); PB324 implemented §12.4.5.3 GR3 whole, so `ASSIGN USING data-name-1` is bound and the connector is associated at every OPEN, SORT and MERGE through the ONE writer `FileConnector.Associate`, called once from PB321’s unified `FileRegistry.OpenCore` before the Table-19 arbitration and outside the retry loop (design decision **D15** in `docs/COBOLNET_FILES_DESIGN.md`); PB325 extracted the keyed connectors’ shared branch fact into a new `KeyedConnector` base and made the OPEN MODE a SCREEN rather than a SELECTOR, closing four (organization × access) Table-20 cells at once; PB326 gave EC-FLOW-REPORT, EC-REPORT-FILE-MODE, EC-REPORT-ACTIVE and EC-REPORT-INACTIVE their raise sites over one run-unit-scoped `ReportFlowState` and one new `OpenModeIfOpen` accessor. ⛔ **One OWNER-VISIBLE item:** PB324 determines that `DELETE FILE` on a bare `ASSIGN USING` connector that has not yet been associated takes §14.9.10.4 GR14’s '05' rather than deleting the registration-default `<file-name>.txt` the program never named — re-resolving data-name-1 there would be an implementor extension to GR3’s closed list of associating statements, on the one verb where guessing wrong destroys data (`docs/CONFORMANCE.md` §7, DOC-A.1-73). Before it **landing train 9 carried PB231 (its POINTER third — the note CLOSES) + PB319 + PB317 + PB321 onto main in ONE landing (four cluster commits, one inventory/prose commit, DEVLOG 1510) — GAP 2830 → 2821**, no cluster dropped. **The diagnostic codes claimed are PB231’s COBOLNET1796 (`external-pointer-or-object-item`) and COBOLNET1797 (`based-subject-of-class-object`) and PB317’s COBOLNET1802 (`open-norewind-organization`) and COBOLNET1803 (`open-norewind-open-mode`); the lowest unclaimed code is still COBOLNET1779.** PB231’s pointer third gave `StorageCell` MANAGED SLOTS keyed by byte offset beside its byte image, so `ByteWindowResidueOf` now has NO residue and `kb/Work/PB231.md` CLOSES after three thirds. The other three are one statement from three sides: PB319 deleted §14.9.27.3 SR8’s SELECT-side copy and implemented its never-implemented APPLY COMMIT antecedent; PB317 bound `OPEN … WITH NO REWIND` per file-name onto PB316’s `BoundOpenFile` and gave it ONE runtime effect site; PB321 made §14.9.27.4 Table 19 a 35-cell structure that arbitrates EVERY open. ⛔ **THREE OWNER-VISIBLE ITEMS.** (1) PB317 determines that an absent OPTIONAL input file opened WITH NO REWIND reports **‘07’, not ‘05’** (§14.9.27.4 GR11 is an explicit assignment; ‘05’ is a §9.1.13.2 description and GR17 excludes the phrase’s modes), recorded in `docs/CONFORMANCE.md` §3. (2) `GR-14.9.27.4-12` is CONFORMS-but-untested because its witness is a **§8 DERIVATION row that is DRAFTED AND AWAITING THE OWNER’S SIGNATURE** — GR12 governs media that permit rewinding and no supported medium is one. (3) PB321 leaves PB322’s implementor default UNDECIDED at the one `FileRegistry.ImplementorDefaultSharing` null seam, and its universal arbitration makes **two clause-less connectors on one physical file answer ‘61’ at COBOL-85** (PB670, an owner determination). ⚠ The train re-trued ONE golden it had itself invalidated: train 8’s `2002/pb316_open_group_scope` read `REF-A=00 / ONE-A=00` because a clause-less connector was invisible to arbitration; §9.1.15 1) makes ‘61’ the conforming answer, GR20’s two written forms still agree, and the `.cob` records the discrimination that pair LOSES. Before it **landing train 8 carried PB647 + PB300 + PB305 + PB303 + PB316 onto main in ONE landing (five cluster commits, one inventory commit and the prose, DEVLOG 1504) — GAP 2845 → 2830**, no cluster dropped. **The only diagnostic codes claimed are PB303’s COBOLNET1794 (`externalized-name-literal`) and COBOLNET1795 (`externalized-name-contained`)** — the band now has holes from COBOLNET1779 to COBOLNET1793 and the lowest unclaimed code is still **COBOLNET1779** (COBOLNET1790–1793 and COBOLNET1796–1801 were reserved for this train’s other four clusters and none was needed). ⛔ **Two OWNER-VISIBLE items.** (1) PB647 makes the **ROUNDED phrase effective on the whole §15.4.1 floating-point intrinsic family, where it had been a NO-OP**, and moves the float→fixed landing to the STATEMENT’s rounding mode at the resultant identifier’s scale (§14.7.4.1 / §14.7.4.3 r2, which IS §14.6.8.2 r4’s MOVE landing), so `COMPUTE S = FUNCTION SQRT(3)` and `MOVE FUNCTION SQRT(3) TO S` now agree as §15.4.1 requires; native `**` carried the identical defect and is fixed with it, §14.7.4.3 r7’s PROHIBITED raise moved into `CobolIntrinsics.FromDouble`, and the whole corpus moved exactly ONE golden in two lines (`2023/pb65_codomain_clamp`, re-derived from the exact binary64 expansions and given six new ROUNDED legs, because truncation could never leave a codomain the double is already inside — PB65’s clamp had been held open by a green test that no longer exercised it). (2) PB300 implemented the removed `PADDING CHARACTER` clause’s edition gate (`padding-character-removed-2014`, COBOLNET0902) **where `docs/CONFORMANCE.md` carried no row and so no owner determination existed** — implemented to its edition window rather than declined, with the removal EDGE derived (the repo holds no 2002/2014 text) and the derivation stated in VCR row 7.20 so it can be overturned in one line; its second arm retired the hand-maintained reservation-gating list that had already rotted, generating `reservedGatedWord` from `cobol-words.json` instead. PB305 corrected the cross-argument class screen to PROJECT through §8.5.2.1 Table 2 before merging — **GAP delta ZERO, which is the point: eleven of the twelve rows that rule governs already read CONFORMS while the compiler refused legal source under them**, so it removed a false conformance claim rather than closing a gap. PB303 gave the ISO `AS literal` externalized-name phrase a surface on all five identification paragraphs through one grammar rule, one edition gate and one literal screen, splitting every entity into a declared and an externalized name; ⛔ **its note had framed MODULE-NAME’s form as an OWNER QUESTION and it was not one — `docs/CONFORMANCE.md` DOC-A.1-135 had already chosen the program-id form, so the split makes the compiler MATCH a standing determination and no owner prompt was raised.** PB316 made §14.9.27.2’s REPEATED GROUP the bound tree’s shape (`BoundOpenFile` per file-name; `BoundOpen` carries nothing but the list), fixing both a sharing-phrase leak across groups and a forward-only bind rejection of legal source; `GR-14.9.27.4-23` moved to PB322 rather than being orphaned. Before it **landing train 7 carried PB231’s NATIONAL third + PB623 (with PB90) + PB297 + PB639 + PB635 onto main in ONE landing (five cluster commits plus the prose, DEVLOG 1498) — GAP 2850 → 2845**, no cluster dropped and NO diagnostic code claimed (COBOLNET1780–1789 all stay free; next free is still COBOLNET1779). PB231’s NATIONAL third discharged RESIDUE-11 on the byte-window channel by giving the storage extent ONE authority (§13.18.44.4 GR1 / §14.9.3.4 GR3 count bits and bytes where the class machinery read `ImageWidth`, and NATIONAL is the one leaf kind where those differ) — `kb/Work/PB231.md` STAYS `open` for the pointer-class third, and its three rows re-record PARTIAL at GAP 0; PB623 replaced FOUR copies of `v * Pow10.AsDouble(scale)` with one exact landing `CobolFloat.TryExactScaled` and ANSWERED PB90 by derivation (§14.6.8.2 rule 1 has no latitude), which is also ⛔ **an OWNER-VISIBLE VALUE CHANGE: `MOVE <a COMP-2 holding 8.2> TO PIC 9V9` now stores 8.1** — the double nearest 8.2 is 8.19999999999999928945726423989981… and §14.6.8.2 rule 4 truncates (`ROUNDED` still gives 8.2), forced by rule 1 for every standard-float sender and taken for COMP-2 too because `COMPUTE r = c2` lands through a rule-1 intermediate; PB297 deleted `ConditionRenderer.AnchorWidth` (three wrong arms and a fourth that could not be written, since `X(I:L)` is a runtime count) for the one §8.3.3.6.4 GR2 sizing `CobolString.FigToWidth`; PB639 made `CobolNum.WideningFits` the ONE widening predicate and split the unchecked store on the receiver’s capacity discipline (decimal low-order digits vs the container’s two’s-complement residue — the note’s fix applied blindly would have BROKEN COMP-5); PB635 derived the float lane’s precondition from the whole §15.3 argument run, which was crashing TEN functions with Roslyn CS0117 under `--permissive`. ⚠ The train also reconciled two things no single cluster owned: `RuntimeApiGuardTests`’ ratchet stood ABOVE its true counts in two files the train does not touch (`Emit/NumericRenderer.cs` 17 → 16, `Emit/OperandText.cs` 7 → 4, both ratcheted DOWN to the measured value), and `semgrep/verify.py` went UP on `cobolnet-no-biginteger` 36 → 46 — all ten in `CobolFloat.cs`, adjudicated as a recorded decision (the rule’s own past-the-carrier escape clause, and the file already held seven accepted occurrences inside the 36) and re-baselined, with the adjudication written into `kb/Work/PB175`. Before it **landing train 6 carried PB253 + PB254 + PB288 + PB292 onto main in ONE landing (four cluster commits, one cross-cluster reconciliation, the inventory and the prose, DEVLOG 1491) — GAP 2864 → 2850**, no cluster dropped. PB253 made the arithmetic-mode-before-receiver-shape ordering STRUCTURAL in `IntrinsicRenderer` (one method became three; `RenderFloatNative` cannot see the mode), moving the whole ACOS/ASIN/ATAN/COS/SIN/TAN/LOG/LOG10/RANDOM residue onto §15.4.1’s SDIDI container at once; PB254 took the two TEST- date validators and FIND-STRING argument-3 out of PB22’s PARTIAL-function narrowing, deriving the intake width from the runtime body’s declared carrier so the next total function is one act; PB288 made `CobolArgAdapt.Land` the ONE numeric landing of the CALL ABI on §14.2.3 GR9/GR10’s own reading (the crossing IS a COMPUTE without ROUNDED, so DOC-A.1-70’s low-order digits govern — the note’s proposed raising widening was wrong); PB292 gave the declined RECORD DELIMITER clause its mandatory §4.2.6 ¶3 warning on BOTH arms (**COBOLNET1778**, the only code this train claimed — **next free is COBOLNET1779**) and resolved all EIGHT of §2’s placeholder citations, banning the shape mechanically. ⛔ **PB292 raises an OWNER QUESTION this landing deliberately does not answer:** should COBOL.NET ever PROVIDE the RECORD DELIMITER facility — the STANDARD-1 arm’s ISO/IEC 1001:2012 7.1.2 tape framing (§12.4.5.11.4 GR3, a standard not in `specs/`) or a feature-name of its own under Annex A.1 item 150 — rather than keeping the documented decline, which has NO owner decision behind it (`git log -S` puts §2 row 26 in the document’s creating commit)? The warning is required under either answer. ⚠ The train’s one red was a cross-cluster reconciliation, not a cluster defect: PB254’s golden was cut before train 5 landed PB248, whose §15.3 type-6 reading REJECTS a floating-point operand at an integer argument position, so its two float legs left the strict corpus and the float lane’s witness moved to `FloatIntegerArgumentPermissiveTests` — the home PB248 itself created for PB21 and PB2. Before it **golden-lane round 4 landed 19 spec-derived goldens (24 files — 20 positive across `2023`/`2002`/`85` and 4 negative) closing 19 traceability rows — GAP 2883 → 2864** (DEVLOG 1486): 23 rows drawn over three writer slugs, 19 new-golden and 4 not-closable, the refuter overturning 7 of the 23 and the fixer refusing none. ⛔ **The CONFORMS-but-untested population is now FOUR rows and every one of them is an OWNER DECISION, not a missing test** — measured on the landed tree, exactly four inventory rows carry verdict `CONFORMS` with an empty `test-ref` and no `derivation`: `GR-14.9.20.4-9` and `GR-14.9.28.4-2` wait on the owner's signature of a `docs/CONFORMANCE.md` §8 derivation row on PB386's mechanical `undefined-A.2` arm (the two DRAFT rows, signature cell blank, are held OUT of the tree in the round's scratchpad), and `GR-13.18.52.4-4`/`-5` wait on `DOC-A.1-177` and `DOC-A.1-178`, which §7's A.1 table does not carry (it jumps 171 → 179) — adding an A.1 row is an owner decision, so none was added and no golden was written that would pin an undocumented determination (`PB160`, recorded in `kb/Work/PB370`). Before it **landing train 5 carried PB248 + PB250 + PB251 + PB252 onto main in ONE landing (four cluster commits plus the prose, DEVLOG 1484) — GAP 2893 → 2883**, no cluster dropped and no new diagnostic code claimed. PB248 gave ISO §15.3 type 6's integer-argument screen its floating-point arm in all four places, over the ONE new primitive `PicInfo.IsIntegerDescription` and its five readers — ⛔ **an owner-visible reversal: the strict verdict for a floating-point operand at an integer argument position changed ACCEPT → REJECT, retiring the landed golden `2023/pb21_float_arg_integer_family` (its real subject, two `--permissive`-reachable Roslyn crashes, moved to `FloatIntegerArgumentPermissiveTests`) and narrowing `2023/pb2_float_argument_exact_family`'s MOD pair to INTEGER operands per §15.64.3 r1**; PB250 gave `>>COBOL-WORDS` its second mechanism, so the directive reaches all 552 §8.9 ∪ §8.10 words rather than the 447 the lexer tokenizes, through the ONE `CobolWordsMap.Resolve` reading of §7.3.10.4 GR2/GR3/GR4 plus a generated §8.10 table answering SR3/SR4; PB251 put NUMVAL and NUMVAL-C on the SDIDI carrier in EVERY arithmetic mode (§15.4.1's latitude is withdrawn by §15.67.4 r1 / §15.68.4 r1) and deleted the three now-unreachable Int128 projections; PB252 stopped the exact Int128 carrier wrapping in SUM, RANGE and MEAN over one `SizeEscape` raise site and replaced the always-SDIDI name chain with the derived `CrossAlignedNativeArms` ∪ `StandardValueFixedByRule`, adding MOD/REM. All four notes flip to `landed`; the two clusters that overlapped (PB251 × PB252, measured before the train ran) were resolved to ONE spelling of the routing decision. Before it **landing train 4 carried PB233 + PB598 + PB237 + PB238 onto main in ONE landing (four cluster commits plus the prose, DEVLOG 1478) — GAP 2906 → 2893**, no cluster dropped, and three of the four clusters are the same statement: PB233 split the CALL's single `catch (CobolCallException)` into §14.9.4.4 GR3h/GR3i's three routing facts on an explicit activation boundary (`ProgramTable.CallProgram` marks `ControlTransferred`; GR3e's external check hoisted to the default-no-op `ICobolProgram.DescribeExternals()`); PB237 landed the program-prototype registry whole — §12.3.8.2's `PROGRAM program-prototype-name-1 [AS literal-3]` specifier, gated 2002+ as `repository-program-2002`, feeding ONE `ResolvePrototype` at both the CALL and CANCEL reference sites, with `NestedCalleeSignature` → `CalleeSignature` carrying seven §14.8.2/§14.8.3 checks onto prototype calls in one assignment (COBOLNET1760/1761 new, the 0899 `CallAsPrototypeName` descriptor DELETED); PB238 made a Format-2 CALL argument classify ONCE (`Gr8Classify`) and cross on its own carrier, bringing the FLOAT lane into the CALL ABI and modelling §14.9.4.3 SR20's object-property carve-out (COBOLNET1762). PB598 is the fourth: §13.18.63.3's SIZE sentences stopped binding a condition-name, replacing an `int? sizePositions` + `bool groupSubject` PAIR with one `ValueSubject` descriptor and three factories. All four notes flip to `landed`; `SR-14.9.4.3-14` moved from PB237 to `PB609`, and the cross-cluster `GR-14.9.4.4-9` — PARTIAL on PB238's tree for want of a prototype formal — was re-verdicted CONFORMS on the merged tree with both goldens as its witness. The GnuCOBOL differential ended at **ZERO flips**: `syn_misc:3951` ("use of program-prototype-names") moved WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT on PB237's specifier and was rebaselined as spec-licensed (§12.3.8.4 GR10 c)), and `syn_misc:388` is back on its committed row — the debt battery #45 recorded against PB598 is retired. Before it **landing train 3 carried PB231 + PB485 + PB230 + PB236 + PB235 onto main in ONE landing (five cluster commits plus the prose, DEVLOG 1474) — GAP 2925 → 2906**, no cluster dropped: PB231 collapsed the byte-window carriage gate to one predicate (`DataBinder.ByteWindowResidueOf`) and stays OPEN on its NATIONAL and pointer-class thirds; PB485 gave the level-number a domain on BOTH axes (§13.18.33.3's four per-section sets as COBOLNET1746, §13.18.33.4 GR2 / §13.16.3's entry formats as COBOLNET1747); PB230 wired §14.6.13.2 rules 1 and 2 with `SendingRef` as the per-rule exemption structure; PB236 split `BoundUnsupported`'s three jobs, announcing every remaining deferral at the one funnel (COBOLNET1756, a warning) and moving the operand rules to bind time (COBOLNET1757); PB235 made CLOSE's post-close state model Table 14 itself and discharged Annex A.1 item 24. PB230 and PB236 flipped to `landed` with their residue rows moved to `PB589`, `PB601` and `PB602`. The GnuCOBOL differential ended at ONE flip (`syn_misc:388`, owed to `PB598`): PB485's five rows held, battery #44's pair returned to its committed rows, and PB236's two new tightenings (`syn_move:374`, `run_misc:3544`) were verified against the corpus source and the spec before being rebaselined. Before it **golden-lane round 3 landed 59 spec-derived goldens (43 positive + 16 negative) closing 44 traceability rows — GAP 2969 → 2925**: six writer reports over 48 rows, 43 CONFORMS rows closed and the Annex A.1 witness `DOC-A.1-127` (`conformance:2023/pb373_object_computer_name_inert`, §7 line 1003's `Pinned by` now filled); three rows stay not-closable on owner decisions and `DOC-A.1-206`'s witness is held OUT of the tree behind `kb/Work/PB592` (its §7 determination denies a range the compiler provides). Before it **landing train 2 carried PB473 + PB204 + PB205 + PB206 + PB207 + PB208 onto main in ONE landing (six cluster commits plus the prose, DEVLOG 1468) — GAP 2992 → 2969**, 26 rows changed of which 8 were re-adjudications of rows the batch-3 registrar had recorded PARTIAL with "closes when this note lands". Its three §13.18.63 VALUE clusters MERGED into one screen rather than three: PB206's `sizePositions` now measures PB208's format-2 occurrence literals (a size screen neither cluster delivered alone, pinned by `ValueFormat2Tests.Format1AndFormat2_ScreenTheSameLiteralAlike`), and PB207's DELETION of COBOLNET0899 `bit-group-level-value` won over PB206's reordering of the same refusal. PB473's fix is in, and `IF141A` is a MATCH through both guards on the landed tree. Before it **landing train 1 carried PB203 + PB201 + PB470 + PB193 onto main in ONE landing (four cluster commits plus the prose, DEVLOG 1460) — GAP 3013 → 3005**, PB201's four rows and PB193's four being disjoint. Before it the
