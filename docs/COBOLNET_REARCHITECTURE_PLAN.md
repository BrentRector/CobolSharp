...
> **PHASES 00–01 ✅ DONE; PHASE 02 IN PROGRESS — steps 1–6 of 11 DONE** (2026-07-07; §6 owner decisions ALL RESOLVED
...
> protocol, §4 phase index, and per-phase exit criteria are the resumable record). **P2 (Editions leaf + diagnostic
> registry) steps 1–6/11 landed across 8 green commits (DEVLOG 670–677, `62e09db1`→`4990fae7`, all pushed; recon
> `wf_9944fe61-fcc`):** the edition machinery now lives in the new lowest-layer `Cobol.Net.Editions` assembly (referenced by
> both Frontend + Compiler), is **single-sourced from `tests/version-matrix/constructs.json`** (registry / `Constructs.*` ids /
> `GateId` enum generated by `scripts/gen-constructs.ps1` + drift-guarded — the committed-`.g.cs` reserved-words discipline,
> not MSBuild), `EditionContext` is a byte-stable adapter over an immutable `EditionInfo` + `IDiagnosticSink`,
> `ConstructRegistry.Check` is layer-neutral + sink-based, the ONE `EditionSeverityPolicy` is the sole strict/permissive
> decision, and every binder gate (incl. the 5 formerly-inline 08xx/0882 gates) routes through the registry funnel + enters
> the version matrix (0816 END-ACCEPT folded-but-`pending` — the grammar has no `END_ACCEPT`, so that gate is unreachable).
> Current baseline: **2055 greenfield conformance + 224 unit GREEN; FULL legacy guard NIST 353 MATCH.**
> **RESUME AT: Phase 02 step 7** — forward `{Gate(edition, GateId)}?` predicate stamping replacing `EditionGateHints`
> (grammar-touching → FULL legacy guard per `.g4` fragment; ANTLR speculative-eval risk; `EditionGateDiagnosticTests`
> re-baselines because the parse-layer 0900 message FORMAT differs; `GateId` 55 members already generated) — see the five
> concrete gotchas in `docs/rearchitecture/PHASE-02-editions-assembly-diagnostic-registry.md`'s STATUS block.
