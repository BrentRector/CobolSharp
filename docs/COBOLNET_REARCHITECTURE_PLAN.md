...
- **▶ LIVE STATE (2026-09-13 — TRAIN 35 IS ON MAIN):** **the P14 traceability inventory is 4,348 rows at GAP 2456.** **Landing train 35 carried PB405+PB414 + PB416 + PB426+PB844 + PB482 + PB368 onto main in ONE landing** — five cluster commits plus one closing DEVLOG/§0 commit (DEVLOG 1608) — **GAP 2468 → 2456 with the denominator unchanged at 4,348**, 12 verdict records over five batch files re-applied on the MERGED tree in manifest order. All five clusters landed; none was dropped. Rows 1, 2, 3 and 5 were based on `a2255819` / `b929f96e` — before trains 33 AND 34 — and row 4 on `d80cb676`, so every cluster was three-way merged onto train 34's head. **ONE conflict, in `DiagnosticCatalog.cs`**: trains 33/34's COBOLNET2012–2026 block against PB416's COBOLNET2030/2031 block, resolved KEEP-BOTH because each side is a whole set of descriptor declarations. ⛔ **No corpus `manifest.json` and no `traceability-inventory.json` hunk was three-way merged at all** — a conflict INSIDE one JSON list element is silently lossy (train 33 lost a `cobol-words.json` row exactly that way), so every golden was inserted into its manifest by a script that parses the JSON, asserts the `enabled` element COUNT before and after and asserts that no existing element moved (2002 272 → 276, 85 81 → 85, negative 1084 → 1098), and all five verdict batches were re-applied from their batch files. `docs/DIAGNOSTICS.md` was REGENERATED from the catalogue by `gen-diagnostics-doc.ps1`, never hand-merged. The one real intra-train overlap — PB405's transfer rewrite against PB368's `DispatchEmitter`/`EcEmitter` hunks — merged clean and both sides were verified present by name before the gate. The 14-term union gate — `~SectionDifferential|~ControlFlowDifferential|~Initialize|~MoveTable16|~move_|~MoveAlphanumericSender|~ReportWriter|~Exception|~Use|~Declarative|~Drift|~EditionGate|~Nist|~Corpus`, extended with `~Corpus` (all five clusters add goldens and only `CorpusRunnerTests` executes them) and `~Nist`, every term measured LIVE by `filter_population.py` — ran over Conformance, the UNFILTERED `Cobol.Net.Tests.Unit` assembly, Characterization and the legacy `CobolSharp.Tests.Integration` assembly: Conformance **3137 / 3137**, the full Unit assembly **24027 / 24027**, Characterization **33 / 33** and legacy Integration **503 / 504** (1 skipped, unchanged). Four terms the clusters named (`~DispatcherTransferIdiom`, `~SendingRef`, `~ReportSumOperandCapture`, `~FlowUseReentrancy`) were measured DEAD against Conformance and DROPPED — each is a Unit-only drift test and each is live (5, 4, 5, 6) in the UNFILTERED Unit leg. **The gate went RED once and the red was the train's own guard working**: PB482's `EveryKeyReferenceCapture_IsANamedOperandHelper` flagged `ReportOccursOf`, which is train 34's PB562 code and so invisible to every cluster's own gate. It was ADJUDICATED with a spec derivation, not silenced — `OCCURS … DEPENDING ON data-name-1` (§13.18.38.2 format 3) writes DATA-NAME-1, a position admitting qualification only (§8.4.2.2.2 format 1 carries no subscript; §8.4.3.3.3 SR5's NOTE bars reference modification where a general format writes data-name-n) — so the key helper keeps the whole written reference there and drops nothing. ⚠ Residue reported to the registrar and NOT fixed under another note: the grammar still PARSES `DEPENDING ON WS-T(2)` and drops the subscript in silence, where COBOLNET2046 refuses the analogous subscripted UPON operand. That red is also the guard's falsification — red before the row, green after. Semgrep: no rule increased; `cobolnet-raw-diagnostic-code-literal` fell **419 → 416** and the baseline was deliberately left UNTIGHTENED so a concurrent train's CI is not reddened by a ratchet it did not cause. **FOUR diagnostic codes are claimed: COBOLNET2030** (PB416, §14.9.20.3 SR1's class exclusion), **2031** (PB416, SR4's ‘the implied MOVE shall be valid’ over `MoveTable16`), **2045** (PB482, §13.18.54.3 SR5's category screen on a SUM addend) and **2046** (PB482, SR7's UPON-is-a-detail screen); checked for collision ACROSS clusters. COBOLNET2036–2038, 2042–2044 and 2054–2056 were allocated to clusters that did not need them and are returned FREE — PB405+PB414, PB426+PB844 and PB368 are each a RAISE or an emission, not a diagnostic. The **next free code is COBOLNET2047** (the orchestrator has allocated centrally up to COBOLNET2077 — do not reuse). ⚠ Pre-existing and NOT this train's: `COBOLNET1535` is allocated TWICE in `DiagnosticCatalog` and still is; and `EmitterState.cs` / `ControlFlowEmitter.cs` quote §14.9.14.4 GR5a as “the most closely preceding, unterminated inline PERFORM” where the standard reads “the most closely preceding, and as yet unterminated, inline PERFORM” — right clause, elided quotation, `cite.py --check` fails on it. Notes flipped to `landed`: **PB405, PB414, PB416, PB426, PB844, PB482 and PB368**. No note was filed by the lander. **The register is 909 notes.** NOT closed on purpose and named for their owners to re-measure: GR-14.9.17.4-1 (PB413) and GR-14.9.37.4-1 (PB388), both fixed by PB405's repair; SR-14.9.25.3-6, now implemented and pinned by PB416 but belonging to §14.9.25.3's own inventory. ⛔ The worklist is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8).
