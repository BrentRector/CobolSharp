...
- **▶ LIVE STATE (2026-09-13 — TRAIN 33 IS ON MAIN):** **the P14 traceability inventory is 4,348 rows at GAP 2481.** **Landing train 33 carried PB535 + PB506 + PB721 + PB463 + PB390 onto main in ONE landing** — five cluster commits plus one closing DEVLOG/§0 commit (DEVLOG 1604) — **GAP 2501 → 2481 with the denominator unchanged at 4,348**, 22 verdict records over seven batch files, every one re-checked on the MERGED tree in manifest order (all CONFORMS; the arithmetic 4 + 3 + 7 + 2 + 4 lands exactly on the manifest's predicted 2481). All five clusters landed; none was dropped. Rows 1–3 were based on `a1de916e` and rows 4–5 on `c68205ee`, with trains 31, 32 and registrar #4 in between, so three conflicts needed real resolution: `PictureAnalyzer`'s float-edited arm met train 32's PB646 (PB646's `ScreenUsageAgainstPicture` call KEPT — it replaced the hand-staged national block with the one §13.18.60.3 USAGE × PICTURE screen — and PB535's one-line `Length: CharacterPositions(expanded)` taken on top of it), PB721's design determination was authored against a tree without train 31's PB337 D24 and is **renumbered D25**, and ⛔ `tests/version-matrix/cobol-words.json` conflicted MID-OBJECT between main's `BOOLEAN` row and PB721's `BYTES` row — a both-sides merge produced ONE object with duplicate keys that `json.load` accepts and that SILENTLY DROPPED BOOLEAN, caught only because `gen-cobol-words.ps1` printed `total=123` where 124 was due. The 14-term union gate — `~Drift|~EditionGate|~DataClause|~Corpus|~VersionMatrix|~Picture|~Nist|~Report|~Value|~Diagnostic|~File|~Sort|~DynamicLengthTests|~OoSpine` — ran over Conformance (the 356 `NistDifferentialTests` cases included), the UNFILTERED `Cobol.Net.Tests.Unit` assembly, Characterization and the legacy `CobolSharp.Tests.Integration` assembly: Conformance **5570 / 5570**, Unit **23938 / 23938**, Characterization **33 / 33**, legacy Integration **503 / 504** (1 skipped), and all four audits at zero (code citations 0 findings, doc citations 0 MISFILED, Annex A.1 clean, evidence supersession 0 unmarked claims); the GnuCOBOL external corpus was absent in this fresh worktree and `build-local.ps1` fetched it successfully, so both `ExternalCorpusPopulationDriftTests` ran for real. `~CobolDynStringTests` and `~DecidedRuleStage` are Unit-only class names and were DROPPED from the Conformance filter rather than run dead (`filter_population.py` refuses a gate with a dead term); both classes run in the unfiltered Unit leg. Semgrep: `cobolnet-raw-diagnostic-code-literal` 417 → 419, both added lines attributed to PB535's two new COBOLNET0808 assertions — the 6th and 7th raw site of the descriptor-less COBOLNET0808 picture-symbol BAND, in the same method as their five siblings — **re-baselined to 419** with the disposition in `kb/Work/PB175`. **FIVE diagnostic codes are claimed: COBOLNET2009 / 2010** (PB721, the RECORD clause's own integer pair and a record description outside it), **2012 / 2013** (PB506, the VALUE and SOURCE multi-operand repetition rule) and **2027** (PB463, a LIMIT phrase above the implementor maximum); they were checked for collision ACROSS clusters. COBOLNET2003–2005, 2011 and 2015–2017 were allocated to clusters that found they did not need them and are returned FREE. The **next free code is COBOLNET2028** (src-grep max = catalog max = COBOLNET2027, measured on this tree); the holes below it are 1962/1963, 1967–1969, 1972–1975, 1978–1980, 1986–1993, 1996, 2000–2008, 2011 and 2014–2026. Notes flipped to `landed`: **PB535, PB506, PB721, PB463 and PB390** — no row was deliberately left open in any of the five. ⚠ **TWO DETERMINATIONS the owner may overturn**, both recorded in DEVLOG 1604 and in their notes: PB463 fixes the Annex A.1 item 62 implementor maximum for a dynamic-length elementary item at **1,073,741,791 (0x3FFFFFDF)**, the carrier's own ceiling (`docs/CONFORMANCE.md` §7 row DOC-A.1-62), rejecting a smaller "always allocatable" policy maximum; and PB390 **REFUSES `SET SW-ON TO TRUE`** on a switch-status condition-name, reading §8.4.4.1's last sentence as not admitting a condition-name into a SET format because Format 3's printed general format (PDF p759, RENDERED, not the OCR) has no condition-name alternative and Format 4's SR6 excludes the switch kind by name. ⚠ **THE REPORT-LEAD BACKLOG IS REGISTERED, train 33's included.** Registrar #4 (`a1d1b200`, DEVLOG 1602) filed the trains 27–33 leads as 36 notes (PB835–PB870) with 10 extensions, and registrar #5 (`b929f96e`, DEVLOG 1603) — which landed under this train WHILE IT WAS GATING — filed the reports that arrived after it as PB871–PB875 with 4 extensions. **The register is 908 notes.** All NINE leads train 33's five reports named were checked one at a time against `kb/Work/` on the rebased tree and every one has an owner: PB852 (SOURCE's `arithmetic-expression-1` / ROUNDED, no grammar surface), PB551 (§13.18.63.3 SR25's vacuous format-4 leg), PB853 (§13.15.3 SR10 unenforced — a report item with no SOURCE, VALUE or SUM prints `000`), PB858 (SR-13.18.43.3-6 has no site until `DataItem` carries its SECTION), PB859 (§5.5 1)'s "nonzero" default enforced nowhere as a general mechanism), PB871 (a dynamic-length receiver of STRING/UNSTRING/ACCEPT/INSPECT/INITIALIZE stored through its PICTURE's ONE character position), and PB390's own note for the three decided rules shipped as run-time aborts (`SearchBinder`, `ReportWriterBinder`, `EvaluateBinder`). ⛔ The worklist is `kb/Work/` — `python scripts/spec/work.py next` — never this paragraph (rule 8). ⚠ `GR-13.18.63.4-21`, owned by PB562+PB565, is now WRONG at DIVERGES — PB506 discharged its three measured legs and its residue is GR9 OCCURS and GR1 float.
...
- **⛔ BATTERY REFERENCE — CURRENT: battery #74, 2026-09-13, on `359a2ebc` — train 32 — run in a DETACHED WORKTREE cut at the batch head.**
...
- **BATTERY REFERENCE — PREVIOUS (#73, superseded by #74 above): battery #73, 2026-09-13, on `67be3552` — train 31 — run in a DETACHED WORKTREE cut at the batch head.**
