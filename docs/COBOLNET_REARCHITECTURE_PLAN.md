...
- **⛔ BATTERY REFERENCE — CURRENT: battery #82, 2026-09-21, on `c774def44` — train 43 — run in an ISOLATED WORKTREE pinned at the batch head, never on main.**
  ✅ ALL GREEN ON EVERY COMPILER LEG — Conformance **7709 / 7709**, Unit **24365 / 24365** (the GPL corpus was copied into the battery worktree, so `ExternalCorpusPopulationDriftTests` MEASURED its population rather than being red by absence), characterization **33 / 33**, guard evidence-rule witnesses AND the compiler-identity watchdog green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN (the shipped `cobol` compiler), all three static audits 0, differential **1323 cases** with **1 ATTRIBUTED FLIP**: `run_file:3586` `AGREE_ACCEPT → WE_REJECT_THEY_ACCEPT` on COBOLNET2186 — PB433's PERFORM-range-across-declaratives screen, landed in train 43. ⚠ **That is the DIVERGENCE-ADDING shape — the shape a REGRESSION takes — and it is not one.** The case (GnuCOBOL's own title: “DECLARATIVES procedure referencing (multiple)”) declares TWO separate declarative sections and writes one `PERFORM … THRU` naming both, from a non-declarative section; **§14.9.28.3 SR11** — “When procedure-name-1 and procedure-name-2 are both specified and either is the name of a procedure in the declaratives portion of the procedure division, both shall be procedure-names in the same declarative section”, re-derived here and `--check`ed as rule 11, never inherited — makes that source NON-CONFORMING, and §14.9.28.4 GR4's specified set of statements has no conforming meaning across the boundary. GnuCOBOL accepts it only under its own `-fno-section-exit-check`, which its case file's comment says is required because control is “guaranteed” to leave the section — their latitude, not the standard's. **The standard licenses the new answer**, so the baseline was regenerated with `--write-baseline` on the same tree (the re-run reproduced the identical four totals — 579/467/236/41 — so the leg is deterministic here) and its diff is EXACTLY that one row, tier unchanged (`DEFAULT_DIALECT`), zero other churn. **Complement measured, not assumed:** `corpus_sweep --codes COBOLNET2186` finds that ONE case over 1323 compiled cases / 1611 extracted programs, and of the eight external programs that write a `PERFORM … THRU` anywhere after a DECLARATIVES header the other seven were ALREADY `WE_REJECT_THEY_ACCEPT` for unrelated reasons and did not move — so the screen fired where SR11 is violated and nowhere else. ⛔ **#81's standing requirement was “ALL GREEN with 0 flips” and this run has ONE — recorded as a miss, attributed and licensed, not rounded down to green.** No corpus drift (1323 cases == 1323 baseline rows), no case without a compiler verdict, no harness failure. Population over #81: Conformance **+77** (7632), Unit **+74** (24291); both figures reproduce the train-43 lander's own whole-assembly gate exactly. 1080 s wall. DEVLOG 1627. Battery #83 at the next train's head must be ALL GREEN with 0 flips.
- **BATTERY REFERENCE — PREVIOUS (#81, superseded by #82 above): battery #81, 2026-09-20, on `c701786d` — train 42 — run in an ISOLATED WORKTREE pinned at the batch head.**
  ✅ ALL GREEN ON EVERY COMPILER LEG — Conformance **7632 / 7632**, Unit **24291 / 24291**, characterization **33 / 33**, guard witnesses and the compiler-identity watchdog green, NIST **364 MATCH / 0 REGRESSION** and audit CLEAN, all three static audits 0, differential **1323 cases** with **3 ATTRIBUTED FLIP(S) — ALL THREE FIXES**: `run_misc:470` and `run_misc:513` `WE_REJECT_THEY_ACCEPT → AGREE_ACCEPT` (PB511's `IS EXTERNAL [ AS literal-1 ]`, §13.18.22.2 general format + §13.18.22.3 SR1 — legal source we used to refuse) and `syn_definition:1132` `WE_ACCEPT_THEY_REJECT → AGREE_REJECT` on COBOLNET2121 (PB466's ambiguous procedure-name reference, §8.4.2.2.1). The baseline was regenerated in the same commit and its diff was exactly those three rows. DEVLOG 1624.
