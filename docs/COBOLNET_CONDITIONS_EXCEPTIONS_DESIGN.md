...
Binder detects the partial form (leftmost token is a relop/class-name-without-id/sign-word) and synthesizes `subjectK <partial>` as a full condition; the corresponding subject is treated as TRUE. Grammar already admits `condition` as a WHEN item; injection happens in binding. **NOT YET REALIZED** — `evaluateWhenItem` admits no object beginning with a bare relational operator, so no partial expression parses and none reaches the binder. Two consequences show up elsewhere as residuals rather than as their own defects: Table 15's Partial-expression row is unenforceable (§14.9.13.3 SR10 stays PARTIAL) and SR6 e) — a partial-expression object over a boolean/numeric data-item subject — is unexercisable (§14.9.13.3 SR6 stays PARTIAL). Both close when this lands.
