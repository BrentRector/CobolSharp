...
**Residue, named.** A BARE user-defined class-name or alphabet-name object (`WHEN MY-CLASS`) is indistinguishable from identifier-2 in the grammar and still binds as identifier-2; only the resolved SYMBOL separates them, which is the same doctrine that makes a bare level-88 object condition-2. The `IS`-led spelling (`WHEN IS MY-CLASS`) reaches the partial rule unambiguously.
