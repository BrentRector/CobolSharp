# COBOL.NET — OO slice implementation briefs (workflow-regenerated 2026-07-04)

> **Status: LEDGER / implementation briefs** for the remaining OO slices, regenerated by the 3a/3b
> review workflow (DEVLOG 603; 5 parallel brief agents over the deep-dive + spec + code seams). The
> AUTHORITATIVE design is `docs/COBOLNET_OO_DESIGN.md` — each brief's decisions FOLD INTO the deep-dive
> when its slice is implemented (the feedback_plans_in_repo discipline; these are the working copies,
> kept in-repo so they survive sessions — the /e/tmp brief-loss lesson). Verify spec line anchors on use.
...
> against the live tree as of the uncommitted slice 3a/3b state (OoClassTable already marks `OverrideOf` by
...
> the post-DEVLOG-602 tree (spine parts 1–2 + slice 2 landed; slices 3a/3b INHERITS + SELF/SUPER in flight —
> this slice builds on `OverrideOf`/`ConformsTo`/`InvokeForm.Self|Super`, all already in the tree). The EC
> engine substrate is LANDED (DEVLOG 577): `>>TURN`/TurnState fold, USE F3 `__EcDispatch`, RAISE/RESUME,
