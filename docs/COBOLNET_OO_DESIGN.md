...
### Slice 1 (DEVLOG 447) — CLASS-ID + one METHOD-ID + NEW + no-arg INVOKE + USAGE OBJECT REFERENCE
...
**Reusable test assets:** `tests/conformance/2002/oo_hello|oo_instance_data|oo_method_perform|oo_method_args|oo_inherit|oo_super|oo_self|oo_self_polymorphic|oo_object_group` (.cob + .out goldens — 9 pairs, today run only by the LEGACY runner, `tests/CobolSharp.Tests.Integration/ConformanceTests.cs:24`; a greenfield 2002 conformance runner is part of the spine) and the `OoTests` suite (`tests/CobolSharp.Tests.Integration/OoTests.cs` — 13 tests; its FailsLoudly set is precisely the checklist of gaps the port must implement or re-diagnose).
...
### Binder seam — parses-then-rejects loudly today (one silent-drop hazard to fix FIRST)
INVOKE has no bind arm: it falls to the default `BoundUnsupported` (`src/Cobol.Net.Compiler/Binding/Bound/StatementBinder.cs:182`), rendered as a loud runtime failure (`CSharpEmitter.cs:346`); SET obj-ref → BoundUnsupported (StatementBinder.cs:668-669); EXIT METHOD → BoundUnsupported (:218); RAISE identifier / EXIT PROGRAM RAISING identifier await the OO wave (`StatementBinder.Exceptions.cs:53-54`, StatementBinder.cs:211-214). USAGE OBJECT REFERENCE parses but DataBinder ignores it (`DataBinder.cs:624-630`; no object-reference category in PicInfo — matches the hard problem above). ⚠ HAZARD: `CallCollectUnits` iterates ONLY `group.programUnit()` (`CSharpEmitter.Call.cs:155-157`) — classDefinition units in the group are silently SKIPPED from emission. Fix in the spine (collect class units or fail loudly) before anything else.

### Emitter seam — the CallUnit machinery IS the ClassUnit template
