...
- **I-O status '39' (fixed-file-attribute conflict, §9.1.13.6 item 7; E.3.3 item 35) — TWO DIFFERENT CASES, and
  ...
  - **OPEN — a genuine unimplemented gap.** §14.9.27.4 GR10's set is **not** empty here (kb/Work PB193): every
    surveyed implementation validates something on OPEN, and doing so needs the persisted physical-attribute
    catalog (record size / organization / code-set) the host-file model does not carry. A.1 item **129** is
    therefore still undischarged and OPEN never returns '39'. Tracked, not documented as a determination — a
    deferral written as a definition would be the worse of the two errors §7's preamble warns about.
