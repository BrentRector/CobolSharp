...
> documents only the entries determined so far — currently items 2, 59, 158 and the **storage-representation
> family 205–215** that V59 pinned (BINARY · the BINARY-CHAR family · COMPUTATIONAL · INDEX · PACKED-DECIMAL:
> the byte width, the radix, the byte order and a worked example for each, since a COBOL developer reading this
> needs to know what lands on disk). Completing it is **PHASE-14 Step 0** work — the four-edition
