...
> **MEASURED 2026-08-08 — this register discharges 13 of the 199** (`python scripts/spec/audit_annex_a1.py`
> reports it, and the numbers below are its output, not a hand count): items 2, 59, **135**, 158, **179**,
> **202**, and the **storage-representation family 205–215** that V59 pinned (BINARY · the BINARY-CHAR family ·
...
> Int128 intermediate) is documented voluntarily. **186 obligations remain.**
...
| 216 | **The counter registers' declared capacity — LINAGE-COUNTER, PAGE-COUNTER, LINE-COUNTER**, §8.4.3.14.4 GR1 / §8.4.3.15.4 GR1, documented (the report counters' size is not specified by the standard) | All three registers are **unsigned integers** per their GRs, so `LOWEST-ALGEBRAIC` of any of them is **0** and `SMALLEST-ALGEBRAIC` is **1**. **LINAGE-COUNTER**: §8.4.3.14.4 GR1 ties its size to "the page size specified in the LINAGE clause", so `HIGHEST-ALGEBRAIC(LINAGE-COUNTER OF f)` folds to the LINAGE literal's value directly (`LINAGE IS 66 LINES` → 66); with a data-name operand the page size is set at run time, and the compile-time capacity is the MAXIMUM that operand can specify — its PICTURE's all-nines. **PAGE-COUNTER / LINE-COUNTER**: §8.4.3.15.4 GR1 gives them no size, so COBOL.NET declares them **PIC 9(18)** — the all-nines of the runtime's 64-bit counter carrier (`ReportWriter.LineCounter`/`PageCounter`, both `long`), the same one-constant-not-two discipline as item 202 — and `HIGHEST-ALGEBRAIC` folds to 999,999,999,999,999,999. (kb/Work R26; §8.5.2.12 items 3/4/5 make all three registers category-numeric DATA ITEMS, which is what admits them to the §15.43.3/§15.58.3/§15.83.3 r1 argument position at all.) |
