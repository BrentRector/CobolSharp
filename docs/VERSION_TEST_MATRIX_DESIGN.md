...
> **STATUS BANNER — DESIGN (proposed; not yet built). LIVE design reference.**
>
...
`cobol --standard V prog.cob` **is** the "COBOL-V compiler." Correctness is per-edition: for every COBOL construct
...
- **Positive — COBOL-85 baseline:** the NIST CCVS85 corpus (`tests/nist/programs` + `tests/nist/valid`, 364 programs)
  is the authoritative 85 positive set. Today `NistDifferentialTests` runs it at **DialectLevel 85 only**
  (`NistDifferentialTests.cs:52`, hard-coded). **INV-1 extends this:** run each NIST program at 2002/2014/2023 too and
...
- `CompilerDriver.Options.DialectLevel` already exists and flows to the front-end. **Thread it into bind/emit**:
  `CSharpEmitter.Emit(tree, dialectLevel)` → `EmissionContext` → `OperandText`/`ConditionRenderer`/binders (the §3
  INV-3 hook; not needed until a real behavior-variant appears, but the seam belongs here).
