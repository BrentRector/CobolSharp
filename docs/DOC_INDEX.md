...
| `docs/COBOLNET_REARCHITECTURE_PLAN.md` | LIVE | **The rearchitecture & 100%-ISO master roadmap.** A resumable, execution-grade plan: north-star 5-assembly architecture, 11 principles, the dual-backend §3 mandate (Roslyn↔CIL), a 17-phase dependency-ordered index (status checkboxes), an owner-decisions table, and §7 backfill refinements. Step-by-steps in `docs/rearchitecture/PHASE-00..16-*.md`; designs in `docs/rearchitecture/DESIGN-*.md`; execution ledgers in `docs/rearchitecture/PLAN-*.md` (e.g. `PLAN-bindtime-gating-migration.md` — moving edition introduction-gating to bind-time `ConstructRegistry.Check`); per-phase anchor-scout references in `docs/rearchitecture/PHASE-NN-scout-notes.md` (LEDGER — verified spec §/GR/SR anchors + hand-derived golden values + code seams; the working reference during a wave, retained afterward as durable spec-to-code traceability, e.g. `PHASE-11-scout-notes.md`); review-findings ledgers in `docs/rearchitecture/PHASE-NN-plan-vs-spec-review.md` (LEDGER — adversarially-verified plan/impl/spec-drift findings with per-item dispositions, e.g. `PHASE-13-plan-vs-spec-review.md`); as-is survey/critique in `docs/rearchitecture/{SURVEY,CRITIQUE}-*.md`. **Live status/resume point: the plan's own STATUS BANNER (kept current every session; do not duplicate it here — this row drifted twice).** As of 2026-07-15: Phases 00–08 + Exec Steps A–E DONE; RESUME AT PHASE-09. |
