...
| `docs/COBOLNET_REARCHITECTURE_PLAN.md` | LIVE | **The rearchitecture & 100%-ISO master roadmap (DEVLOG 665).** A resumable, execution-grade plan: north-star 5-assembly architecture, 11 principles, the dual-backend §3 mandate (Roslyn↔CIL), a 17-phase dependency-ordered index (status checkboxes), an owner-decisions table, and §7 backfill refinements. Step-by-steps in `docs/rearchitecture/PHASE-00..16-*.md`; designs in `docs/rearchitecture/DESIGN-*.md`; execution ledgers in `docs/rearchitecture/PLAN-*.md` (e.g. `PLAN-bindtime-gating-migration.md` — moving edition introduction-gating to bind-time `ConstructRegistry.Check`); as-is survey/critique in `docs/rearchitecture/{SURVEY,CRITIQUE}-*.md`. Execution IN PROGRESS: **Phases 00–06 DONE** (P03 the version-conformance pipeline ✅ 2026-07-10 — the two-arm `VersionConformancePass` is the SOLE edition gate, binder edition-agnostic save the documented UDF exception; P04 frontend consolidation ✅ 2026-07-10, D10 relocated → P15 §CUT 2.5; **P06 the Real Binder ✅ 2026-07-11, DEVLOG 767–774** — `BinderDriver`→immutable `BoundCompilation`, the declared `GroupTail` manifest with the conformance pass as NAMED terminal, sealed `IReadOnly` binder views, the DEBUG watermark gate, and the ONE scope-aware `SymbolTable` replacing the lookup quadruple). Per the §4.1 TOOLING-FIRST resequence: **Exec A ✅** (the source-generated exhaustive bound-tree visitor, PHASE-07 Step 6, DEVLOG 755–766) · **Exec B ✅** (P06) · **Exec C ✅** (P05 the unified data model CLOSED 2026-07-11, DEVLOG 776–785 — the `StoreAsImage` FLAG deleted, `Storage` computed once by `StorageFormPass` with the name kept as the read-only projection; `RecordLayout` the ONE width/offset authority, §13.18.44.3 SR8 enforced COBOLNET1539; `Binding/Model/` + `PictureAnalyzer`/`StrongTypeModel`; sentinels → `DataItem.Pending`; `RedefinesClass.Classify`; apostrophe goldens) · **◐ Exec D (NOW) = the rest of PHASE-07** (structural `Place`, god-class decomposition, the `ICodeGenBackend` seam). |
