...
| `scripts/spec/repairs/annex_d_*.py` | TOOL | The three Annex D figure generators — `_flowcharts` (VARYING charts, D.1), `_truth_charts` (condition evaluation, D.7–D.10), `_structure` (D.3's nested schematic, and where D.6 belongs). ⚠ **Each needs the collision guard**: `put` refuses to overwrite a non-blank cell, with a separate `junction()` for the one legitimate case. It has caught seven defects that would have rendered as plausible pictures. Border WEIGHT is notation in D.3 (heavy/light/dashed = group/unit/element). |
