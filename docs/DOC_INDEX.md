...
| `docs/COBOLNET_REARCHITECTURE_PLAN.md` | LIVE | **The rearchitecture & 100%-ISO master roadmap (DEVLOG 665).** A resumable, execution-grade plan: north-star 5-assembly architecture, 11 principles, the dual-backend §3 mandate (Roslyn↔CIL), a 17-phase dependency-ordered index (status checkboxes), an owner-decisions table, and §7 backfill refinements. Step-by-steps in `docs/rearchitecture/PHASE-00..16-*.md`; designs in `docs/rearchitecture/DESIGN-*.md`; execution ledgers in `docs/rearchitecture/PLAN-*.md` (e.g. `PLAN-bindtime-gating-migration.md` — moving edition introduction-gating to bind-time `ConstructRegistry.Check`); as-is survey/critique in `docs/rearchitecture/{SURVEY,CRITIQUE}-*.md`. Execution IN PROGRESS: **Phases 00–03 DONE**; **Phase 03 (the version-conformance pipeline) ✅ DONE 2026-07-10** (superset parse · edition-agnostic bind · ONE two-arm `VersionConformancePass` over the bound tree [parse-arm on recognition + bound-arm on resolved facts] · emit-if-clean; `ReservedWordEditionHints` deleted): all 15 steps complete (Steps 1–13 residue migration DEVLOG 709–717; Step 14 the two-arm pass DEVLOG 718–740 incl. the full 14g DATA/PIC/OO gate migration; Step 15 close), all 9 exit criteria hold, the binder is edition-agnostic save the ONE documented UDF exception. **◐ Phase 04 (frontend consolidation — single-source the context-sensitive word set + shared literal fragments + a typed `Cst/` façade) IN PROGRESS @ Group A** (byte-neutral; the Step-A1 word-set recon is done — see the PHASE-04 doc STATUS). |
