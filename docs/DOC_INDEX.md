...
| `docs/rearchitecture/CONFORMANCE-FIX-QUEUE.md` | LEDGER | **The VERIFIED, fix-ready conformance queue** (`wf_29a15db2` verified V54–V59 + CA1–CA38 against spec+code): 44 CONFIRMED (2 blocker/30 major/10 minor/2 nit), each with a decision-complete spec-derived FIX + a spec-derived GOLDEN; 0 refuted; 2 reclassified to NEEDS-OWNER-DECISION (CA14, V59). **This is the active work-list** — fix top-down by severity, batched by area, each landing with its golden. |
