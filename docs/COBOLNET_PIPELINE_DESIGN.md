...
The COBOL.NET compiler is a 5-phase pipeline: Frontend (reused ANTLR preprocess/lex/parse → parse tree) → Bind (resolve symbols + build a typed/categorized BOUND TREE that PRESERVES COBOL structure) → Desugar (bound→bound passes: MOVE CORR, PERFORM VARYING AFTER, condition-names) → Emit C# (a decomposed emitter over the bound tree, NO lowered IR) → Roslyn compile → assembly + .g.cs (+ PDB).
...
### D7. Thread exactly ONE DialectMode (an enum: Cobol85/2002/2014/2023, NOT a bare int) from CLI → Frontend (grammar admit/reject via the {isXXXX()}? gates) → Binder (semantic gating AND flagging — obsolete/extension diagnostics). Keep the legacy two-axis model (version vs strictness) rather than invent a new one.

**Rationale.** The current CLI carries `int DialectLevel` (85/2002/...) and the frontend already has DialectLevel gating, but a bare int has no place for the strictness axis or the flagging behavior that FlaggingConformanceTests exercises. A single typed DialectMode threaded through every phase (one source of truth) matches feedback_singular_pattern and the documented project_dialect_strictness two-axis model. Flagging (e.g. an OBSOLETE construct used under --std 2023) is a binder-phase diagnostic, not a parse error — so the binder must see the dialect.

**Rejected alternatives.** (a) Bare int DialectLevel (current): rejected — no strictness axis, no flagging seam. (b) A new dialect model: rejected — the legacy two-axis (version × strictness) model is already designed, documented, and tested; reuse it.
...
PIPELINE TYPES (new namespaces under src/CobolNet):
...
DIALECT: enum DialectMode threaded via EmitContext.Dialect + Frontend.Dialect + Binder.Dialect.
...
### Scale-correct fixed-point arithmetic with ROUNDED (8 modes) and ON SIZE ERROR, on native long/decimal (no decimal substrate by default per owner lock).
...
- For 19-38 digit pictures the owner lock specifies Int128. Confirm the CobolNum/NumProfile runtime substrate (reused) is Int128-capable, or whether that is net-new runtime work that the pipeline design must schedule before such pictures are bound (currently PicInfo maps numeric only to long/decimal/float/double).
