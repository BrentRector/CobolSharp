...
| M4-3 | Other 2023 intrinsic/bit/boolean + dynamic-table finalization | open | PARTIAL | **CONCAT (§15.18) + BASECONVERT (§15.12) LANDED (Phase 5, DEVLOG 628)** — the clean positional-arg pair, live+golden (intrinsics_string_2023, GreenfieldOnly). Residue STAGED-LOUD: CONVERT/FIND-STRING/MODULE-NAME/SMALLEST-ALGEBRAIC/SUBSTITUTE (IntrinsicCatalog.cs Deferred+2023-gated) | phase 5 (+ a, 6) | Umbrella. Intrinsics→5 (FIND-STRING/SUBSTITUTE/CONVERT/TRIM need a shared keyword-argument mechanism; MODULE-NAME needs module context; SMALLEST-ALGEBRAIC needs PIC metadata); boolean/bit→4(a) not-started; dynamic-table→6 (overlaps M3-1). |
