...
COBOL.NET stores a datum as its VALUE (PIC 9(4)→a long holding unscaled 1234; PIC X(4)→a 4-char string). REDEFINES (ISO 13.18.44) and RENAMES/level-66 (13.18.45) are byte-image REINTERPRETATION: a second, differently-typed VIEW over the same storage. Value model and byte image coincide only for trivial same-USAGE/same-layout puns; every hard case is hard because they diverge (a PIC 9(4) COMP punned as PIC XX is the two binary bytes, unrelated to the long's value; PIC X(20) punned as -9(9).9(9) reinterprets 20 char positions under an edit template). With NO shared byte[], two typed reps cannot both be live, stored, and coherent. THE ONE CORRECT COHERENCE ANSWER: they do not both exist. A "redefines class" (entries sharing a storage area) has exactly ONE stored backing — the canonical — and EVERY other view is a computed accessor over it (backend-neutral; the primary Roslyn backend renders it as a C# property, a CIL backend as get/set method pairs). Never two stored fields per storage area (the incoherence trap; violates feedback_one_mechanism_per_job). IMPLEMENTED MODEL = 3 tiers, one per class, priority cascade D>B>A: A-Alias (every member elementary with the SAME CLR storage type AND image width: identical PIC+USAGE, or numeric-over-numeric of equal width where each view reinterprets the shared unscaled value via its own scale/NumProfile; plus RENAMES-without-THRU): one typed field, other names are pass-through accessors. B-StringCanonical (the whole class has a BYTE IMAGE — every leaf's storage is a pinned NumericByteForm or a character string): canonical = ONE string of class-max width under the char==byte convention; each view = a typed (offset,width) accessor over it. The window holds the leaf's TRUE BYTES, not its digits (V59): zoned characters for DISPLAY, radix-2 for BINARY/COMP-5/BINARY-*, BCD for PACKED-DECIMAL, big-endian IEEE for the float family, the 8-byte occurrence number for INDEX — §13.18.60.4 GR4/GR11/GR13-GR15 leave each representation to the implementor and CONFORMANCE.md items 205-215 pin ours. Since the Step D arm-1 dissolution this is not merely the dominant case, it is EVERY non-alias, non-rejected case, mixed-USAGE puns included. C-ByteCanonical: DELETED (Step D, kb/Work PB164) — there is no such tier and no RedefCodec. It was reserved for mixed-USAGE puns over usages thought to have "no character storage image"; once every numeric usage had a pinned NumericByteForm, those puns became ordinary Tier-B byte windows over the one string backing, so the separate class-scoped byte[] was never needed. D-Reject loud (spec-forbidden/unmodelable: object/pointer/message-tag/strongly-typed rules 12/14; OCCURS DEPENDING ON / variable-length / dynamic-length rules 5/17): a diagnostic, which is conformant since these are already illegal. RENAMES folds into the same tiers as a COMPOSED view over EXISTING fields (it adds no storage; GR1 no-THRU = attribute inheritance = Tier A; GR2 THRU = alphanumeric group view = Tier B composition over the spanned leaves' display images). The THRU alias's CARRIER is one composed string, but the DATA ITEM is a GROUP item for every rule worded "group item": `ItemCategory.GroupKindsOf` answers Alphanumeric for it and `ItemCategory.IsGroupItem` is the category predicate (never the structural `DataItem.IsGroup`, which a level-66 entry fails for want of subordinates), and `VariableLengthCompatibility.AliasAtoms` gives it its §8.5.1.12 layout from `RenamesInfo.Span` (kb/Work PB907). Tier classification reuses the legacy RecordClassificationPass closure shape (byte propagates across the REDEFINES class + to all subordinates, monotone, terminating), re-verdicted to the lattice A⊑B⊑D (join=max tier). Status (IMPLEMENTED): the model is live — `src/Cobol.Net.Compiler/Binding/Model/RedefinesModel.cs` (RenamesInfo / RedefinesTier / RedefinesClass) + `DataItem` (RedefinesTargetName / RedefinesTarget / Renames / Class / IsCanonical / Renames66); `DataBinder` binds level 66, resolves targets post-build, and classifies via `ComputeTier` — Tiers A+B emit and Tier D rejects loud. The owner fork recorded at `COBOLNET_DESIGN.md` §18 #1 (whether Tier C's persistent class-scoped byte[] was the accepted "bytes only at a boundary" realization) is MOOT: the tier dissolved into B rather than being built or collapsed into D, so Tiers A/B stay 100% typed and no byte[] canonical exists anywhere.
...
- **The NESTED pointer/object leaf and its backstop (kb/Work PB179 → PB183).** `ComputeTier` keeps a Tier-D arm for a pointer/object-class LEAF nested inside a member — which is NOT §13.18.44.3 SR12/SR14's letter (those name the ENTRY-level items, per the drafting contrast with SR9's "nor any entry subordinate to it"). That leaf's real defect is in its own DECLARATION: **§13.18.60.3 SR14** confines the five pointer/object usage phrases to an elementary item at level 1 (or 77) or one subordinate to a STRONG type declaration, so a nested pointer leaf under an ordinary group is nonconforming before REDEFINES is reached. That screen now exists — `CheckUsageDeclarations` (`DataBinder.UsageDeclaration.cs`, COBOLNET1724), a declared bind pass running well ahead of `ClassifyRedefinesClasses` — and the backstop is therefore unreachable-by-construction on conforming source. ⛔ **It is KEPT, not deleted:** it remains the loud guard for the recovery paths, and without it such a leaf reaches the byte-window machinery as a silent zero-width Tier-B alias, which is the exact outcome the tier model exists to prevent. `PointerObjectClass` is now THE ONE class predicate shared by both screens (pinned by `UsageDeclarationPlacementDriftTests`), so the REDEFINES rule's class list ("class object, message-tag, or pointer") and the USAGE rule's phrase list cannot drift apart as MESSAGE-TAG and FUNCTION-POINTER gain models — and it is the STORAGE predicate too, since kb/Work PB231 defined it as `SlotWindow.CarriedBySlot` ("does this member ride the area's managed slots rather than its bytes?"), which is the same population for the same reason.
  ⛔ **THE ARM'S REASON CHANGED WITH kb/Work PB231's pointer third (2026-09-05), and the change is worth stating precisely.** A pointer-class leaf is no longer refused by the shared byte-window carriage gate `DataBinder.ByteWindowResidueOf` — it RIDES a shared storage area, on that area's MANAGED SLOTS (D-SLOT in `COBOLNET_DATA_MODEL_DESIGN.md`). What a REDEFINES class lacks is the CARRIER: the slots live on a `StorageCell`, which the three cell surfaces (EXTERNAL, ADDRESS OF, BASED) have and a REDEFINES class's plain stored `string` backing does not. That is a property of the carrier, not of the leaf, so it is asked HERE, once, rather than smuggled back into the shared gate — which would have re-created exactly the two-arm divergence PB231's collapse removed. `RedefinesClass.IsCellBacked`, set by the ONE forcer `ForceStringCanonical`, is the test.
- **SR14 decides what a group with a pointer leaf can even BE.** A consequence worth stating here because it bounds this subsystem's input: on conforming source the ONLY way to write a group containing a pointer/object-class leaf is inside a `TYPEDEF STRONG` template. Every other spelling is now rejected at the declaration.
