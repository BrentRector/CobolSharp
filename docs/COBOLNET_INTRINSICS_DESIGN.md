...
**Rationale.** Replaces the legacy ad-hoc AlphanumericFunctions HashSet + scattered special-cases with one table that the NumPrimary, DISPLAY, MOVE-source, and condition-operand paths all consult. Decades-sustainable: adding a function is one row. Fits the parse-tree-direct emitter (no bound nodes/IR).
...
EMITTER HOOK — extend NumPrimary's existing TODO cell + a parallel string path:

  // in NumPrimary(...) — the existing "0L /* TODO: function-call operand */" returns:
...
FIGURATIVE CONSTANTS — context-materialized at the use site (CobolNet.Binding.Figurative):
  ...
  // MOVE LOW-VALUES  TO X → CobolString.Store(new string(' ', width), width)
...
### SET cond-name TO TRUE, SET index, and class/condition-name conditions all depend on data-division features the current DataBinder skips (levels 66/88, OCCURS INDEXED BY).

Design the SET dispatch slots now (index→long, cond-name→store the 88's first VALUE into its parent, switch→bool, pointer→ManagedPointer) but flag the hard dependency on G2-continuation (88-level binding + INDEXED BY) and stub the cond-name/index arms until those land — the catalog/registry spines do not depend on this.
