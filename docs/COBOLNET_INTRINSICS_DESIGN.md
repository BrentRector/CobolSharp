...
BINDER HOOK — replace `StatementBinder`'s `BoundExprError("function-call operand")` cell with a structured node the
...
### SET cond-name TO TRUE, SET index, and class/condition-name conditions depend on data-division features that have PARTLY landed (88-level binding + OCCURS INDEXED BY are in DataBinder now; level 66 is still skipped).

Design the SET dispatch slots now (index→long, cond-name→store the 88's first VALUE into its parent, switch→bool, pointer→ManagedPointer) — the 88-level binding and INDEXED BY dependencies have since landed (`Condition88`, DataBinder index-name registration), so the cond-name/index arms can be implemented directly; only the level-66 and switch-mnemonic dependencies remain stubbed — the catalog/registry spines do not depend on this.
