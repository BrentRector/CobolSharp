...
| Layout | one walk, two products: typed records → field list (offsets only for islands); byte islands → today's offsets. REDEFINES island sizing stays here. **CORRECTION (DEVLOG 400, owner decision):** the *live* layout pass is `StorageLayoutComputer` (Semantics, `Compilation.cs:123`), **not** `RecordLayoutBuilder` — the latter is currently **dead** (its `IrRecordType`/`IrField` output has zero `IrLoadField`/`IrStoreField` consumers; `ARCHITECTURE_ASSESSMENT.md` item 17), with one live side-effect (`DataSymbol.ElementSize`). The owner approved **building the real record-`struct` storage substrate** (this §9 intent, Option B) — typed items live as fields of an emitted `record struct` while byte-backed items remain in the `ProgramState` byte areas (the §1.6 safety floor); the two meet only at the §2.5 `IDataSlot` chokepoint. Concrete staged engineering plan: **`docs/RECORD_STRUCT_STORAGE_DESIGN.md`**. The dead `RecordLayoutBuilder`/`IrRecordType`/`IrLoadField`/`IrStoreField` are rebuilt as the real producer (preserving the `ElementSize` side-effect), not left vestigial. |
