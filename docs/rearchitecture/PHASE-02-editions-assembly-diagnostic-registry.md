...
`IN PROGRESS @ step 7` (started 2026-07-07; recon wf_9944fe61-fcc complete; baseline green: conformance 2055 · unit 224 · legacy guard NIST 353 MATCH). Steps 1–6 DONE (7 commits, all pushed to main); steps 7–11 remain.
...
>
> **⛔ RESUME AT step 7 — READ THESE GOTCHAS FIRST (predicate stamping is the riskiest step):**
> 1. **`GateId` already exists** (generated, 55 members, `src/Cobol.Net.Editions/Gating/GateId.cs`) + a `GateIds.ConstructId(GateId)→id` map. `EditionInfo`/`Gate` do NOT yet exist on `CobolParserCoreBase` (`src/Cobol.Net.Frontend/Parsing/CobolParserCoreBase.cs`, `namespace CobolNet.Frontend.Generated`, `DialectLevel {get;set;}=85` @17, `is85/2002/2014/2023` @19-22 delegate to `DialectLevel>=N`). Frontend references Editions (P2.1), so add `using CobolNet.Editions;`.
> 2. **The parse-layer 0900 MESSAGE FORMAT differs** between `EditionGateHints` (`{Display} requires COBOL-{IntroducedIn} (targeting COBOL-{dialect}) — {Citation} (constructs.json row {RowId})`) and `ConstructRegistry.Check` (`{Display} … — {where} ({Citation})`). So switching `CobolErrorStrategy.cs:113-117` to the Check path CHANGES the emitted 0900 text → `EditionGateDiagnosticTests` (Conformance) must be re-baselined (code unchanged, wording changes — a reviewed gate-2 diagnostic-golden change). The `{where}` for the parse layer should be chosen to read well (e.g. the construct's grammar context).
> 3. **The 49 plain `{isXXXX()}?` gates** (6 .g4 files under `src/Cobol.Net.Frontend/Grammar/`) map many-to-one onto GateId (retryPhrase ×6=Retry etc.). **3 compound sites need care:** `CobolExpressions.g4:111 {is2002() && boolExprAhead()}?` = a REAL gate (hand-convert, keep `boolExprAhead()`); `CobolData.g4:382/389 {!(is2002() && LA(1)==PROPERTY)}?` = NEGATIVE disambiguation guards, MUST NOT be stamped (leave calling `is2002()`). 5 grammar-gated constructs have no GateId row (inlineMethodInvocation-2023, parameterDescription-2002, raisingClause-2002, readAdvancingOnLock-2002, recordLockPhrase-2002) — decide per-gate.
> 4. **ANY .g4 or `CobolParserCoreBase` change touches the SHARED frontend → FULL legacy guard** (`bash scripts/guard-fast.sh` on Windows Git Bash; NIST 353 MATCH), one fragment at a time. `Frontend.cs` is at `Pipeline/Frontend.cs` (sets parser DialectLevel @:125, `parser.Reset()` @:143 between SLL/LL — reset `LastRejectedGate`/`LastRejectedTokenIndex` on BOTH). Full drift-corrected recon in `scratchpad/P2_ASBUILT_NOTES.md` (may be gone after compaction — the corrections are also in DEVLOG 670-676).
> 5. **Step 8 (delete EditionGateHints) gate:** the go/no-go is `EditionGateDiagnosticTests` identical between the new stamped path and the old table (run both in parallel one commit). The JSON/XML `COBOL0313` branch (EditionGateHints.cs:86-89, msg substring "not an ISO/IEC 1989 construct" pinned by EditionGateDiagnosticTests.cs:208) must be re-homed into CobolErrorStrategy as a token-TYPE check (rule stack is unwound — token type only).
