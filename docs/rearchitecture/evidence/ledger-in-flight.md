  ...
  <div class="cardgrid">
    <div class="card">
      ...
      <ul>
        ...
        <li><strong>Q4 — answered (PB371): CONFORMS, with a witness pinning the refusal.</strong> A syntax rule that constrains a <em>claimed</em> statement, but whose antecedent only a <em>declined</em> module can create, closes on the negative golden that pins the antecedent's named diagnostic; that settles the family of fourteen across OPEN, READ, REWRITE, UNLOCK, CLOSE, DELETE, LOCK MODE and SAME. The same implementer brief carries the CLASS clause, declined alongside VALIDATE.</li>
    ...
    <div class="card">
      ...
      <ul>
        ...
        <li><strong>PB377 — the collation key cache under contention</strong> — a Linux-only red in three of eight unit runs; reproduce under load, find the mechanism, prove the failure branch before trusting a green</li>      </ul>
    </div>
  </div>
