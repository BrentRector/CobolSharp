...
-->
  <div class="flight">
    <h3>Six landing trains and a fourth golden round since the pause; the CONFORMS-but-untested population is down to the four rows that wait on the owner; train 7 is forming <span class="pill good">LANDING</span></h3>
    ...
    <ol>
      <li><strong>Fix lane, at its cap</strong> — train 6 landed: <strong>PB253</strong> (the arithmetic mode decides a prose-float function's container, asked first), <strong>PB254</strong> (a total function taken out of a partial function's landing), <strong>PB288</strong> (the CALL argument crossing lands with the standard's COMPUTE semantics, not a raise) and <strong>PB292</strong> (RECORD DELIMITER declined by name on both arms). Train 7 holds <strong>PB231</strong>'s NATIONAL third (a BASED record with a national leaf carried on the byte window) and <strong>PB623</strong> (one exact float-to-fixed landing for every channel); running toward it: <strong>PB297</strong> (a reference-modified operand against LOW-VALUE/HIGH-VALUE), <strong>PB639</strong> (the inline arithmetic store's wrap) and <strong>PB635</strong> (a float trailing argument mis-routing a string function). Every implementer this night has found its note's mechanism wrong in some respect and reported it; twenty-two of those findings are now notes of their own.</li>
