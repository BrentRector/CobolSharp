...
-->
  <div class="flight">
    <h3>Four landing trains in one night; battery #47 at train 4's head is ALL GREEN with a zero-flip differential — the first since #43 with nothing to attribute <span class="pill good">LANDING</span></h3>
    ...
    <p><strong>The instruments.</strong> Batteries #44, #45 and #46 were each attributed to the row: #44's two differential flips were one pre-existing hole (no level-number validation), fixed as <strong>PB485</strong> in train 3; #45's five were that pair, two rejections the standard licenses, and one over-rejection the VALUE screen completed at level 88 (<strong>PB598</strong>, fixed in train 4); #46 is all green on every compiler leg and names exactly that PB598 flip; #47, at train 4's head, prints ALL GREEN with the differential at zero after one spec-licensed rebaseline (a program-prototype-name the new registry makes legal source) — the first battery since #43 with nothing to attribute. A pinned build at that head is cut for golden-lane round 4's validation.</p>
    <ol>
      <li><strong>Fix lane, at its cap</strong> — train 5 landed (PB248's integer-argument screen made one primitive with five readers, retiring a golden that pinned the defect; PB250's <span class="mono">&gt;&gt;COBOL-WORDS</span> reaching every §8.9/§8.10 word through one resolution rule; PB251's NUMVAL and NUMVAL-C on the SDIDI carrier in every arithmetic mode; PB252's exact carrier no longer wrapping in SUM, RANGE or MEAN). <strong>PB254</strong>, <strong>PB288</strong> and <strong>PB292</strong> are running; <strong>PB253</strong> (the arithmetic mode decides a prose-float function's container, asked first) opens train 6.</li>
      ...
      <li><strong>Golden lane</strong> — round 4 drafted nineteen spec-derived goldens for the rows batch 3 left resolved-but-untested (every refuter correction applied) and is validating them against a build pinned at train 4's head; four rows stay open on purpose — two wait on the owner's derivation signature, two on §13.18.52 A.1 rows the conformance document does not yet carry.</li>
