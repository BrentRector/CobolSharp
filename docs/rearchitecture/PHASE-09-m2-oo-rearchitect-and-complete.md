...
- **P4 done?** The generated parser namespace is `CobolNet.Frontend.Generated` (NOT `CobolSharp.Compiler.Generated`) and a typed `Cst` façade exists. If OO files still `using CobolSharp.Compiler.Generated;` and `using Core = CobolParserCore;`, P4 is not merged — the moves below assume the P4 namespace. (Current head still shows `using CobolSharp.Compiler.Generated;` at `CSharpEmitter.Call.cs:9` and `OoClassTable.cs:3` — expect these renamed by P4.)
