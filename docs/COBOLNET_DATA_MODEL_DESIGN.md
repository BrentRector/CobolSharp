...
Default (no VALUE): alphanumeric→spaces, numeric→0 (unscaled), index→1, pointer→null. Already implemented for flat items. Extensions needed:
  ...
  • OCCURS with VALUE: ISO permits VALUE on a table item → every element initialized to that value (Format 1 GR9). Emit a collection-expression / array initializer: `Items = [.. Enumerable.Repeat(elemInit, n)]` or an explicit `new _T_Item[]{ init, init, … }`. The **Format 2 (table) VALUE** (ISO §13.18.63.2, COBOL-2002) is NOT a bare one-literal-per-element positional list — it REQUIRES a `FROM (subscript-1)` phrase (optional `TO (subscript-2)`) keying a literal list to occurrence RANGES with cyclic reuse (GR13), no-TO = fill to the maximum (GR14), later-FROM-wins on overlap (GR15), and the GR16 dynamic-capacity computation. Emit per-occurrence: a fixed table becomes `new _T_Item[]{ ElemInit(1), …, ElemInit(n) }` (default outside every range — §13.18.63.4 leaves those occurrences UNDEFINED, so this is the implementation default, not a spec guarantee), a dynamic table a `CobolDynTable` opened at the GR16 initial capacity with a per-occurrence seed. **ONE occurrence→literal map serves BOTH storage lanes** — `ValueInitializer.ResolveTableValueMap` (GR12–GR15), read by `ValueInitializer.TableValueInit` for the record-struct array literal AND by `GroupImageCodec.ImageInitOf` for the character-image backings (Tier-B REDEFINES, EXTERNAL cells, BASED records, OO backings), which composes the occurrences' images instead of repeating one (kb/Work PB208: it used to read only `item.RawValue` — null for a table VALUE — and `StrRepeat` the VALUE-less default, discarding the seed silently for every image-stored leaf). **The literals are screened exactly as a format-1 literal is**: §13.18.63.3 SR2 is an ALL FORMATS rule and SR16 carries SRs 10–15 into format 2, so `DataBinder.ValidateTableValues` puts each occurrence-literal through `DataBinder.ScreenValueLiteral` — the ONE funnel the format-1 `RawValue` takes — and stores what it returns, so the `--permissive` numeric rewrite reaches the emitter's per-occurrence override too. Landable today = an ELEMENTARY single-dimension table on its own OCCURS entry (both emit lanes guard `!IsGroup`); a multi-dimension odometer or a subordinate-item table VALUE is staged (COBOLNET0899, P14 GAP), and a GROUP entry's table VALUE is a group-level VALUE governed by SR13/SR14 through `CheckGroupValueDeclarations` and `GroupValueSlicer.AreaTextOf`.
