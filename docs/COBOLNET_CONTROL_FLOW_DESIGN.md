...
### D8. EXIT PARAGRAPH → `pc = myIdx+1; break;` (return from current paragraph). EXIT SECTION → `pc = sectionEndPc+1; if (__exitPc == sectionEndPc) return pc; break;` (transfer to the section end AND fire the section's return mechanism). EXIT (bare) is a no-op (ISO §14.9.14: common end point). CONTINUE (bare, §14.9.9) → no statement emitted (no-op); CONTINUE AFTER arithmetic-expression-1 SECONDS → suspend execution for the interval, raising EC-CONTINUE-LESS-THAN-ZERO when the count evaluates below zero (§14.9.9 GR1).

**Rationale (§14.9.14.4 GR6/GR7; §14.9.14.3 SR9).** EXIT PARAGRAPH (GR6) transfers to the current paragraph's end — `pc=myIdx+1; break;` — and the bounded dispatch's own `__atExit` tail-check (`if (__atExit && __pc == __exitPc+1) return __pc;`) returns from a `PERFORM PARA` / `PERFORM … THRU PARA` when that paragraph IS the exit, via the same `break` path. EXIT SECTION (GR7) transfers to "the unnamed empty paragraph that follows the last paragraph of the current section, preceding any return mechanisms for that section" — it must FIRE the section's PERFORM/SORT/USE return, not merely move the pc. The emit therefore sets `pc = sectionEndPc+1` and adds an EXPLICIT `if (__exitPc == sectionEndPc) return pc;`: a mid-section EXIT SECTION cannot reach the `__atExit` tail-check (which captures the *current* pc, not the section end), so the explicit test is what replicates the return for a `PERFORM SECTION` / `PERFORM … THRU` the section end / SORT-or-USE procedure / the top-level end wall. The `break` handles the exit≠section-end fall-through (top level → the next section; a THRU range extending past the section end). §14.9.14.3 SR9: the SECTION phrase may be specified only in a section (`ctx.CurrentSection` null ⇒ COBOLNET0827).

**Rejected alternative — a bare `pc = sectionEndPc+1; break;` for EXIT SECTION.** Incorrect from a mid-section paragraph of a PERFORMed section: `__atExit` is false there, so the bounded loop does not return and instead runs the FOLLOWING section — violating GR7's "preceding any return mechanisms for that section." The section return must be fired explicitly.
