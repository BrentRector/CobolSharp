...
- ⭐ **FIVE CLUSTERS PER LANDING (target 5; 4–6 is the band).** A landing is ~90 % fixed cost — bring the work in,
  ...
  diff in turn, **one build**, the **WHOLE Conformance assembly, unfiltered** (never a union of the implementers' filter terms — CI's `rest` shard is exactly the tests no term names, and it was red on the first push of trains 39, 40 and 41 in one day), plus Unit and Characterization, **one commit per cluster inside the landing** so
