...
| USAGE COMP-5 | Implemented | UsageKind.Comp5; PicRuntime.cs:`DecodeComp5`/`EncodeComp5`; grammar: COMP_5/COMPUTATIONAL_5 tokens | Extension | Native binary (little-endian, full binary capacity); sizing identical to COMP; overflow based on binary capacity, not PIC digit count |
...
| RENAMES (level 66) | Implemented | DataSymbol.cs:`RenamesInfo`; SemanticBuilder.cs:`ResolveRenames`; StorageLayoutComputer.cs:`LayoutRenames`; DiagnosticDescriptors CBL0810-0812 | Spec-true | Full end-to-end: parse renamesClause, resolve FROM/THRU targets, validate (no level-66/88 targets), compute storage alias as contiguous byte range, bind via GetStorageLocation, emit. THROUGH synonym supported |
...
| COMP-5 native binary | Implemented | PicRuntime.cs:`DecodeComp5`/`EncodeComp5`; `WouldOverflow` Comp5 case | Extension | Little-endian encode/decode via BinaryPrimitives; overflow based on binary capacity, not PIC digit count |
