...
| ALTER | Not implemented | Grammar:CobolControlFlow.g4 (parsed), no bound node | N/A | Parsed but silently ignored; no BoundAlterStatement exists. Obsolete in COBOL-85 |
...
| Sign conditions | Not implemented | Grammar:CobolExpressions.g4 (parsed as signCondition), no binding | N/A | Grammar rule exists; BoundTreeBuilder:BindUnaryLogical skips signCondition (only handles comparisonExpression) |
...
| Negated conditions (NOT) | Partially | BoundNodes:BoundBinaryExpression (Not in enum), Grammar:unaryLogicalExpression | Likely incorrect | NOT keyword parsed in grammar but BindUnaryLogical ignores it (line 2579 skips to comparisonExpression). NOT works only inside relational operators (NOT EQUAL) and class/condition-name negation, not as general logical NOT |
