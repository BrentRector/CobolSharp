...
**Spec mapping.** `BoundSearch` represents both formats of the SEARCH verb (ISO §14.9.37): Format 1 serial scan and Format 2 `SEARCH ALL` binary/key-ordered scan, distinguished by the `IsAll` flag, which selects ONE OF TWO LOWERINGS — the two formats' scans are written separately because §14.9.37.4 GR4 and GR9 disagree about the range the search index may be left in (D14, kb/Work PB447). Each `WHEN` arm becomes one `BoundSearchWhen(Condition, Statements)`; the optional `AT END` block is `AtEnd`. `VARYING` binds to `AlsoVaried` — a same-table index IS the search index (GR3 c) 1.), a foreign index (GR3 c) 2.) or data item (GR3 b) rides in step, taking the final increment with it. Format 2 has no VARYING phrase, so `AlsoVaried` is null there. See [[kb/Spec/Lookup/Grammar]], [[kb/Spec/Lookup/IR Mapping]], [[kb/Spec/Language Features]].

**Semantic rules.** Binder-invariant (`SearchBinder`): the operand must be a table with `INDEXED BY` (§14.9.37.3 SR2) — else `BoundUnsupported`; `NOT AT END` is rejected as non-ISO. Nested dynamic-capacity tables lacking a whole-table path are refused (D9). EC gating (§14.9.37.4 GR4) is edition/turn-conditional via `ctx.EcState.Turn`: `CheckSearchIndex` (serial only) and `CheckSearchNoMatch`; `SEARCH ALL` forces index 1 (GR9) so only NO-MATCH can arise. See [[kb/Semantics/Validation Rules]], [[kb/Spec/Lookup/Semantic Rules]], [[kb/Semantics/Passes]].

**Runtime behavior.** `ControlFlowEmitter.EmitSearch` is the entry for BOTH formats; `EmitSearchScan` is the shared FRAME (the scan bound, the EC-raise plumbing, the AT-END funnel — GR1 b) is an ALL FORMATS rule) and it dispatches to ONE LOWERING PER FORMAT over an inline label-loop (index-name a `long`). `EmitSerialScan` (GR4): the initial-index guard (`<1 || >bound`), WHEN tests in order, then advance-by-1 and judge — so an unsuccessful serial search ends at N+1, which GR4 contemplates. `EmitAllScan` (GR9): index := 1, then WHEN tests, then the bound is judged BEFORE advancing — so the index never exceeds the last element, and an empty table is probed not at all. `EmitSearchWhens` is the ONE WHEN emission (GR1/GR2). Failure funnels to `AT END`. Range ECs raise via `ExceptionState.Set`; a dynamic table brackets the scan with `EnterSearch`/`ExitSearch` in try/finally (EC-FLOW-SEARCH, §14.9.39 GR31). See [[kb/Runtime/Execution Model]], [[kb/Spec/Lookup/Runtime Mapping]].

```text
BoundSearch(IsAll=false)     --[ §14.9.37 F1 GR3/GR4 serial from the CURRENT index ]-> EmitSearchScan → EmitSerialScan: init guard (idx<1||>bound), WHEN order, +1 THEN judge (ends N+1)
BoundSearch(IsAll=true)      --[ §14.9.37 F2 GR9 initial setting ignored ]-----------> EmitSearchScan → EmitAllScan: idx:=1, WHEN order, judge THEN +1 (never leaves [1,N])
  └─ BoundSearchWhen         --[ §14.9.37.4 GR1 re-eval per probe ]-----------------> if(cond){ body; goto __searchEnd } (per-eval UDF wrapper)
     range EC (GR4/GR9)      --[ EC-RANGE-SEARCH-INDEX / -NO-MATCH ]-----------------> ExceptionState.Set(...)
