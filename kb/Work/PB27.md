...
## ▶ TRIAGE (2026-08-06) — `under_rejects` added

Flagged while fixing the ranker that had been hiding this class of item ([[PB48]]'s sweep). The note carried
`silent` alone, and `silent` is a QUALIFIER on a harm rather than a harm — so the item scored as doing nothing
to a user's program and never appeared in `work.py next`.

Its own text says what the harm is: **the SPECIAL-NAMES `LOCALE` clause is SILENTLY ACCEPTED** by the
`genericClause` catch-all instead of reaching the ratified `COBOLNET1518` non-support diagnostic, and
`LOCALE-TIME-FROM-SECONDS` carries `IntroducedIn 2002` for a function that belongs to the 2014 package. Both are
source this compiler admits and should refuse — `under_rejects`, with `silent` correctly retained alongside it.
