...
> docs). Two distinct workstreams: **§A 16 items**, **§B empty** (the fix queue is worked out; it refills from the review). See [[kb/Modernization/Tasks]].
...
## B. Conformance-fix-queue remainder (0 — the queue is EMPTY)

A **distinct** workstream from §A: verified, spec-derived defect *fixes*, not unimplemented constructs. The SSOT is
[[docs/rearchitecture/CONFORMANCE-FIX-QUEUE]] and its LANDED header is the tally — do not enumerate item IDs here,
they drift. Mirror the count only.

**⛔ EMPTY IS NOT DONE.** The queue empties by being WORKED and refills from the Phase-B traceability review, which
is the design working as intended: adjudicating a clause OPENS items, fixing them CLOSES them. An empty queue means
"adjudicate the next clause", never "no known defects". The three older campaigns are closed — the 46-finding audit
(45 landed, 1 refuted), the discovered set DA1–DA7, and the review-fed set PB1–PB9 — so the next entry will come
from the next batch.

- [x] **The 46-finding audit** — closed (45 landed, CA12 refuted).
- [x] **DA1–DA7** — the discovered-during-implementation set, closed.
- [x] **PB1–PB9** — opened by the traceability review and closed; two were SILENT blockers (PB5, PB7).

**The live burn-down is now the traceability inventory, not this section** — `pwsh scripts/session-probe.ps1`
reports rows/GAP, and plan §0 owns the worklist.
...
**COMPLETE for the page-by-page sweep** — all 1261 pages reconciled, every claim adjudicated. ⚠ That scopes the SWEEP, not the transcription: a defect of exactly this class (a page-boundary run-in in §8.11's intrinsic-name list) was found and repaired AFTER it closed, found by using the file rather than by re-sweeping it. Treat the count as 'what that pass found', not as a proof of absence.
...
**Repair discipline:** correct each FAMILY as a whole — the seven ON/OFF directive notes must agree with each other or the inconsistency merely moves. After each family re-run `python scripts/spec/extract_rule_catalog.py` (the catalog derives from this file) and `python scripts/spec/lint_rendering.py` (the only gate that measures LEGIBILITY rather than fidelity). ⛔ The old instruction to confirm a page-anchor count of 1261 is GONE: pages were REMOVED from the transcription (it is clause-structured now), so there are no `#page-N` anchors left to count and `render-spec-page.py` reads the PDF directly.
