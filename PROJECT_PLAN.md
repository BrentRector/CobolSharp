...
| 2026-06-08 | **OO goes TYPED-NATIVE (ADR §7) + the multi-method keystone (DEVLOG 453–456).** Owner course-correction: OO object data must be per-instance typed .NET fields, NOT the byte `ProgramState` `State`; the `EnableTypedFields` default-OFF gate is migration-safety for the LEGACY corpus, so OO is an **always-on consumer** of the typed pipeline (memory `feedback_oo_typed_native_not_byte`). Landed, each guard-green + conformance-tested + (455) adversarially reviewed: **453** object char data → per-instance `.NET string`; **454** object numeric → per-instance `long`/`decimal` + the general typed-numeric→byte MOVE cell; **455** multi-method classes (N `METHOD-ID`/class, each its own .NET method with an exit-bounded dispatch range; per-method paragraph scope threaded through SemanticBuilder/ReferenceResolver/BoundTreeBuilder/ProcedureNameResolver/Binder fixes CBL3104) + `INVOKE SELF`→`callvirt` incl. **polymorphic** across INHERITS (COBOL0112 lifted) — a 3-agent adversarial panel confirmed the core correct and surfaced edge gaps now failing LOUD: **COBOL0116** (SECTION-in-method, was a silent skip) + **COBOL0117** (multi-method class WITH method params — `FindLinkageField` is module-level; per-method LINKAGE is a later slice); **456** object group → per-instance `record struct` + `OCCURS` table → per-instance typed array (byte `State` now **empty** on a fully-typed class). Instance dimension via one chokepoint `CilDataEmitter.EmitTypedFieldOwner` (typed analogue of the byte engine's `EmitLoadBackingArray`). New conformance: `oo_instance_data`/`oo_self`/`oo_self_polymorphic`/`oo_object_group` (9 `oo_*` total) + `OoTests`. Guard ALL GREEN **1204 unit / 535 integration / 364 NIST**; latest commit `a10bda9`. | **(a) subclass own typed OBJECT data** (lift COBOL0113 — factor the typed field-defs + typed-init out of `CilEmitter.EmitProgramState`, emit the subclass's own typed instance fields + init in the subclass ctor, reject byte-island subclass data which has no own State); **(b) per-method LINKAGE** (multi-method-with-args — per-method LINKAGE layout + `FindLinkageField`, lifts COBOL0117) + typed-field-by-reference INVOKE args; **(c) FACTORY**; then PROPERTY / universal-ref+EC; then the rest of M2 → M3 → M4 (`docs/ISO2023_CONFORMANCE_PLAN.md` §3). |
