...
/// settable; switch scope is the RUN UNIT (one switch shared by all runtime elements — per-process static state);
/// the external facility that supplies the initial status is the process environment — variable
/// <c>COBOL_&lt;SWITCH-NAME&gt;</c> (hyphens become underscores, upper-cased), value <c>ON</c> | <c>1</c> |
/// <c>TRUE</c> (case-insensitive) = on, anything else or absent = off. The probed value is cached on first read,
/// so a later <c>SET</c> governs for the remainder of the run unit.
/// </summary>
public static class ExternalSwitches
{
    /// <summary>Per-process switch state keyed by implementor switch-name — the §12.3.7 GR4 NOTE 1 run-unit
    /// scope (one switch referenced by all runtime elements of the run unit).</summary>
    private static readonly ConcurrentDictionary<string, bool> States = new(StringComparer.OrdinalIgnoreCase);
    ...
    public static bool Get(string implementorName)
    {
        if (States.TryGetValue(implementorName, out bool state)) return state;
    ...
    public static void Set(string implementorName, bool isOn) => States[implementorName] = isOn;
