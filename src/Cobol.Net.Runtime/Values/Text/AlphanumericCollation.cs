...
/// indexed keys, MAX/MIN) read <see cref="Positions"/> raw: their <c>c &lt; 256 ? positions[c] : c</c> weight
/// tail is ORDER-EQUIVALENT to <see cref="Weight"/> — both rules place every above-block code unit after
/// every positioned one, strictly increasing in code — so only the functions that expose the position
/// NUMBER itself need the exact arithmetic.
/// </summary>
public sealed class AlphanumericCollation
{
    ...
    public AlphanumericCollation(ushort[] positions, ushort[] repByPos, int nextFree)
    {
    ...
    /// <summary>The raw 256-entry position table — the comparison paths' carrier (order-equivalent tail; see
    /// the class summary).</summary>
    ...
    public int PositionCount => _nextFree + (0x10000 - _positions.Length);
    ...
    public int Weight(char c) => c < _positions.Length ? _positions[c] : _nextFree + (c - _positions.Length);
