...
public sealed class FileRegistry
{
    ...
    public bool Read(string name, bool previous, out string image)
    {
        if (_files.TryGetValue(name, out var c) && c is SequentialConnector f) return f.Read(previous, out image);
        image = "";
        return false;
    }
