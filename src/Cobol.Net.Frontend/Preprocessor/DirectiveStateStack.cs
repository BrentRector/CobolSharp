...
/// <summary>
/// The LINE-SCOPED directive events one stage collects, with the PUSH/POP history applied — the carrier of every
/// directive whose state is a fold over its events (TURN, REF-MOD-ZERO-LENGTH, FLAG-02, FLAG-14, and the
/// group-prefix LEAP-SECOND / COBOL-WORDS).
///
/// <para>For such a directive "the state" at a line is the fold of the events in effect there, so SAVING it is
/// remembering how many events were in effect, and RESTORING it is revoking every event recorded since: an event
/// is REVOKED at the POP line, and from that line on it is as though it had never been written — which is
/// exactly §7.3.20.4 GR1's "the state … shall be restored". The events are never reordered or synthesized, so a
/// fold that walks them in line order needs one extra test (<see cref="DirectiveTimeline{T}.InEffectAt"/>) and
/// nothing else.</para>
/// </summary>
public sealed class DirectiveEventLog<T>
{
    private readonly List<T> _events = [];
    private readonly List<int> _revokedAt = [];
    private readonly Dictionary<string, List<int>> _live = new(StringComparer.Ordinal);

    /// <summary>Record <paramref name="ev"/>, a directive of <paramref name="row"/>, in line order.</summary>
    public void Add(string row, T ev)
    {
        Live(row).Add(_events.Count);
        _events.Add(ev);
        _revokedAt.Add(int.MaxValue);
    }

    /// <summary>The carrier of <paramref name="row"/>'s share of this log — the rows sharing one log (FLAG-02 and
    /// FLAG-14 share <see cref="FlagEvent"/>) are saved and restored independently.</summary>
    public IDirectiveStateCarrier CarrierFor(string row) => new RowCarrier(this, row);

    /// <summary>The events, each with the line from which a POP revoked it.</summary>
    public DirectiveTimeline<T> ToTimeline() => new([.. _events], [.. _revokedAt]);

    private List<int> Live(string row)
    {
        if (!_live.TryGetValue(row, out var live)) _live[row] = live = [];
        return live;
    }

    private sealed class RowCarrier(DirectiveEventLog<T> log, string row) : IDirectiveStateCarrier
    {
        public object? Save() => log.Live(row).Count;

        public void Restore(object? saved, int popLine)
        {
            var live = log.Live(row);
            int keep = (int)saved!;
            for (int k = keep; k < live.Count; k++) log._revokedAt[live[k]] = popLine;
...
public sealed class DirectiveTimeline<T> : IReadOnlyList<T>
{
    ...
    internal DirectiveTimeline(IReadOnlyList<T> events, IReadOnlyList<int>? revokedAt)
    {
        _events = events;
    ...
    public static DirectiveTimeline<T> Of(IReadOnlyList<T>? events) =>
        events as DirectiveTimeline<T> ?? (events is null || events.Count == 0 ? Empty : new(events, null));

    /// <summary>Is event <paramref name="index"/> still in effect at a construct on <paramref name="siteLine"/>?
    /// False from the line of the POP that restored the state saved before it (§7.3.20.4 GR1/GR3). A directive
    /// occupies its own line, so no construct shares a POP's line.</summary>
