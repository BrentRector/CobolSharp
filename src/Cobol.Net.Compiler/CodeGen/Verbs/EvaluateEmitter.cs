...
internal sealed class EvaluateEmitter(EmitContext ctx, ConditionRenderer cond, CSharpEmitter host)
{
    ...
    public void Emit(BoundEvaluate ev)
    {
        ...
        foreach (var arm in ev.Whens)
        {
            ...
            using (w.Block(first ? $"if ({match})" : $"else if ({match})"))
                host.EmitStatementList(arm.Statements);
            first = false;
        }
        if (ev.Other is { } other)
        {
            if (first) { host.EmitStatementList(other); return; }   // only WHEN OTHER — unconditional
            using (w.Block("else"))
                host.EmitStatementList(other);
        }
    }
}
