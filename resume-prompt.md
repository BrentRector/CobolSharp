...
> `docs/rearchitecture/` (10 `DESIGN-*.md`, 6 `SURVEY`/`CRITIQUE-*.md`, 17 `PHASE-00..16-*.md`). **EXECUTION IN PROGRESS:
...
> gate. **PHASE 02 (`Cobol.Net.Editions` leaf + first-class diagnostic registry) ✅ DONE — all 11 steps
> (2026-07-08; DEVLOG 670–695, commits `62e09db1`→`8d79ea64`, all pushed; recon `wf_9944fe61-fcc`):** the edition machinery
...
> **Step 7 (DEVLOG 679) LANDED — the parse-layer 0900 flows through the ONE `ConstructRegistry.Check` funnel.** Forward
> `{Gate(edition, GateId)}?` predicate stamping was TRIED and adversarially DISPROVED (wf_b73eff97): ANTLR evaluates
> hoisted predicates SPECULATIVELY at the stuck token, so typos (`IF W = .`, a stray `)`, `SUPPRESS`) got a
> confidently-wrong 0900 (risk R1). So the stamping was reverted; landed instead: `CobolParserCoreBase` single-sources
...
> **✅ THE BIND-TIME GATING MIGRATION + PHASE-02 CLOSEOUT ARE COMPLETE** (owner: "implement correctly regardless of
> rework cost", DEVLOG 680). The decades-correct realization of steps 7/8: edition introduction-gating for every
> HARD-reserved construct (24 constructs / 11 clusters, DEVLOG 680–690) moved from parse-time REJECTION predicates to
> **bind-time `ConstructRegistry.Check`** at each construct's recognition point (the five-inline-gate pattern), deleting
> BOTH the parse predicates AND the reverse signatures for them. What remains at the parse layer is the irreducible
> reservation-word residue (XOR/booleans/SHARING/RETRY/UNLOCK/PROPERTY — user words below their edition; ungating would
> miscompile), now in the renamed `ReservedWordEditionHints` (DEVLOG 693). Forward `{Gate}?` predicate stamping was
> adversarially DISPROVED (wf_b73eff97, risk R1: ANTLR evaluates hoisted predicates SPECULATIVELY at the stuck token).
> **Then the closeout (this session, DEVLOG 692–695):** step 9 — the frontend preprocessor gates consume the ONE
> `EditionSeverityPolicy` (P2.9); step 10 — the first-class `DiagnosticDescriptor` registry
> (`Cobol.Net.Editions/Diagnostics/DiagnosticCatalog`): the `COBOLNET0899` catch-all split into ~40 addressable
...
> §9.5). **⛔🔀 RESUME AT: EXEC STEP B — P6 (Real Binder: `SymbolTable` + `BoundCompilation` + `BindPipeline`;
> `docs/rearchitecture/PHASE-06-*.md`). ✅ PHASE-07 Step 6 (the exhaustive visitor) is DONE incl. the 6h SYSTEMATIC AUDIT
> (DEVLOG 755–765).** The Roslyn source generator emits the 7 `IBound*Visitor` + `Accept` + `BoundStatementTree.StatementChildren`;
> EVERY completeness-critical bound-node dispatch is converted — emitter (6b), all 5 renderers (`NumericRenderer` 6d,
> `ConditionRenderer` 6e, `OperandText` 6f, `BooleanRenderer`, `IntrinsicRenderer`), `ArgExpr`, `StoreKindOf` (6c) — and
> all FIVE statement walkers (`UsageCollectionPass`, `VersionConformancePass.Recurse`, `ContainsNextSentence`,
...
> script; commercial-quality, decades-maintainable.** **✅ 6b DONE (DEVLOG 756):** `CSharpEmitter` now implements
> `IBoundStatementVisitor<bool>` — the 79-arm `EmitStatement` switch + its loud `default` became 79 `Visit` methods
...
> **RESIDUE-FIRST migration: ALL 7 gates DONE + the guesser DELETED** (each a green+pushed commit; DEVLOG 709–717):
> ✅ UNLOCK #5 (`2daec9cb`), ✅ PROPERTY #7 (`7311dfbd`), ✅ PD-RAISING #6 (`840b0abf`), ✅ XOR #1 (`d3cdae6c` — Batch A),
> ✅ SHARING #3 (`1b74f739` — Batch B, OPEN collision byte-safe), ✅ RETRY #4 (`4efcca71` — 6 grammar sites, forward-detect
> `retryPhraseAhead()` for OPEN, closed a real 4-site bind gap), ✅ boolean #2 (`3d0ec86e` — operator tiers + COMPUTE F2 +
...
> statements · 14h.3 phrase statements · 14h.4a clean expression/phrase · 14h.4b boolean-ops + national/boolean literals).
> This is the DEVLOG-724 root-cause fix: introduction/removal gates fire on the construct's syntactic PRESENCE, so a
> below-edition construct names its edition even when it ALSO fails to bind (the bound-arm dropped the 0900 on
> `BoundUnsupported`/`BoundNop` error paths). NO `.Syntax` back-ref was added — the `BoundTree.cs` invariant STANDS (the
> design's §2.2/§6 provisions realized as a two-arm pass). The binder now holds ZERO statement/expression edition Checks;
> 7 gates needing a resolved semantic fact STAY bound-arm (SET-object-ref/SET-ptr-UP-DOWN/INVOKE/READ-PREVIOUS/
...
> TYPE-clones + compiler temps; 8 exact-count witnesses). ✅ 14g.2 DONE (DEVLOG 733–734) — the 4 data-description-clause
> gates ALL to the parse-arm (recognition): 4 `ParseArm` overrides (BASED/TYPE/PROPERTY/TYPEDEF), each guarded against a
> level-66/88 mis-attachment. ⚠ TYPEDEF was reclassified bound→parse by the adversarial review (DEVLOG 734, 3 confirmed
> byte-neutrality defects: the typedef ITEM is discarded from `ConformanceForest` when `RegisterTypeDecl` rejects it
> (unnamed/duplicate) or it binds into method scope, so the bound-arm dropped the 0900 — the DEVLOG-724 class).
> `DataClauseEditionTests` 9 witnesses; byte-neutral (characterization 32 byte-exact — the content-sorted diag surface
> is blind to the arm change). ✅ 14g.3 DONE (DEVLOG 735–736) — OO class/interface definition + OCCURS DYNAMIC →
> parse-arm; `OoOccursDynEditionTests` 8 witnesses. ⚠ the adversarial review (DEVLOG 736) caught a SHARED-RULE hazard:
> `occursClause` is shared by data/report/screen grammars but the old gate reached only data — the bare walk over-fired
> OCCURS DYNAMIC in report groups (extra 0900) + screen sections (compiles→rejects); fixed by a POSITIVE
> `InGatedDataEntry` guard (fire only inside a non-66/88 `dataDescriptionEntry`, reused by all 5 data-clause gates).
> **Lesson: check whether a parse-arm gate's grammar rule is SHARED across sections.** ✅ 14g.4 DONE (DEVLOG 737) — 7
...
> Read `DESIGN-version-conformance-pipeline.md` FIRST. The dual-backend goal
...
> 2036/213-green baseline they describe is superseded by the current battery figures above; the go-forward PLAN is the rearchitecture
