...
- **Guard** (`bash scripts/guard.sh`): 1000 unit / 346 integration (345 pass + 1 unrelated skip) / **150 NIST baselines (94 NC + 42 IF + 12 SM + 2 IC)**. Must stay ALL GREEN; baselines must stay 0 FAIL*. (NC214M was dropped — live ACCEPT FROM DATE/DAY/TIME, inherently non-deterministic; DEVLOG 226.)
- **Suites:** NC 100%, **IF 100%** (42 baselined), **SM COPY-feature 100%** (12 baselined). **IC in progress: 18/47 CLEAN (IC224A + IC225A baselined, DEVLOG 233).** SQ/IX/RL/ST surveyed but paused.
...
1. **Finish IC** (current suite). DEVLOG 233 fixed transitive BY CONTENT → **IC224A + IC225A baselined** (guard now 150). Remaining, all substantial features (no quick wins):
   - **IC203A** (13 FAIL*): CANCEL "return to initial state" — the next CALL after CANCEL must re-initialize the subprogram's WORKING-STORAGE to its VALUE clauses. Subprogram loading already works (values change); only the re-init on re-activation is missing. Needs: refactor `.cctor` VALUE-init into a reusable init method + runtime activation/cancel tracking in `CobolProgramRegistry` + an activation check in the emitted `Entry`. Touches ALL programs' Entry codegen — regression-test carefully.
