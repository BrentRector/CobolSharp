...
#
# ⛔ THE ROW SHAPE, DECIDED ONCE FOR THE WHOLE CATALOG (fix-queue PB698): **one row per TOP-LEVEL numbered rule
# of a rule block; every sub-item — lettered `a)`, or numbered under one, at any depth — is FOLDED into its
# parent rule's text and is addressed by CITATION PATH, never by a row of its own.** Three independent things
# already said so and the extractor was the only dissenter:
...
# A row whose text cannot be read without its parent is not a rule but a FRAGMENT — `SR-14.9.41.3-L2.2` read
# "It has the same class, category, and usage as that record key.", whose subject and referent both live in the
# item above it. That is the same silent-false-confidence class as PB689's contamination, in the other direction.
#
# The transcription flattens the TAIL of some nested lists onto column 0 at a PAGE BREAK (§14.9.41.3 SR6 b) 2.
# and 3. are printed indented under b) on folio 755 — measured, x0=115.05 against that page's 80.03 body margin —
# and transcribed at column 0). Such a tail is EVIDENCE the segmenter needs: a column-0 ordinal that CONTINUES
...
def main() -> int:
    ...
    def flattened_tail(n: int, form: str) -> bool:
        ...
        a real rule and wins outright (that is what keeps `GR3` with an indented `1.-3.` list from swallowing
        `4\)`), and only then does an ordinal that continues the indented run count as the nested list's tail.
        """
