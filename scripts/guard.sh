...
echo "=== Integration tests (145 pass, 1 skip expected) ==="
...
for test in NC101A NC102A NC103A NC171A NC106A NC176A NC116A NC118A; do
    ...
    if diff <(sed 's/ *$//' tests/nist/valid/$test.txt) <(sed 's/ *$//' "$outfile") > /dev/null 2>&1; then
        echo "  $test: MATCH"
    else
        # Fallback: check print-file.txt for non-NIST compiled programs
